...
import { boardOrder, boardScore } from './boardOrder'
...
export function buildHeadlines({ players = [], headline, results, isLive, airRanked = [] }) {
  ...
  // 2026-09-25: #1 is the top of The Board (lib/boardOrder.js), not the top hr_score.
  const top = boardOrder(players)[0] || null
  ...
  if (top) out.push({ k: 'top', icon: '🎯', tag: 'THE BOT’S #1', name: nameOf(top), why: `${teamOf(top)}${clean(top?.pitcher_name, '') ? ` vs ${clean(top?.pitcher_name, '')}` : ''}${n(top?.pitcher_hr9, 0) > 0 ? ` · ${n(top?.pitcher_hr9, 0).toFixed(2)} HR/9` : ''}`, stat: boardScore(top) != null ? `Board ${boardScore(top).toFixed(0)}` : `HR ${hrScore(top).toFixed(0)}`, col: C.orange, p: top })
