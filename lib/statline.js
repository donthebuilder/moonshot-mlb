...
export const marketKey = (type) => {
  const t = String(type || 'hr').toLowerCase()
  if (t.startsWith('hrr')) return 'hrr'
  if (t.startsWith('hit')) return 'hit'
  // 'tb' is the Total Bases board's own key for the CONTACT market — it comes
  // in from RankedBoard as type="tb" and was silently falling through to the
  // HR stat set, so the total-bases cards were leading with barrel rate.
  if (t.startsWith('con') || t.startsWith('ctg') || t === 'tb') return 'contact'
