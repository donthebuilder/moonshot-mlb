...
import { lampTex, brickTex } from './stadiumWorld'
...
export function addParkProps(scene, { venue, P, wallD, wallH, steps = [], extra = [], inner = null }) {
  ...
  // `inner` (batch pass) is the world's bowl edge — past the poles it hugs
  // the foul lines, so a prop at -150° lands behind the 3B stands, not 400
  // ft out in the car park
  const beyond = (a, off, y = 0) => { const v = P((inner ? inner(a) : wallD(clamp45(a))) + off, a); v.y = y; return v }
  ...
  // ── A TOWER, NOT A BOX (v3, "why is everything so blocky"). A building
  //    is a box with a parapet, a lit crown line, mullioned windows, a
  //    setback on anything tall, and rooftop plant. The same builder draws
  //    the warehouse and every skyline block.
  const tower = (v, w, h, d, color, { windows = true, lit = 0.3, brick = false, setback = h > 140, spire = false } = {}) => {
    const grp = new THREE.Group()
    let mat
    if (brick) {
      const t = brickTex(color); t.repeat.set(Math.max(1, Math.round(w / 40)), Math.max(1, Math.round(h / 40)))
      mat = new THREE.MeshLambertMaterial({ map: t })
      mat.userData.surface = 'brick'; mat.userData.photoTint = 0xc0aaa0
      if (windows) {
        const wt = windowTex(Math.max(2, Math.round(w / 12)), Math.max(2, Math.round(h / 9)), 0x000000, 0.35)
        mat.emissiveMap = wt; mat.emissive = new THREE.Color(0xffd9a0); mat.emissiveIntensity = 0.45
      }
    } else {
      mat = new THREE.MeshLambertMaterial({ map: windows ? windowTex(Math.max(2, Math.round(w / 12)), Math.max(2, Math.round(h / 9)), color, lit) : null, color: windows ? 0xffffff : color })
    }
    const hMain = setback ? h * 0.68 : h
    const body = new THREE.Mesh(new THREE.BoxGeometry(w, hMain, d), mat)
    body.position.y = hMain / 2; grp.add(body)
    const trim = lam(new THREE.Color(color).multiplyScalar(0.7))
    const parapet = new THREE.Mesh(new THREE.BoxGeometry(w + 2, 2.5, d + 2), trim)
    parapet.position.y = hMain + 1.25; grp.add(parapet)
    const crown = new THREE.Mesh(new THREE.BoxGeometry(w + 2.2, 0.6, d + 2.2), basic(0xffe2b0, { fog: false }))
    crown.position.y = hMain + 2.6; grp.add(crown)
    if (setback) {
      const w2 = w * 0.62, d2 = d * 0.7, h2 = h - hMain
      const top = new THREE.Mesh(new THREE.BoxGeometry(w2, h2, d2), mat)
      top.position.y = hMain + h2 / 2; grp.add(top)
      const cap = new THREE.Mesh(new THREE.BoxGeometry(w2 + 2, 2, d2 + 2), trim)
      cap.position.y = h + 1; grp.add(cap)
      const crown2 = new THREE.Mesh(new THREE.BoxGeometry(w2 + 2.2, 0.6, d2 + 2.2), basic(0xffe2b0, { fog: false }))
      crown2.position.y = h + 2.2; grp.add(crown2)
    }
    // rooftop plant and a beacon
    const plant = new THREE.Mesh(new THREE.BoxGeometry(w * 0.18, 5, d * 0.25), trim)
    plant.position.set(-w * 0.25, (setback ? h : hMain) + 2.5, d * 0.2); grp.add(plant)
    if (spire) {
      const sp = new THREE.Mesh(new THREE.CylinderGeometry(0.3, 1.2, h * 0.22, 6), trim)
      sp.position.y = h + h * 0.11; grp.add(sp)
    }
    const beacon = new THREE.Mesh(new THREE.SphereGeometry(1.2, 6, 6), basic(0xff4040, { fog: false }))
    beacon.position.y = (spire ? h * 1.22 : h) + 3.5; grp.add(beacon)
    grp.position.copy(v); grp.position.y = 0
    faceHome(grp)
    scene.add(grp)
    return grp
  }
    ...
    switch (p.kind) {
      ...
      case 'pines': {
        ...
        for (let i = 0; i < p.n; i++) {
          ...
          // each tree is a group tagged for lib/stadiumLook.loadModels: when
          // public/models/pine.glb exists the cone is swapped for the model
          const g = new THREE.Group()
          g.userData.model = 'pine'; g.userData.h = h
          const trunk = new THREE.Mesh(new THREE.CylinderGeometry(0.7, 1.1, h * 0.55, 6), lam(0x3b2a1e))
          trunk.position.y = h * 0.275; g.add(trunk)
          const crown = new THREE.Mesh(new THREE.ConeGeometry(h * 0.22, h * 0.6, 7), lam(0x1f4a2c))
          crown.position.y = h * 0.55 + h * 0.3; g.add(crown)
          g.position.set(c.x + dir.x * t, 0, c.z + dir.z * t); g.rotation.y = Math.random() * Math.PI * 2
          scene.add(g)
        }
        break
      }
      ...
      case 'skyline': {
        ...
        for (let i = 0; i < p.n; i++) {
          ...
          const v = beyond(a, p.off + Math.random() * 120, 0)
          tower(v, 44 + Math.random() * 34, h, 40 + Math.random() * 20, p.color, { lit: 0.3, setback: h > p.h * 0.8, spire: Math.random() < 0.25 })
        }
        break
      }
      // ── BUILDING: one long box with windows (the warehouse)
      case 'building': {
        const v = beyond(p.a, p.off + p.d / 2, 0)
        tower(v, p.w, p.h, p.d, p.color, { windows: !!p.windows, lit: 0.28, brick: !!p.brick, setback: p.setback ?? p.h > 140, spire: !!p.spire })
        break
      }
      ...
      case 'videoboard': {
        ...
        box.position.copy(v); faceHome(box); scene.add(box)
        // a lit lip along the top and a service catwalk below the face
        const lip = new THREE.Mesh(new THREE.BoxGeometry(p.w + 2, 0.8, 9), basic(0xffe2b0, { fog: false }))
        lip.position.copy(v); lip.position.y += p.h / 2 + 0.6; faceHome(lip); scene.add(lip)
        const walk = new THREE.Mesh(new THREE.BoxGeometry(p.w + 4, 1.2, 12), lam(0x2a3038))
        walk.position.copy(v); walk.position.y -= p.h / 2 + 1; faceHome(walk); scene.add(walk)
        // the face (v3): what a board actually shows at night — a header
        // bar with the park's name, a video window of soft colour (a crowd
        // shot, out of focus), a lineup grid of dim rows either side, and
        // a bright score strip along the bottom. Nothing branded.
        const cv = document.createElement('canvas'); cv.width = 512; cv.height = 256
        const g = cv.getContext('2d')
        g.fillStyle = css(0x0a0f18); g.fillRect(0, 0, 512, 256)
        // header
        g.fillStyle = css(0x13203a); g.fillRect(0, 0, 512, 34)
        g.fillStyle = 'rgba(255,255,255,.9)'; g.font = '900 20px SF Mono, Menlo, monospace'; g.textAlign = 'left'; g.textBaseline = 'middle'
        g.fillText(String(p.text || '').toUpperCase(), 14, 17)
        // video window
        const vw = { x: 128, y: 42, w: 256, h: 150 }
        const vg = g.createLinearGradient(vw.x, vw.y, vw.x + vw.w, vw.y + vw.h)
        vg.addColorStop(0, css(0x2a4a7a)); vg.addColorStop(0.5, css(0x6a4a3a)); vg.addColorStop(1, css(0x1e3a5a))
        g.fillStyle = vg; g.fillRect(vw.x, vw.y, vw.w, vw.h)
        for (let i = 0; i < 40; i++) {
          const rg = g.createRadialGradient(vw.x + Math.random() * vw.w, vw.y + Math.random() * vw.h, 2, vw.x + Math.random() * vw.w, vw.y + Math.random() * vw.h, 30 + Math.random() * 40)
          const warm = Math.random() > 0.5
          rg.addColorStop(0, warm ? 'rgba(255,200,150,.35)' : 'rgba(150,200,255,.3)'); rg.addColorStop(1, 'rgba(0,0,0,0)')
          g.fillStyle = rg; g.fillRect(vw.x, vw.y, vw.w, vw.h)
        }
        // lineup grids
        for (const x0 of [12, 396]) for (let r = 0; r < 9; r++) {
          g.fillStyle = r % 2 ? 'rgba(255,255,255,.05)' : 'rgba(255,255,255,.09)'; g.fillRect(x0, 46 + r * 16, 104, 13)
          g.fillStyle = 'rgba(255,240,200,.55)'; g.fillRect(x0 + 4, 50 + r * 16, 18 + Math.random() * 40, 5)
          g.fillStyle = 'rgba(255,255,255,.35)'; g.fillRect(x0 + 84, 50 + r * 16, 14, 5)
        }
        // score strip
        g.fillStyle = css(0x0e1626); g.fillRect(0, 200, 512, 56)
        for (let i = 0; i < 12; i++) {
          g.fillStyle = 'rgba(255,255,255,.08)'; g.fillRect(140 + i * 22, 210, 18, 36)
          g.fillStyle = i < 9 ? 'rgba(255,214,90,.9)' : 'rgba(255,255,255,.9)'; g.font = '900 18px SF Mono, Menlo, monospace'; g.textAlign = 'center'
          g.fillText(String(i < 9 ? Math.floor(Math.random() * 3) : Math.floor(Math.random() * 9)), 149 + i * 22, 230)
        }
        g.fillStyle = 'rgba(255,255,255,.7)'; g.font = '900 12px SF Mono, Menlo, monospace'; g.textAlign = 'left'
        g.fillText('R  H  E', 410, 214)
        const tex = new THREE.CanvasTexture(cv)
      ...
      case 'palms': {
        ...
        for (let i = 0; i < p.n; i++) {
          ...
          const g = new THREE.Group()
          g.userData.model = 'palm'; g.userData.h = h
          const trunk = new THREE.Mesh(new THREE.CylinderGeometry(0.6, 1.0, h, 6), lam(0x6b5340))
          trunk.position.y = h / 2; trunk.rotation.z = (Math.random() - 0.5) * 0.12; g.add(trunk)
          for (let k = 0; k < 7; k++) {
            ...
            frond.translateY(-h * 0.14); g.add(frond)
          }
          g.position.set(c.x + dir.x * t, 0, c.z + dir.z * t); g.rotation.y = Math.random() * Math.PI * 2
          scene.add(g)
        }
      ...
      // ── DOME: a big low arena roof beyond the park (Ford Field over
      //    Comerica's left field) — a sliced sphere on a walled block
      case 'dome': {
        const v = beyond(p.a, p.off + p.d / 2, 0)
        const mat = lam(p.color || 0xd8d6cf)
        const base = new THREE.Mesh(new THREE.BoxGeometry(p.w, p.h * 0.55, p.d), mat)
        base.position.set(v.x, p.h * 0.275, v.z); faceHome(base); scene.add(base)
        // the roof: an ellipsoid whose lower half is buried in the block
        const cap = new THREE.Mesh(new THREE.SphereGeometry(1, 28, 14), mat)
        cap.scale.set(p.w * 0.5, p.h * 0.45, p.d * 0.5)
        cap.position.set(v.x, p.h * 0.55, v.z); cap.rotation.y = base.rotation.y; scene.add(cap)
        if (p.text) { const sp = textSprite(p.text, 0x2b4a8a, p.w * 0.3, 12, false); sp.position.set(v.x, p.h * 0.6, v.z); scene.add(sp) }
        break
      }
      // ── MAST: a lattice light mast standing beside a board or on a deck
      case 'mast': {
        const v = beyond(p.a, p.off, p.y || 0)
        const dark = new THREE.MeshBasicMaterial({ color: 0x0a0c10 })
        for (const s of [[-1.4, -1.4], [1.4, -1.4], [-1.4, 1.4], [1.4, 1.4]]) {
          const leg = new THREE.Mesh(new THREE.BoxGeometry(0.7, p.h, 0.7), dark)
          leg.position.set(v.x + s[0], v.y + p.h / 2, v.z + s[1]); scene.add(leg)
        }
        for (let y = 8; y < p.h; y += 8) {
          const ring = new THREE.Mesh(new THREE.BoxGeometry(3.6, 0.4, 3.6), dark)
          ring.position.set(v.x, v.y + y, v.z); scene.add(ring)
        }
        const rig = new THREE.Mesh(new THREE.PlaneGeometry(p.w || 26, 9),
          new THREE.MeshBasicMaterial({ map: lampTex(12, 4), transparent: true, opacity: 0.9, fog: false, side: THREE.DoubleSide }))
        rig.position.set(v.x, v.y + p.h + 4, v.z); rig.lookAt(0, 30, 0); rig.userData.noShadow = true; scene.add(rig)
        const pl = new THREE.PointLight(0xffd9ae, 0.18, 600); pl.position.set(v.x, v.y + p.h, v.z); scene.add(pl)
        break
      }
      // ── NUMBERS: a row of retired numbers on a wall face (Comerica's
      //    wall behind the left-centre seats)
      case 'numbers': {
        const p0 = beyond(p.from, p.off), p1 = beyond(p.to, p.off)
        const w = p0.distanceTo(p1)
        const cv = document.createElement('canvas'); cv.width = 1024; cv.height = 96
        const g = cv.getContext('2d')
        g.fillStyle = css(p.color || 0x14233a); g.fillRect(0, 0, 1024, 96)
        g.font = '900 56px SF Mono, Menlo, monospace'; g.textAlign = 'center'; g.textBaseline = 'middle'
        const items = p.items || []
        items.forEach((t, i) => { g.fillStyle = 'rgba(244,244,245,.95)'; g.fillText(String(t), 1024 * (i + 0.5) / items.length, 50) })
        const m = new THREE.Mesh(new THREE.PlaneGeometry(w, p.h), new THREE.MeshBasicMaterial({ map: new THREE.CanvasTexture(cv), side: THREE.DoubleSide }))
        const mid = p0.clone().lerp(p1, 0.5); mid.y = p.y + p.h / 2
        m.position.copy(mid); faceHome(m); scene.add(m)
        break
      }

      // ── GLASS: a tall wall of pale panels behind the stands (Chase's
      //    window wall over left-centre)
      case 'glass': {
        const pos = [], uv = []
        for (let a = p.from; a < p.to; a += 2) {
          const b = Math.min(p.to, a + 2)
          const p0 = beyond(a, p.off), p1 = beyond(b, p.off)
          pos.push(p0.x, p.y0, p0.z, p1.x, p.y0, p1.z, p0.x, p.y1, p0.z, p1.x, p.y0, p1.z, p1.x, p.y1, p1.z, p0.x, p.y1, p0.z)
          const w = p0.distanceTo(p1) / 14
          uv.push(0, 0, w, 0, 0, 1, w, 0, w, 1, 0, 1)
        }
        const cv = document.createElement('canvas'); cv.width = 64; cv.height = 64
        const g = cv.getContext('2d')
        g.fillStyle = css(p.color || 0x9fb3c8); g.fillRect(0, 0, 64, 64)
        g.fillStyle = 'rgba(0,0,0,.35)'; g.fillRect(0, 0, 3, 64); g.fillRect(0, 0, 64, 3); g.fillRect(0, 31, 64, 2)
        const tex = new THREE.CanvasTexture(cv); tex.wrapS = tex.wrapT = THREE.RepeatWrapping
        const geo = new THREE.BufferGeometry()
        geo.setAttribute('position', new THREE.Float32BufferAttribute(pos, 3))
        geo.setAttribute('uv', new THREE.Float32BufferAttribute(uv, 2))
        geo.computeVertexNormals()
        const m = new THREE.Mesh(geo, new THREE.MeshLambertMaterial({ map: tex, emissive: p.color || 0x9fb3c8, emissiveIntensity: 0.18, side: THREE.DoubleSide, transparent: true, opacity: 0.85 }))
        scene.add(m)
        break
      }
