...
//   upper   optional per-sector second-deck overrides [[from, to, off, depth, y0, y1]].
//   material 'brick' draws the fascias in brick (else grained concrete).
//   sky     optional { el, az, turbidity, rayleigh, mie } for the park's dusk.
...
export const PARK_BOWLS = {
  ...
  'Petco Park':                     B({
    ...
    // the left-field terrace: a shallow second level, not a full deck
    upper: [[-72, -20, 116, 52, 50, 78]],
    // that August dusk: violet overhead, ember at the skyline
    sky: { el: -4.2, az: 100, turbidity: 3, rayleigh: 2.4, mie: 0.006 },
  ...
  'Comerica Park':                  B({
    up: [[-72, -52], [50, 72]], open: [[-7, 7]], seat: 0x1f4d33, fascia: 0x6e3a2c, material: 'brick',
    // the first-base upper deck is a storey lower (one suite level, not two)
    upper: [[50, 180, 116, 96, 47, 97]],
    sky: { el: -3.6, az: 120, turbidity: 4, rayleigh: 1.4 },
  ...
  'Chase Field':                    B({
    ...
    sky: { el: -2.4, az: 110, turbidity: 9, rayleigh: 1.0, mie: 0.008 },
