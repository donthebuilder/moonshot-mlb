...
export function matchesPitcherWeakness(p, weakness) {
  ...
  // Same dead read as lib/verdict.js carried until 2026-09-19: neither
  // air_pull_rate nor recent_air_pull_rate exists on a slate row (0 of 270 on
  // the live slate), so this was 0 for everyone and the 'pull-air prone'
  // branch below fell through to its barrel clause every time. The published
  // name is recent_pull_air_rate -- pulled AND in the air, which is what a
  // rule called "pull-air prone" is asking for.
  const air_pull = n(p.recent_pull_air_rate, 0) || n(p?.bbe_profile?.pull_air_rate, 0)
...
//
// ── RE-TESTED 2026-08-11. DO NOT TURN THIS BACK ON. ──────────────────────
//
// Donovan asked whether ISO would beat the raw ranking if applied as a
// TIEBREAKER rather than as the full multiplier. It does not. Measured on the
// graded archive available from the data branch — 15 nights, 1,019
// player-nights, 151 homers (14.8%) — ranking each night and scoring the
// top-5 slots:
//
//   raw hr_score (what ships)      28.0% top-5    AUC 0.644   <- baseline
//   ISO lens, full x0.56-1.52      25.3%          AUC 0.637
//   ISO half-strength (sqrt)       26.7%          AUC 0.643
//   ISO as a tiebreaker only       29.3%          AUC 0.644
//   season ISO alone               16.0%          AUC 0.603
//
// Every form of applying it is at best indistinguishable and mostly worse.
// The tiebreaker's apparent +1.3pp is ONE homer across all 75 top-5 slots in
// the sample; a 4,000-resample bootstrap over nights puts the 95% CI at
// [+0.0, +4.0]pp with zero on the boundary, and it wins in 64% of resamples —
// a coin flip. ISO alone is far worse than the score, which is the cleanest
// sign that hr_score has already absorbed what ISO knows (season_power, 0.12).
//
// So the 2026-08-09 removal was right, and this is now measured rather than
// argued. isoAdjustedHr stays exported for ONE consumer: The Read's ISO lens,
// which shows it as a visible second opinion and does not rank on it.
//
// CAVEAT worth keeping honest: 15 nights, not the full 133. The archive lives
// at ~/Desktop/results and the data branch only retains recent runs. Re-run
// this against the full set before treating it as final — but note the
// direction was consistent across every variant tested, which is not what a
// sample-size problem usually looks like.
//
...
export function isoAdjustedHr(p, rawScore) {
  // SOFT KNEE, not a hard clamp (fixed 2026-08-05). The original Math.min(99)
  // looked harmless in theory and then the live board showed SIX hitters tied
  // at 99.0 — every big raw × big ISO product flattened into one indistinct
  // blob at the exact top of the page, which is where ordering matters most.
  // Above 92 the curve now compresses asymptotically toward 100 instead:
  // still on the 0–100 scale every grade band assumes, but 105 vs 130 raw
  // products stay distinguishable instead of both reading 99.0.
  const v = (Number(rawScore) || 0) * isoMultiplier(p)
  if (v <= 92) return v
  return 92 + 7.9 * (1 - Math.exp(-(v - 92) / 18))
}
...
// ── RUN SCORE & RBI SCORE ────────────────────────────────────────────────────
//
// Donovan, 2026-08-21: "what do you think of player run and rbi scoring same
// with hit scoring... maybe matchups make it different." hit_score already
// exists as a bot field with its own column everywhere; runs and RBI never
// got the same treatment even though the payload carries real production
// counts for both (season_runs, season_rbi, last5/7_runs, last5/7_rbi) and a
// real matchup signal for both — a hitter's own avg/ISO split against LHP or
// RHP, which can be resolved to a genuine tonight's-matchup number rather
// than a generic pair nobody picks between.
//
// TWO scores, not one blended "production" number. A run and an RBI are
// earned by different things — a run wants ON-BASE ability and a spot that
// bats AHEAD of the order; an RBI wants drive-in ability and a spot that
// bats BEHIND runners — and folding them into one column would hide exactly
// the difference a lineup-spot read needs to see.
//
// SAME DISCLOSURE AS kRiskScore, for the same reason: this is a transparent
// blend of published rates, not a bot field. The graded archive DOES carry
// actual_runs and actual_rbi (lib/pairEvidence.js already joins box scores by
// them), so this is backtestable in a way kRiskScore's strikeout number isn't
// — it just hasn't been walk-forward tested against that archive yet. Until
// it has, both scores carry the same "not calibrated" language, and both
// stay REMOVABLE the moment that backtest exists and says the weights below
// are wrong.
//
// MATCHUP TERM: the hitter's own avg against LHP or RHP, picked by tonight's
// actual starter's throwing hand (pitcher_throws) — not an average of both
// splits, which would erase the exact thing "matchups make it different" is
// asking for. Falls back to season_avg when pitcher_throws or the matching
// split is missing (rather than dragging the whole score to zero for a
// payload gap).
export function matchupAvg(p) {
  const throws = String(p?.pitcher_throws || '').toUpperCase().slice(0, 1)
  const split = throws === 'L' ? p?.avg_vs_lhp : throws === 'R' ? p?.avg_vs_rhp : null
  const v = Number(split)
  if (Number.isFinite(v) && v > 0) return v
  const seasonV = Number(p?.season_avg)
  return Number.isFinite(seasonV) ? seasonV : null
}

// RBI SCORE — drive-in opportunity × drive-in ability.
//   season_rbi_per_pa   35%   the base rate: RBI actually produced per trip
//   lineup spot         25%   curve peaking at the 4-hole (the most runners
//                              on ahead of it across a season), tapering to
//                              both ends — the raw slot is the only per-game
//                              opportunity signal the payload publishes
//   matchup avg         25%   average against tonight's actual starter hand
//   recent RBI form      15%   last7_rbi, so a hot week still moves it
export function rbiScore(p) {
  if (!p) return 0
  const rate = norm(p.season_rbi_per_pa, 0.04, 0.17)
  const s = Number(p?.lineup_spot)
  const spotCurve = Number.isFinite(s) && s >= 1 && s <= 9 ? clamp01(1 - Math.abs(s - 4) / 6) : 0.5
  const matchup = matchupAvg(p)
  const matchupTerm = matchup != null ? norm(matchup, 0.15, 0.32) : 0.5
  const recent = norm(n(p.last7_rbi, 0), 0, 6)
  return 100 * (rate * 0.35 + spotCurve * 0.25 + matchupTerm * 0.25 + recent * 0.15)
}

// RUN SCORE — on-base ability × opportunity to be driven in.
//   season_runs_per_pa  35%   the base rate
//   lineup spot         25%   curve peaking at the 1–2 hole (mirrors RBI's,
//                              flipped — the most at-bats stacked behind it)
//   season_obp          25%   reaching base is the precondition for a run;
//                              no stolen-base or speed field is published, so
//                              OBP is the only leg of "run" this data can see
//   recent run form      15%   last7_runs
export function runScore(p) {
  if (!p) return 0
  const rate = norm(p.season_runs_per_pa, 0.05, 0.16)
  const s = Number(p?.lineup_spot)
  const spotCurve = Number.isFinite(s) && s >= 1 && s <= 9 ? clamp01(1 - Math.abs(s - 1.5) / 6.5) : 0.5
  const obp = norm(p.season_obp, 0.26, 0.40)
  const recent = norm(n(p.last7_runs, 0), 0, 6)
  return 100 * (rate * 0.35 + spotCurve * 0.25 + obp * 0.25 + recent * 0.15)
}
