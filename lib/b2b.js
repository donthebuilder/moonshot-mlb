...
// ── THE WATCH WAS SLOW, AND THIS IS WHY (2026-09-03) ────────────────────────
//
// Donovan: "the back-to-back watch is kinda slow to show the players that went.
// I like that it shows it, it just isn't showing it fast enough."
//
// Round 6 (08-29) made the league call UNCONDITIONAL, and it was right to --
// the graded file only covers the bot's own ~50 designated picks, so it always
// undercounted who actually went deep. But that turned one cheap request into
// a schedule call plus ONE BOXSCORE PER GAME, and the hook awaited every one
// of them before it set state once. On a fifteen-game slate that is sixteen
// round-trips to statsapi with nothing on screen until the last one lands.
// Nothing was broken; it was just silent for as long as the slowest boxscore.
//
// Two fixes, neither of which changes what the panel proves:
//
//   1. PAINT IN TWO STAGES. The graded file is one request and usually the
//      fastest thing here, so its names go up the moment they arrive and the
//      league's union fills in behind them. "No proof, no render" is intact --
//      every name in stage one is as proven as every name in stage two.
//   2. CACHE THE DAY. A setup date's homers are a finished fact once its games
//      are final, and this hook remounts on every tab switch. The cache is
//      module-level and keyed on the date, so coming back to the board is
//      free instead of sixteen requests again.
//
// A day still in progress is deliberately NOT cached permanently -- see
// LEAGUE_TTL below.
const _leagueCache = new Map()   // setupDate -> { at, set, final }
// A finished day never changes. A day still running gets re-asked after this.
const LEAGUE_TTL = 5 * 60 * 1000

async function homersFromLeague(date) {
  const hit = _leagueCache.get(date)
  if (hit && (hit.final || Date.now() - hit.at < LEAGUE_TTL)) return hit.set
  try {
    ...
    // Whether every game that day has finished decides if this answer can be
    // cached forever or only for the TTL.
    const allFinal = (sched?.dates?.[0]?.games || [])
      .filter((g) => ['Live', 'Final'].includes(g?.status?.abstractGameState))
      .every((g) => g?.status?.abstractGameState === 'Final')
    const out = new Set()
    ...
    const result = out.size ? out : null
    _leagueCache.set(date, { at: Date.now(), set: result, final: allFinal })
    return result
  } catch {
    return null
  }
...
export function useSetupHomers(dateKey) {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      // STAGE ONE. Whatever the graded file proved goes up now rather than
      // waiting behind fifteen boxscores. `alive` is checked because a date
      // change can retire this run between the two awaits.
      if (alive && s.size) setSetupHr(new Set(s))
      ...
      // full slate.
      // STAGE TWO. The league's own boxscores -- every homer that day, not
      // just the bot's picks -- unioned in. Unioning only ever ADDS a proven
      // name, so the list can grow under the reader but never shrink.
