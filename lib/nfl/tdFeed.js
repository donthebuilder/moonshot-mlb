...
const PATTERNS = [
  ...
  // Seen 2026-09-20: "Elijah Ponder Fumble Recovery in End Zone" -- no yardage.
  { kind: 'fumble return', re: /^(.+?) Fumble Recovery in End Zone(?:\s*\((.+?)\))?$/, noYards: true },
]

export function parsePlayText(text) {
  // 2026-09-24 audit: ESPN writes rush TDs with a TRAILING SPACE ("Christian
  // McCaffrey 1 Yd Rush "), and the `$` anchor refused every one of them --
  // 14 touchdowns in weeks 1-2 (McCaffrey, Henry, Taylor among them) were
  // stored with no scorer, no kind, no board rank, and counted on /called
  // as "not on the board". Trim first.
  const t = String(text || '').trim()
  for (const { kind, re, noYards } of PATTERNS) {
    const m = t.match(re)
    if (!m) continue
    if (noYards) return { kind, scorer: m[1], yards: null, passer: null, note: m[2] || null }
...
export function buildTdEvent(play, { game, roster, directory, logs, picksCard, matchup, season, day } = {}) {
  const parsed = parsePlayText(play.text)
  const scorerName = parsed?.scorer || null
  // 2026-09-24: `roster` is the SLATE (nfl_week.json -- the only thing with
  // TD scores, which boardRankFor below ranks). `directory` is the full
  // 53-man + IR file (nfl_roster.json, published from 09-24) and is only
  // for the name -> gsis_id join: a scorer the slate never rated (a
  // fullback, a call-up) still gets his id, season line and card, and then
  // honestly no board rank. Slate first so a rated man keeps his slate row.
  const rosterRow = scorerName ? (matchRoster(roster, scorerName, play.team) || matchRoster(directory, scorerName, play.team)) : null
...
export function tdCaptureFrom(rows) {
  ...
  // The three states come from lib/callStatus.js (tdCallStatus), the same
  // rule /called and the event record read.
  const called = list.filter((r) => tdCallStatus(r) === 'called').length
  const rated = list.filter((r) => tdCallStatus(r) === 'board').length
