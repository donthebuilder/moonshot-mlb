...
export function tdPostText(ev, { site = '', handle = '' } = {}) {
  ...
  const season = ev.seasonToDate ? `${ev.seasonToDate.td} TD · ${ev.seasonToDate.games} G` : null
  ...
  if (ev.onBot) {
    ...
    // Rated / resolvable but not a designated call -- mirrors MLB's rated
    // "#N on the board / N HR · ISO / <closing>" state, using the season TD
    // line and the defense-vs-role ranking as the data block.
    if (season) lines.push('', season)
    lines.push('', `${ev.defense.role} vs ${ev.defense.opp}: ${ev.defense.tag} (#${ev.defense.rank} of 32 in TD allowed)`)
