...
// A real finisher is a reason to watch him even off a quiet raw log --
// nfl_offense_value.py's red_zone_conversion() (2026-09-13) grades TD-per-
// red-zone-touch within position, so this is the same 'is he actually being
// asked the question, and does he answer it' philosophy this file already
// runs on usage, applied to the scoring end of a drive instead of the
// volume end. TD-market only -- a red-zone finishing rate has nothing to
// say about REC_YDS or RUSH_YDS on its own.
const FINISHER_PCTL = 80

/** Why a row is on the board. Empty array = plain volume.  is
 * optional -- callers that don't have it (or don't pass it) simply never
 * see the 'finisher' reason, same graceful-absence rule as everywhere else
 * in this file. */
export function reasonsFor(logs, player, market, matchup) {
  ...
  if (market === 'TD') {
    ...
    if (rz && rz.percentile >= FINISHER_PCTL) out.push('finisher')
  }
