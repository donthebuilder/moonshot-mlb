...
/**
 * One slate's verdict on the head-to-head: +1 you cleared more contested
 * rungs than the bot, -1 it cleared more, 0 neither.
 *
 * A slate you didn't contest at all is 0 — not a push you survived. It breaks
 * a streak rather than extending it, mirroring the MLB ledger's nightVerdict
 * (lib/myPicks.js): one definition, used by the strip and the streak
 * sentence both.
 */
export function slateVerdict(row) {
...
/**
 * Current and longest runs of slates, from the ledger.
 *
 * DESCRIPTIVE ONLY. This says what happened, in order. It is not evidence
 * that the next slate goes the same way, and the tab must never word it as
 * if it were — the sample here is slates, and there are a few dozen a
 * season at best.
 */
...
//
// The ledger prints rates from the first graded rung: 5 of 8 comes out as
// 62.5%, in the same type and the same green as a number built on four hundred
// rungs. Eight coin flips land 5-3 about a fifth of the time, so that 62.5% is
// consistent with a true rate anywhere from roughly 31% to 86% — and the page
// said none of that.
//
// Wilson rather than the textbook normal interval, because the normal one is
// visibly wrong at exactly the sample sizes this ledger lives at: at 5 of 8 it
// runs past 96%, and at 8 of 8 it collapses to zero width and claims certainty.
export function wilson95(wins, n) {
  ...
  return { lo: Math.max(0, ((c - r) / d) * 100), hi: Math.min(100, ((c + r) / d) * 100) }
}

// Two rates are only worth comparing when their intervals come apart. Until
// then the honest word for the gap between them is "noise", not "edge".
export function separated(aWins, bWins, n) {
  const a = wilson95(aWins, n)
  const b = wilson95(bWins, n)
  if (!a || !b) return false
