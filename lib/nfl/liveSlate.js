...
function situationOf(comp, sides) {
  ...
  // An ALLOWLIST of the three ways a boolean survives a JSON round trip, not a
  // truthiness test: `Boolean(sit.isRedZone)` would call the string "false"
  // true, which is the one failure mode worse than not firing. `inRedZone` is
  // accepted alongside `isRedZone` because the two spellings both appear in
  // ESPN payloads and neither costs anything to allow.
  const flag = sit.isRedZone !== undefined ? sit.isRedZone : sit.inRedZone
  const redZone = flag === true || flag === 'true' || flag === 1
  return {
    redZone,
    possession,
    downDistance: String(sit.downDistanceText || sit.shortDownDistanceText || ''),
  }
}
