...
// TD added to passing 2026-09-10, for Franchise's own touchdown alert --
// nothing here read a QB's passing score before, because ATD (anytime
// touchdown scorer) never counts one and TUDDY's own tdsIn() below still
// deliberately does not. Same label-not-index rule as everything else in
// this parser: if ESPN's passing category ever drops the TD column this
// just stays 0, the same failure direction as every other stat here.
const WANT = {
  passing: { YDS: 'passing_yards', TD: 'passing_tds' },
...
const EMPTY_LINE = {
  passing_yards: 0, passing_tds: 0, carries: 0, rushing_yards: 0, rushing_tds: 0,
...
let _inflight = null
let _lastError = null
/** The last scoreboard/summary failure, or null. For diagnostics only. */
export const lastNflLiveError = () => _lastError

// 2026-09-13: THE HOST, not the header, was the whole bug. Probed live from
// Vercel (iad1): site.api.espn.com 403s from Vercel's egress -- and a browser
// User-Agent makes it WORSE, turning a working bare request into a 403. Its
// sibling site.web.api.espn.com serves the identical payload and answers 200
// from Vercel with or without a UA. So: switch host, send NO special headers.
// Until today a refused scoreboard was indistinguishable from "no games" (see
// pull()), which is why no TD post, no NFL push and no Franchise NFL alert had
// EVER fired from production.
...
async function pull() {
  ...
  try {
    ...
    // silently cancels every alert. 2026-09-13: but SAY so -- the error is
    // recorded so a caller (the NFL tick's response, the push tick's log)
    // can tell "couldn't ask" from "nothing happening".
    _lastError = String(err?.message || err)
    console.error(`[nflLive] scoreboard fetch failed: ${_lastError}`)
    return _snap ? { ..._snap, error: _lastError } : { at: Date.now(), games: [], lines: new Map(), plays: [], liveCount: 0, error: _lastError }
  }
  ...
  // One summary per game in progress, all at once. 2026-09-13: the scoring
  // plays and the box score are now parsed in SEPARATE try blocks. They used
  // to share one, so a parseBox() throw on an unexpected payload shape ate
  // that game's TOUCHDOWNS too -- exactly the "plays:0 while live" symptom
  // that kept every TD post from firing even after the host was fixed. A box
  // failure must never suppress a scoring play. The first summary failure of
  // either kind is recorded so the tick can report WHY a live game is dark.
  let summaryError = null
  await Promise.all(live.map(async (g) => {
    let data
    try {
      data = await getJSON(`${SUMMARY}?event=${encodeURIComponent(g.game_id)}`)
    } catch (err) {
      summaryError = summaryError || `summary ${g.game_id}: ${String(err?.message || err)}`
      return
    }
    try {
      for (const play of parseScoringPlays(data)) plays.push({ ...play, game_id: g.game_id })
    } catch (err) {
      summaryError = summaryError || `scoringPlays ${g.game_id}: ${String(err?.message || err)}`
    }
    try {
      for (const [key, row] of parseBox(data)) lines.set(key, { ...row, game_id: g.game_id })
    } catch (err) {
      summaryError = summaryError || `box ${g.game_id}: ${String(err?.message || err)}`
    }
  }))
  _lastError = summaryError

  _snap = { at: Date.now(), games, lines, plays, liveCount: live.length, error: summaryError }
  _at = Date.now()
  return _snap
}
