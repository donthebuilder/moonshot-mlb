// Where the NFL payload comes from.
//
// Same rule as the MLB side, and it is not negotiable: moonshot-mlb is
// READ-ONLY. It never writes, never runs a bot, and must never gain a
// .github/workflows directory. The NFL bot lives with the MLB one in
// MLB-HR-DASHBOARD-STREAMLIT and publishes to that repo's `data` branch.
//
// Two candidates, tried in order:
//
//   1. the data branch — what the bot publishes on a schedule, the live path
//   2. /data/nfl in this repo's public/ — a committed snapshot
//
// (2) exists so the NFL side is never a blank page: the first preseason build
// is committed here, so the tab works the moment it deploys, before the bot
// has a scheduled run. Once the workflow is live, (1) wins every time and the
// snapshot just sits there as a floor. The MLB side has no equivalent because
// its payloads run to hundreds of megabytes; a week of NFL is 69 KB.

const REPO = 'donthebuilder/MLB-HR-DASHBOARD-STREAMLIT'
const BRANCH = 'data'

// Note the `current/` + `nfl_` shape. publish_data.sh copies files out of
// public/data/current/ BY NAME into a single orphan commit on the data branch;
// it handles concurrent publishers and force-push races correctly and isn't
// worth reimplementing for a second sport. So the NFL bot writes into that
// same directory with an nfl_ prefix and its three filenames were added to
// that script's PUBLISH_FILES list.
export const NFL_DATA_BASE =
  process.env.NEXT_PUBLIC_NFL_DATA_BASE ||
  `https://raw.githubusercontent.com/${REPO}/${BRANCH}/public/data/current`

// THIS WEEK / NEXT WEEK (2026-09-18). TUDDY's header carries the same switch
// MOONSHOT's does, and MOONSHOT's Tmrw is a full board rather than a schedule
// stub -- so the bot builds next week on every run under its own nfl_next_
// prefix (.github/workflows/nfl.yml, "Build next week"). Four payloads move
// with the switch; everything historical (report card, results, the graded
// record) stays single-copy, because a future week has no history to show.
//
// NO COMMITTED FALLBACK on the next-week paths, deliberately. The repo's own
// public/data/nfl/*.json mirror is a snapshot of THIS week; serving it under
// the Next week switch would quietly show the wrong week rather than an
// honest empty state. Same reasoning as nflFantasyStatsPaths below.
const nextWeek = (m) => String(m) === 'next'

export const nflSlatePaths = (weekMode) => (nextWeek(weekMode)
  ? [`${NFL_DATA_BASE}/nfl_next_week.json`]
  : [`${NFL_DATA_BASE}/nfl_week.json`, '/data/nfl/week.json'])

export const nflReportPaths = () => [
  `${NFL_DATA_BASE}/nfl_report_card.json`,
  '/data/nfl/report_card.json',
]

// FRANCHISE LIVE SCORING (2026-09-14). Per-player box-score lines for the
// current week, published every 15 min in game windows by the bot's
// nfl_fantasy_stats.py. No committed fallback on purpose: a stale local copy
// would score last week's box against this week's lineups. Absent means "no
// stats yet", which the scorer already renders honestly.
export const nflFantasyStatsPaths = () => [
  `${NFL_DATA_BASE}/nfl_fantasy_stats.json`,
]

export function nflFantasyStatsLooksReal(j) {
  if (!j || typeof j !== 'object') return false
  return Number(j.season) > 0 && Number(j.week) > 0 && Array.isArray(j.games) && j.players && typeof j.players === 'object'
}

// Defence-vs-position, coverage shells, explosive allowed, team usage.
export const nflMatchupPaths = (weekMode) => (nextWeek(weekMode)
  ? [`${NFL_DATA_BASE}/nfl_next_matchup.json`]
  : [`${NFL_DATA_BASE}/nfl_matchup.json`, '/data/nfl/matchup.json'])

// Per-game logs for the hit-rate chart. Separate because only the player modal
// reads them and they're the biggest single payload.
export const nflLogPaths = (weekMode) => (nextWeek(weekMode)
  ? [`${NFL_DATA_BASE}/nfl_next_logs.json`]
  : [`${NFL_DATA_BASE}/nfl_logs.json`, '/data/nfl/logs.json'])

// The pick card — seven market ladders, five deep. Small (~8 KB) and read by
// the Picks tab on load.
export const nflPicksPaths = (weekMode) => (nextWeek(weekMode)
  ? [`${NFL_DATA_BASE}/nfl_next_picks.json`]
  : [`${NFL_DATA_BASE}/nfl_picks.json`, '/data/nfl/picks.json'])

// Graded lines for the slate, keyed by player id for EVERY man who recorded
// one — not just the card — so an override on any player can be graded.
export const nflResultsPaths = () => [
  `${NFL_DATA_BASE}/nfl_results.json`,
  '/data/nfl/results.json',
]

// ROSTER (2026-09-13): name/team -> gsis_id, published by bots/nfl/nfl_roster.py
// specifically so the live touchdown alert (free-text off ESPN's scoring-play
// feed, no player id on it) can join a scorer to the season log / matchup /
// picks-card data that everything else here is keyed on. No committed
// snapshot fallback yet — unlike week/matchup/logs, "not published yet" is a
// harmless empty-roster state (lib/nfl/tdFeed.js's own matchRoster skips
// enrichment rather than guessing), not a blank page.
export const nflRosterPaths = () => [
  `${NFL_DATA_BASE}/nfl_roster.json`,
]

export const nflMetaPaths = () => [
  `${NFL_DATA_BASE}/nfl_meta.json`,
  '/data/nfl/meta.json',
]

// ODDS (2026-08-24). bots/nfl/nfl_odds_fetch.py's two regenerated-every-run
// outputs — the nfl_ analog of MLB's odds_latest.json/odds_status.json, same
// nfl_ prefix convention as every other file here. No committed snapshot
// exists yet at the /data/nfl/ fallback path (unlike the slate/picks/matchup
// files, "no odds" is already a well-handled, clearly-labelled state rather
// than a blank page — see nflOddsLooksReal below), so that candidate simply
// 404s harmlessly until one is ever added.
export const nflOddsPaths = () => [
  `${NFL_DATA_BASE}/nfl_odds_latest.json`,
  '/data/nfl/odds_latest.json',
]

export const nflOddsStatusPaths = () => [
  `${NFL_DATA_BASE}/nfl_odds_status.json`,
  '/data/nfl/odds_status.json',
]

// A payload is only real if it has games AND players. A 200 carrying an empty
// shell must not beat the committed snapshot sitting behind it in the list —
// same validity test the MLB slate does in lib/data.js, for the same reason.
export function nflSlateLooksReal(j) {
  if (!j || typeof j !== 'object') return false
  return Array.isArray(j.players) && j.players.length > 0
}

// A card is only a card if some market has rungs on it. An empty shell
// publishing over a good committed snapshot is the same failure the matchup
// validator below exists to stop.
export function nflPicksLooksReal(j) {
  if (!j || typeof j !== 'object' || !j.card || typeof j.card !== 'object') return false
  return Object.values(j.card).some((m) => Array.isArray(m?.rungs) && m.rungs.length > 0)
}

// The matchup payload grows a section at a time, and the bot and the site
// deploy independently — the bot lives in another repo on its own schedule.
// So the failure that matters here isn't a broken file, it's a VALID file
// from a bot that predates the section a tab now renders: a matchup.json
// with dvp but no field publishes fine, wins over the committed snapshot on
// freshness, and the Matchup Map renders empty on a green build.
//
// Requiring both means an older bot simply loses to the snapshot until it
// catches up, which is the right way round.
export function nflMatchupLooksReal(j) {
  if (!j || typeof j !== 'object') return false
  const has = (o) => o && typeof o === 'object' && Object.keys(o).length > 0
  return has(j.dvp) && has(j.field?.def_pass) && has(j.field?.league_pass)
}

// A priced board needs at least one quote in it. write_empty_board() in
// nfl_odds_fetch.py publishes a valid file even when nothing was fetched
// (empty:true, by_player_id:{}, by_name:{}) so the site can say WHY there's
// no pricing — that file is legitimate and should still be shown (it carries
// `reason`), it just isn't "real" pricing data for fetchNfl's freshness
// check, exactly the distinction nflMatchupLooksReal draws for an
// under-built matchup payload.
export function nflOddsLooksReal(j) {
  if (!j || typeof j !== 'object') return false
  const a = j.by_player_id && Object.keys(j.by_player_id).length
  const b = j.by_name && Object.keys(j.by_name).length
  return Boolean(a || b)
}

/**
 * Walk the candidates with a per-candidate TIMEOUT, and take the first that
 * both responds and validates.
 *
 * Why not just reuse fetchJSON from lib/data.js: it has no timeout. A primary
 * that hangs rather than fails — a proxy swallowing the connection, a CDN
 * black-holing the request — leaves the promise pending forever, and because
 * the tab gates its whole render on Promise.allSettled, the page sits on
 * "Loading slate…" indefinitely with a perfectly good committed snapshot
 * sitting one candidate down the list. Observed exactly that against
 * raw.githubusercontent behind a proxy.
 *
 * Six seconds is generous for a 69 KB file and short enough that a wedged
 * primary costs a beat rather than the page.
 */
// 2026-09-24 (PERF-1, football side): same rule as lib/data.js fetchJSON.
// The default is a conditional GET (cache:'no-cache' -> If-None-Match ->
// 304 / 0 bytes when nothing changed) instead of ?t= + no-store, which
// re-downloaded nfl_week (720 KB) + logs + matchup on every poll. `bust`
// keeps the old unique-URL fetch for a manual refresh.
export async function fetchNfl(paths, validate = null, timeoutMs = 6000, { bust = false } = {}) {
  let fallback = null
  for (const p of paths) {
    const ctl = new AbortController()
    const timer = setTimeout(() => ctl.abort(), timeoutMs)
    try {
      const url = bust ? `${p}${p.includes('?') ? '&' : '?'}t=${Date.now()}` : p
      const r = await fetch(url, { cache: bust ? 'no-store' : 'no-cache', signal: ctl.signal })
      if (!r.ok) continue
      const j = await r.json()
      if (!validate) return j
      if (validate(j)) return j
      if (fallback === null) fallback = j
    } catch {
      /* timed out, offline, or bad JSON — try the next one */
    } finally {
      clearTimeout(timer)
    }
  }
  return fallback
}
