...
// ── WHAT THE BOARD LOOKED LIKE WHEN YOU SAVED HIM (2026-09-21) ──────────────
//
// A pin recorded who and which game and nothing about WHEN or WHY, so the
// watchlist could never answer the one question a watchlist exists for: has
// this actually moved since I flagged it. "The signal was there" needs a
// before to point at.
//
// `pinned_at` and `pinned_score` are written once, at pin time, and then
// carried through every rebuild untouched (see the rebuild in useNflWatchlist
// -- this is the exact field-loss trap that note describes). They are a
// SNAPSHOT, never recomputed: the number is what the board said the moment you
// saved him, which is the only thing that makes a delta meaningful.
//
// NOTHING IS BACK-FILLED. Pins made before this shipped have no snapshot and
// say so rather than being handed a fabricated one (#16) -- they pick it up
// the next time they are re-pinned.
function bestScore(player) {
  const entries = Object.entries(player?.scores || {}).filter(([, v]) => Number.isFinite(v))
  if (!entries.length) return {}
  const [market, score] = entries.sort((a, b) => b[1] - a[1])[0]
  return { pinned_market: market, pinned_score: score }
}

function pinFromFreshSlate(data, player, keep = null) {
  ...
  return {
    ...
    // Carried, not recomputed. Absent on a pin made before this existed.
    pinned_at: keep?.pinned_at ?? null,
    pinned_market: keep?.pinned_market ?? null,
    pinned_score: keep?.pinned_score ?? null,
  }
}
...
export function useNflWatchlist(data) {
  ...
  const pins = useMemo(() => {
    ...
    return rows.map((row) => {
      ...
      // `row` is passed as `keep` so the snapshot survives the rebuild. Without
      // it every refresh would silently erase the thing the delta is measured
      // against -- the same class of loss this block was written to fix.
      return { ...pinFromFreshSlate(data, player, row), stale: false }
    })
  }, [store, data])
