...
// HIS NUMBER (2026-09-20, Donovan: "add jersey numbers and such"). Set in the
// display face, in his club's colour, hard against the name -- the way a team
// sheet or a scouting card writes it, not as another labelled stat box. Only
// ever drawn when the roster join actually returned one.
export function Jersey({ n, tint }) {
  if (!n) return null
  // CLEVELAND BROWN ON NEAR-BLACK IS NOT A COLOUR. Seven clubs' primaries are
  // dark enough to disappear on this canvas -- the first render of this put an
  // invisible #12 on a Browns card. Anything under the luminance floor falls
  // back to cream, which is what the rest of the identity line already wears.
  const h = String(tint || '').replace('#', '')
  const lum = h.length === 6
    ? [0, 2, 4].map((i) => parseInt(h.slice(i, i + 2), 16)).reduce((a, v, i) => a + v * [0.299, 0.587, 0.114][i], 0)
    : 0
  return (
    <span style={{ fontFamily: DISPLAY, fontSize: 34, fontWeight: 900, lineHeight: 1, color: lum > 90 ? tint : INK,
      transform: 'skewX(-8deg)', display: 'flex' }}>{`#${n}`}</span>
  )
}
...
export async function tdCard(ev, { site = 'dashnetwork.vercel.app' } = {}) {
  ...
  const tiles = [
    ...
    ev.seasonToDate ? { label: 'TD ENTERING TODAY', value: ev.seasonToDate.td } : null,
  ...
  const statement = ev.onBot
    ...
    : ev.tdBoard
      ...
      : `${nflNickname(ev.team)} FIND THE END ZONE.`
  ...
  const context = [
    ...
    ev.seasonToDate
      ? (ev.seasonToDate.games ? `${ev.seasonToDate.td} TD IN ${ev.seasonToDate.games} G` : `${ev.seasonToDate.td} TD ENTERING TODAY`)
      : '',
  ...
  return frame(
    <div style={{ display: 'flex', flexDirection: 'column', flex: 1, position: 'relative' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', padding: '26px 356px 0 40px' }}>
        <span style={{ fontSize: 13.5, fontWeight: 800, letterSpacing: 6, color: JADE }}>{`${nflNickname(ev.team)} TOUCHDOWN`}</span>
