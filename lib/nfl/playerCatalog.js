...
export function normalizeNflPlayer(raw, context = {}) {
  ...
  return {
    ...
    analytics: {
      ...
      // A rostered man with no NFL history to price -- a rookie, mostly. He is
      // in the payload so he can be searched for and drafted; the flag is so
      // nothing shows his empty projection as if it were a measured zero.
      no_data: Boolean(raw?.no_data),
    },
  }
}
