...
function shrinkToFit(head, lines, tail, { spaced = false, lead = '', closer = '' } = {}) {
  const t = tail || null
...
export function opportunityText(picks, data, stat = 'RZ', { site = '', handle = '' } = {}) {
  ...
  // 2026-09-20: the unit line used to be glued to the headline with a bare
  // \n, so the post opened with two lines jammed together. It is a LEAD now --
  // its own line under a blank -- which is the shape the MLB weekday-leaders
  // post already uses ("Last 5 Fridays:").
  const head = stat === 'GL'
  ...
  return shrinkToFit(head, lines, tail, { spaced: true, lead })
}
...
export function tdHistoryText(picks, data, { site = '', handle = '' } = {}) {
  ...
  // spaced (2026-09-20, Donovan: "make sure the tweets for the nfl are
  // formated with the spacings"). A blank line between every entry, the same
  // treatment lib/dash/tweetFeed.js gives its ranked lists. TUDDY's posts had
  // been running together since the day they shipped.
  return shrinkToFit(head, lines, tail, { spaced: true })
}
...
// ── PLAYER SPOTLIGHT (2026-09-20) ──────────────────────────────────────────
//
// Donovan: "player spot lights just like mlb list do one for this week."
//
// MOONSHOT's THE HOT STRETCH is one hitter, one window, his numbers stacked,
// and a closing line tying him to tonight's board. This is that post in
// football.
//
// THE SOURCE TOOK TWO TRIES AND THE FIRST ONE WAS WRONG IN A WAY WORTH
// KEEPING WRITTEN DOWN. Built first on nfl_logs.json, because that is where
// per-game lines live -- and it spotlighted Rhamondre Stevenson's WEEK 18,
// 2025. The logs file carries 2024 and 2025 and no 2026 at all (counted:
// 4,269 rows and 5,145 rows, nothing this season), so "the last week played"
// was last January. On a September Saturday that is not a spotlight, it is an
// archive post wearing this week's headline.
//
// nfl_fantasy_stats.json is THIS season: season 2026, week 2, and it reports
// how many of the week's games are `completed`. Today that is 1 of 16 -- the
// Thursday night game -- which is exactly the week-so-far a Saturday post
// should be about.
//
// THE HEADLINE SAYS WHICH IT IS. With only the Thursday game in, it says
// THURSDAY NIGHT, because calling one game "Week 2" implies the week is done.
// Once the Sunday slate has played it says WEEK N. Same discipline as the RED
// ZONE post naming the season its rates come from.
const SPOT_TD = 60          // a touchdown is worth about 60 yards of story
const SPOT_PASS_DIV = 2.5   // passing yards are cheaper than scrimmage yards
const SPOT_TNF_MAX = 3      // at or below this many completed games, it is TNF

const spotNum = (v) => (Number.isFinite(Number(v)) ? Number(v) : 0)

export function spotlightPick(stats, data) {
  const week = num(stats?.week)
  const players = stats?.players || {}
  const games = Array.isArray(stats?.games) ? stats.games : []
  const done = games.filter((g) => g?.completed).length
  if (!week || !done || !Object.keys(players).length) return null
  const byId = new Map(activePlayers(data).map((p) => [txt(p.player_id), p]))
  let best = null
  for (const [pid, st] of Object.entries(players)) {
    const p = byId.get(txt(pid))
    if (!p) continue
    const payd = spotNum(st.passing_yards)
    const patd = spotNum(st.passing_touchdowns)
    const ruyd = spotNum(st.rushing_yards)
    const rutd = spotNum(st.rushing_touchdowns)
    const rec = spotNum(st.receptions)
    const recyd = spotNum(st.receiving_yards)
    const rectd = spotNum(st.receiving_touchdowns)
    const td = patd + rutd + rectd
    const impact = td * SPOT_TD + ruyd + recyd + payd / SPOT_PASS_DIV
    if (!(impact > 0)) continue
    if (!best || impact > best.impact) {
      best = {
        impact, week, done,
        player_id: txt(pid), name: txt(p.name), team: txt(p.team) || null,
        position: txt(p.position) || null, opp: txt(p.opp) || null,
        jersey: Number(p?.jersey_number) || null,
        payd, patd, ruyd, rutd, rec, recyd, rectd,
        score: num(p?.scores?.TD),
        // HIS OWN PER-GAME SEASON RATES, straight off the slate row -- the
        // same `stats` block RED ZONE TARGETS reads for RZ/GL. This is what
        // makes the card's comparison panel possible and it is what MOONSHOT's
        // HOT STRETCH card does with a season slash line next to the window:
        // a big number means nothing until you can see what his normal is.
        // Nothing here is computed; every field is published per game already.
        avg: {
          payd: num(p?.stats?.PAYD), patd: num(p?.stats?.PATD),
          ruyd: num(p?.stats?.RUYD), car: num(p?.stats?.CAR),
          att: num(p?.stats?.ATT), td: num(p?.stats?.TD),
          xtd: num(p?.stats?.xTD), tdoe: num(p?.stats?.TDoE),
          rz: num(p?.stats?.RZ), gl: num(p?.stats?.GL),
        },
        // SEASON TD, ONLY WHEN IT CAN'T CONTRADICT THE WEEK (2026-09-20).
        // The slate's season_td is frozen at publish time, which for a
        // mid-week slate is BEFORE the games this card is about -- Josh
        // Allen's Week 2 card printed "5 TOUCHDOWNS IN ONE WEEK" over
        // "2 TD ON THE YEAR". Two published numbers, both real, that read as
        // a mistake side by side. Suppressed rather than adjusted: adding
        // this week to it would be me computing a season total the bot never
        // published (rule 16).
        seasonTd: (num(p?.season_td) != null && num(p?.season_td) >= td) ? num(p.season_td) : null,
      }
    }
  }
  return best
...
export function spotlightText(pick, data, { site = '', handle = '' } = {}) {
  if (!pick) return ''
  const tail = [site, handle].filter(Boolean).join(' · ')
  const window = pick.done <= SPOT_TNF_MAX ? 'THURSDAY NIGHT' : `WEEK ${pick.week}`
  const head = `⭐ ${String(pick.name).toUpperCase()}${pick.team ? ` (${pick.team})` : ''}, ${window}`
  // Only the lines he actually has -- a running back with no catches gets no
  // "0 REC" row, the same rule the MLB slash line follows.
  const rows = [
    pick.payd > 0 ? `${pick.payd} PASS YDS` : '',
    pick.patd > 0 ? `${pick.patd} PASS TD` : '',
    pick.ruyd !== 0 ? `${pick.ruyd} RUSH YDS` : '',
    pick.rutd > 0 ? `${pick.rutd} RUSH TD` : '',
    pick.rec > 0 ? `${pick.rec} REC` : '',
    pick.recyd > 0 ? `${pick.recyd} REC YDS` : '',
    pick.rectd > 0 ? `${pick.rectd} REC TD` : '',
  ].filter(Boolean)
  if (!rows.length) return ''
  // The closer carries the TOTAL, because the stacked rows deliberately split
  // a quarterback's touchdowns across passing and rushing and the sum is the
  // headline fact. "Week 2." on its own said nothing -- it was the fallback
  // branch firing because a QB carries no TD score on the board.
  const td = pick.patd + pick.rutd + pick.rectd
  const word = td === 1 ? 'touchdown' : 'touchdowns'
  // WHAT HIS NORMAL IS (2026-09-20, Donovan: "add more stats just like the mlb
  // spotlight one"). The MLB HOT STRETCH post works because the window sits
  // next to the season; a line of counting stats on its own does not tell a
  // reader whether it was a big night. One line, off the slate's own per-game
  // rates, and only when the model actually publishes them.
  const avg = pick.avg || {}
  const normal = [
    avg.td != null ? `${avg.td.toFixed(1)} TD a game` : '',
    avg.xtd != null ? `${avg.xtd.toFixed(1)} expected` : '',
  ].filter(Boolean).join(' · ')
  const context = normal ? `He averages ${normal}.` : ''

  const close = td > 0
    ? (pick.done <= SPOT_TNF_MAX
      ? `${td} total ${word} to open Week ${pick.week}.`
      : `${td} total ${word} in Week ${pick.week}.`)
    : `Week ${pick.week}.`
  const body = [head, '', ...rows, ...(context ? ['', context] : []), '', close, ...(tail ? ['', tail] : [])]
  return fitsLimit(body) ? body.filter((l) => l != null).join('\n') : ''
}
