...
// Black or cream on a club colour, by luminance -- MLB's inkOn(), which is not
// exported from homerCard.js and is four lines.
function inkOn(hex) {
...
// field position, red-zone geometry, play concepts." Yard lines and hash marks
      ...
      {tiles.length
        ? (
          <div style={{ display: 'flex', gap: 10, margin: '24px 356px 0 40px' }}>
            {tiles.map((t) => (
              <div key={t.label} style={{ display: 'flex', flexDirection: 'column', flex: 1, alignItems: 'center',
                border: `1px solid ${RULE}`, background: PANEL, padding: '14px 6px 12px 6px' }}>
                <span style={{ fontSize: 10, fontWeight: 800, letterSpacing: 2.2, color: '#b9b2a8' }}>{t.label}</span>
                <span style={{ fontFamily: DISPLAY, fontSize: 40, fontWeight: 900, lineHeight: 1.05, color: JADE,
                  transform: 'skewX(-8deg)', marginTop: 2 }}>{String(t.value)}</span>
              </div>
            ))}
          </div>
        )
