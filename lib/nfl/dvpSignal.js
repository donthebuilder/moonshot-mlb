...
export const softLine = (d) => {
  ...
  // td is a running total over the window, everything else is a per-game
  // rate, so the sentence has to say which or "6 touchdowns" reads as a
  // per-game number and the defence sounds ten times worse than it is.
  const unit = d.stat === 'td'
    ? `touchdowns${Number.isFinite(d.games) ? ` in ${d.games} games` : ''}`
    : `${d.label} a game`
