 ...
 * in a head-to-head.
 */
export function savePick(dateKey, gamePk, role, mine, bot) {
...
export function gradeSlate(slots, results) {
  ...
  const h2h = { n: 0, mineWon: 0, botWon: 0, w: 0, l: 0, t: 0 }
  rows.filter((r) => r.contested).forEach((r) => {
    h2h.n += 1
    if (r.mineOut) h2h.mineWon += 1
    if (r.botOut) h2h.botWon += 1
    if (r.mineOut && !r.botOut) h2h.w += 1
    else if (!r.mineOut && r.botOut) h2h.l += 1
    else h2h.t += 1
  })
