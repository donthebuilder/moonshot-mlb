...
/**
 * 🌬 THE ONE READ OF WIND, FOR EVERY SURFACE THAT DRAWS IT.
 *
 * 2026-08-30. SprayField.js worked this out privately in 2026-08-24 and its
 * comment — the long one about weather_wind_deg being a COMPASS bearing with
 * no published park orientation, so the label is the only park-relative fact —
 * is the whole argument and now lives beside the function instead of inside
 * one component. The 3D stadium needed the same numbers; two copies of a
 * calculation are two answers waiting to diverge, which is this repo's
 * standing rule (see lib/interval.js's header for the last time it applied).
 *
 * `to` is a FIELD-RELATIVE bearing in degrees: 0 = straight out to centre,
 * 180 = straight in, 90 = across. A crosswind is drawn on the axis without
 * claiming a side, because left-to-right versus right-to-left is not in the
 * payload. The corner variants get 28° / 152° so an "out to CF/corner" reads
 * as a pull-side push rather than dead centre.
 *
 * @returns { mph, label, way, to, out, in: isIn, cross, color, has }
 */
export function windRead(any) {
  const mph = wWind(any)
  const label = wLabel(any)
  const way = windWay(label)
  const isOut = way === 'out'
  const isIn = way === 'in'
  const cross = way === 'across'
  const to = cross ? 90
    : isOut ? (/corner/i.test(label) ? 28 : 0)
      : isIn ? (/corner/i.test(label) ? 152 : 180) : 0
  // The site-wide verdict pair, not a one-off ternary: out helps carry, in
  // hurts it, a crosswind is neither. verdictInk(null) resolves the neutral
  // case the same way every other null verdict on the site does.
  const color = verdictInk(isOut ? true : isIn ? false : null).color
  return { mph, label, way, to, out: isOut, in: isIn, cross, color, has: mph > 0 && !!label }
}
