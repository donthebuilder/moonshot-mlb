 ...
 * @returns {{ day, season, games, rows, byGame, lineups, modelVersion }}
 */
export async function buildNight(date, { onlyGameIds = null } = {}) {
    ...
    for (const side of ['away', 'home']) {
      ...
      for (const r of P.roster) {
        ...
        candidates.push({
          ...
          context: { oppGaPg: gaById.get(opp) ?? null, b2b: P.b2b, lineupKnown: Boolean(dressed), staleSeason: season.stale, curSeason, prevSeason },
        })
      }
    }
  }
  ...
  return { day, season, games, rows, byGame, lineups, modelVersion: MODEL_VERSION }
}
