...
/** A real calendar day, not just ten characters in the right shape:
 *  2026-13-99 passed DATE_RE, went upstream, and came back as a 404 dressed
 *  up as LIVE DATA DELAYED (caught by the route harness, 2026-09-25). */
...
export async function nhlGet(path, revalidate) {
  ...
  if (!res.ok) {
    ...
    const err = new Error(`nhl ${res.status} ${path}`)
    err.status = res.status   // the route turns a 404 into NOT FOUND, not DELAYED
    throw err
  }
  return res.json()
}
...
export const TTL2 = { roster: 3600, clubStats: 600, clubSchedule: 600, player: 600, gameLog: 600, leaders: 600 }
export const TEAM_RE = /^[A-Z]{3}$/
export const PLAYER_ID_RE = /^\d{7}$/

export const rosterFor = (team) => nhlGet(`/roster/${team}/current`, TTL2.roster)
export const clubStatsFor = (team, season, type = 2) => nhlGet(`/club-stats/${team}/${season}/${type}`, TTL2.clubStats)
export const clubScheduleFor = (team) => nhlGet(`/club-schedule-season/${team}/now`, TTL2.clubSchedule)
export const playerLanding = (id) => nhlGet(`/player/${id}/landing`, TTL2.player)
export const playerGameLog = (id, season, type = 2) => nhlGet(`/player/${id}/game-log/${season}/${type}`, TTL2.gameLog)
// Always the DATED form: the `/current` leaders carry no season field at
// all, and in preseason they are last season's numbers with nothing on them
// to say so. A dated ask for a season with no games yet answers 404, which
// the route turns into a labelled fallback (lib/nhl/season.js).
export const skaterLeaders = (season, type, cats, limit) =>
  nhlGet(`/skater-stats-leaders/${season}/${type}?categories=${cats.join(',')}&limit=${limit}`, TTL2.leaders)
export const goalieLeaders = (season, type, cats, limit) =>
  nhlGet(`/goalie-stats-leaders/${season}/${type}?categories=${cats.join(',')}&limit=${limit}`, TTL2.leaders)
