...
/** The board's order: score, then shots/GP, then name. One comparator for
 *  the in-game rank (scoreNight) and the night-wide one (rankNight), so the
 *  two pages can never sort the same men differently. */
export const byScore = (a, b) => (b.score - a.score) || ((b.legs?.shotsPg || 0) - (a.legs?.shotsPg || 0)) || String(a.name).localeCompare(String(b.name))
...
/**
 * THE BOARD, night-wide (2026-09-25, Donovan: "is there no boards like mlb
 * ranking all the players"). Every scored skater on the night, #1 to #N,
 * across games -- fair because `score` is already a percentile against the
 * whole night's scored population, not the game's. Input is readBoard()'s
 * `games`; unscored (NOT ON THE BOARD) men are not ranked, same as the
 * per-game table. Each row keeps its game's stamp: a PREVIEW is not a call,
 * and a LOCKED score was frozen at that game's lock.
 */
export function rankNight(games) {
  const rows = []
  for (const g of games || []) {
    const stamp = g.graded ? 'graded' : g.locked ? 'locked' : 'preview'
    for (const r of g.rows || []) if (r.status !== 'off' && r.score != null) rows.push({ ...r, gameId: g.game?.id, stamp, graded: Boolean(g.graded) })
  }
  rows.sort(byScore)
  return rows.map((r, i) => ({ ...r, nightRank: i + 1 }))
}
...
export function coverage(graded) {
  // Reads the shared record's `result` (lib/record/nhl.js) when a row has
  // one, else the log's own dressed/hit -- void is "not dressed" either way.
  const played = (r) => (r.result !== undefined ? r.result === 'hit' || r.result === 'miss' : Boolean(r.dressed))
  const scored = (r) => (r.result !== undefined ? r.result === 'hit' : Boolean(r.hit))
  const g = graded.filter(played)
  const scorers = g.filter(scored)
  const called = g.filter((r) => r.status === 'called')
  const band = (lo, hi) => { const b = g.filter((r) => r.rank != null && r.rank >= lo && r.rank <= hi); return { n: b.length, hits: b.filter(scored).length } }
  return {
    ...
    calledN: called.length, calledHits: called.filter(scored).length,
    baseRate: g.length ? scorers.length / g.length : null,
    bands: { top3: band(1, 3), r4to8: band(4, 8), r9to15: band(9, 15), r16plus: band(16, 999) },
  }
}

// The same numbers from COUNTS instead of rows -- one lamp_goal_nights row
// (supabase/migrations/202609260100_lamp_goal_nights.sql), or several added
// with addCounts. Each count is coverage()'s own filter, done in Postgres,
// so the record route reads ~one row a night instead of ~600.
export const COUNT_KEYS = ['games', 'dressed', 'scorers', 'scorers_called', 'scorers_board', 'scorers_off', 'called_n', 'called_hits', 'b1_n', 'b1_hits', 'b2_n', 'b2_hits', 'b3_n', 'b3_hits', 'b4_n', 'b4_hits']

export function addCounts(a, b) {
  const out = {}
  for (const k of COUNT_KEYS) out[k] = (Number(a?.[k]) || 0) + (Number(b?.[k]) || 0)
  return out
}

/** coverage()'s exact shape from counts. */
export function coverageFromCounts(c) {
  const n = (k) => Number(c?.[k]) || 0
  return {
    dressed: n('dressed'), scorers: n('scorers'),
    scorersCalled: n('scorers_called'),
    scorersOnBoard: n('scorers_board'),
    scorersOff: n('scorers_off'),
    calledN: n('called_n'), calledHits: n('called_hits'),
    baseRate: n('dressed') ? n('scorers') / n('dressed') : null,
    bands: {
      top3: { n: n('b1_n'), hits: n('b1_hits') }, r4to8: { n: n('b2_n'), hits: n('b2_hits') },
      r9to15: { n: n('b3_n'), hits: n('b3_hits') }, r16plus: { n: n('b4_n'), hits: n('b4_hits') },
    },
  }
}
