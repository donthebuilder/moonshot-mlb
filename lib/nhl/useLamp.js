...
export function useLampBoard(date) {
  const url = date ? `/api/lamp/board?date=${encodeURIComponent(date)}` : '/api/lamp/board'
  return useLampFetch(url, { pollMs: (b) => (b?.games?.some((g) => !g.locked && g.game.state === 'pre') ? 120000 : 0) })
}
export function useLampRecord(days = 30) { return useLampFetch(`/api/lamp/record?days=${days}`) }
