...
export function penLineParts(pen, opts = {}) {
  ...
  if (pen.hr9 != null) rates.push(`${pen.hr9.toFixed(2)} HR/9`)
  if (rates.length) {
    out.push({
      key: 'rates',
      text: `${rates.length > 1 ? `${rates.slice(0, -1).join(', ')} and ${rates[rates.length - 1]}` : rates[0]} on the season`,
      tone: pen.hr9 != null && pen.hr9 >= 1.25 ? 'hot' : pen.hr9 != null && pen.hr9 <= 0.95 ? 'cold' : 'plain',
      title: 'bullpen_era / bullpen_whip / bullpen_hr9 as published on the slate row. ERA and WHIP are new to this board — it used to show HR/9 alone, which cannot tell a pen that walks the yard from one that simply gives up the odd homer.',
    })
  }
