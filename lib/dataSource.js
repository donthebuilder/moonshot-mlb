...
// RUN META (2026-09-11, item 21). The bot has published current/{mode}_run_meta.json
// since 2026-08-21 (build_run_meta() in bots/mlb_dashboard.py) -- a small
// companion file next to the slate carrying generated_at/run_id/slate_date/
// trigger for the run that produced it. TUDDY has had an equivalent
// (nfl_meta.json -> built_at_human) since the NFL side shipped; MLB's own
// version existed server-side the whole time and was simply never fetched
// by this site. No new bot work needed, just wiring this up.
export const runMetaPaths = (mode) => [
  dataUrl(`current/${mode}_run_meta.json`),
]
...
// EVERY ACTIVE ROSTER (2026-09-24, Donovan: "I should be able to search every
// active player"). bots/roster_cache.py: 30 clubs, the 40-man with status
// (A / D10 / D60 / RM ...) and each hitter's season line, refreshed on every
// today run. QuickSearch reads it first; the live people-search stays as the
// fallback for a name it doesn't have (or before the first publish, when
// this 404s and fetchRosters() resolves to null).
export const rostersUrl = () => bust(dataUrl('current/rosters_mlb.json'))
export function fetchRosters() {
  return fetchShared(rostersUrl(), { ttl: 10 * 60 * 1000 }).catch(() => null)
}
