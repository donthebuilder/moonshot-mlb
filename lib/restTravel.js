...
export async function fetchRestTravel(slateDate) {
  ...
  byTeam.forEach((days, tid) => {
    ...
    if (vT != null && vY != null && vT !== vY && yest.some((g) => g.dayNight === 'night')) {
      ...
      // Spelled out 2026-08-09: the owner read the chip and didn't know what
      // "3-in-3" meant, which is a fair complaint about a label nobody
      // defined. The tooltip now says the whole thing in plain words.
      flags.push({ icon: '🗓', label: '3-in-3',
        title: 'Third game in three straight days — no off day in between. Legs and bullpens are into the back half of a grind. Context only; no score touched.' })
    }
