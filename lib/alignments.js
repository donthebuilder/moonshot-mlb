...
export function axesOf(p, people) {
  ...
  const out = {
    ...
    axes: {
      ...
      // ── THE NUMBER HE IS SITTING ON (2026-08-31) ──────────────────────
      //
      // Donovan: "the 1 can be differcn combos of season hr nummbers i like
      // that it highlight the specific number like 10 but numberolgy speaking
      // its a 1 or like 19th thats a 1."
      //
      // Exactly right, and only half of it was here. `next` reduces his NEXT
      // homer (season_hr + 1); the count he is standing on was never an axis
      // at all. They are different numbers and usually different roots — a man
      // on 18 carries a 9 and is chasing a 1 — so a club could never see both.
      // Both now, and every surface prints the RAW number beside the root, so
      // 10 and 19 are visibly two different ways of arriving at the same 1.
      hr: n(p?.season_hr, 0) > 0 ? digitRoot(n(p?.season_hr, 0)) : null,
...
// surface prints it — the whole point of the request that added `hr` is that
// "10" and "19" are two different ways of reaching the same 1, and a screen
// showing only the 1 has thrown away the half you were looking at.
export const AXIS_META = {
  next: { label: 'next HR', why: (a) => `his next homer is #${a.nextHr}`, raw: (a) => a.nextHr },
  hr: { label: 'season HR', why: (a) => `sitting on ${a.seasonHr}`, raw: (a) => a.seasonHr },
  ...
  spot: { label: 'bats', why: (a) => `bats ${a.axes.spot}`, raw: (a) => a.axes.spot },
  pos: { label: 'fields', why: (a) => `plays ${a.posAbbr} (position ${a.axes.pos})`, raw: (a) => a.axes.pos },
}

/**
 * Everyone whose numbers land on ONE root, ranked by how many of them do.
 *
 * ── WHAT THIS IS, STATED BEFORE IT IS USED (2026-08-31) ─────────────────────
 *
 * Donovan asked for "preditcution using the numeroly reductions". This is the
 * closest honest thing to that, and the honesty is the feature: it returns the
 * EXPECTED count alongside the actual one, every time, from the same call.
 *
 * Seven axes over ~250 hitters is roughly 1,500 numbers spread across nine
 * roots. About 165 memberships per root happen by arithmetic alone, and a
 * hitter carrying two on the same root happens to dozens of men every single
 * night. That is not a discovery, it is what division does — so `expected` is
 * returned beside `members` and the UI prints both. A list that showed only
 * the count would be inventing a signal out of long division.
 *
 * It answers "who is aligned with tonight's number", which is a real question
 * about arithmetic. It does not answer "who is going to homer", and nothing
...
export function alignedWith(root, rows = []) {
  ...
  members.sort((x, y) => (y.strength - x.strength) || (y.a.hrScore - x.a.hrScore))
  ...
  return {
    ...
    byBotScore: [...twoPlusMembers].sort((x, y) => (y.a.hrScore - x.a.hrScore) || (y.strength - x.strength)),
    medianScoreAligned: med(twoPlusMembers.map((m) => m.a.hrScore)),
    medianScoreSlate: med(rows.map((a) => a.hrScore)),
  }
}
