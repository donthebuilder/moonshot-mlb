...
// ── WHO LEADS THE READ, AND WHEN THE PAGE SAYS OTHERWISE ────────────────────
//
// 2026-08-16, Donovan: "HR leads + further clear line."
//
// The read used to lead on whichever of the four calls had the highest
// conviction z. That rotated the hero night to night, and it also broke the
// house rule it was built to serve. `convictionOf` measures a pick against ITS
// OWN category's designated pool, so a z of 1.7 in H+R+RBI and a z of 1.5 in HR
// are standardised against different fields with different spreads, different
// depths and different base rates. Ordering four categories by that number is
// a cross-scale comparison — the same mistake as ranking a 70 on the HR board
// against a 70 on the Hits board, which this repo forbids everywhere else.
//
// So the HOME RUN CALL LEADS, every night, in the same place. This is a site
// called MOONSHOT with an HR in the logo; the hero should not move, and a page
// you read daily is one you should be able to find your way around without
// re-reading the label.
//
// Nothing is hidden by that. Every call still prints its own clearance clause,
// and when another call genuinely stands further clear of its own field the
// page SAYS SO on a line of its own, rather than silently promoting it.
//
// THE MARGIN IS DELIBERATELY CONSERVATIVE, precisely because the comparison is
// soft. The challenger must clear all three:
//   · depth >= 3      — a pool of one or two has no field to stand clear of,
//                       and convictionOf already returns null below 3.
//   · z >= 0.8        — the same bar both surfaces already use before they will
//                       print a clearance claim at all (see ReadTeaser's
//                       `clear`), so this line can never assert something the
//                       clause beneath it declines to.
//   · z >= hero.z+0.5 — half a standard deviation of daylight between the two.
//                       Without it the line fires on noise every other night
//                       and stops meaning anything.
//
// Returns the challenger call, or null. Both TheRead and ReadTeaser import
// this, so Home and the Bot page cannot disagree about who leads or about
// whether anybody out-cleared him.
export const OUTCLEAR_MIN_Z = 0.8
export const OUTCLEAR_MARGIN = 0.5

export function heroCall(calls = []) {
  // `calls` arrives in CALL_CATS order, which is HR first. The head is the home
  // run call whenever the slate has one, and degrades to the next category that
  // exists on a slate with no HR designation at all.
  return (calls || [])[0] || null
}

export function outClearedBy(hero, calls = []) {
  const hz = hero?.conv?.z
  ...
  for (const c of calls || []) {
    ...
    if ((c?.conv?.depth ?? 0) < 3) continue
    if (z < OUTCLEAR_MIN_Z) continue
    if (z < hz + OUTCLEAR_MARGIN) continue
    if (!best || z > best.conv.z) best = c
  }
  return best
}
