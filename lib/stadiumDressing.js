...
// photograph and a diagram never does — foul poles, a batter's eye,
...
export function addParkDressing(scene, { P, wallD, wallH, bowl, foulPens = true }) {
  ...
  {
    // Every park has one — an open sector behind centre (Comerica's, with
    // the fountain on top; Petco's, with the berm behind) still does.
    // `eye: false` on a bowl is the only thing that removes it.
    if (!(bowl && bowl.eye === false)) {
      const pos = []
      for (let a = -7; a < 7; a += 1) {
        const b = a + 1
  ...
  // ── NO DUGOUTS (2026-09-02). Donovan: "those yellow and black things in
  //    the infield — remove them." They were the dugout mouths with a lit
  //    lip; from the broadcast seat they read as blocks on the grass.
  ...
  //    otherwise leaves a bare apron. Skipped when the park's own props
  //    put the pens beyond the wall (Comerica, Petco, Chase).
  if (foulPens) for (const side of [-1, 1]) {
    const a = side * 49
    const v = P(255, a)
