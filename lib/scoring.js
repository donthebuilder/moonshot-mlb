...
import {
  clean, n, nn, pick,
  hrScore, hitScore, prodScore, tbScore, pitchMixScore, mlbId,
...
import { stripGlyphs, tierTone } from './roleBadge'
...
export function avoidHRCandidate(p) {
  ...
  // A DESIGNATED HR/TOP PICK IS EXEMPT from the heuristics below, and this
  // is measured, not diplomatic. Users rightly asked how a hitter can be the
  // game's HR pick AND wear a skip tag — so the heuristics were tested on
  // all 1,377 HR-type picks in the graded archive:
  //
  //   "better on another market" flagged picks homered 20.2% vs 16.7% clean
  //   "high K risk" flagged picks homered 24.8% vs 17.2% clean
  //   low ISO (<.18) flagged picks homered 11.5% vs 19.5% clean
  //
  // Two of the three warnings flag BETTER HR picks — high-K sluggers are
  // three-true-outcome bats, and a big hit/HRR score doesn't subtract power.
  // Only the ISO warning survives contact with the data, and it already has
  // its own red pill. So on a designated HR pick, only the bot's own explicit
  // trap flag (above) can produce an avoid — the site no longer argues with
  // the designation using heuristics the archive says are backwards.
  const designated = String(p?.game_pick_role || '').split('/')[0].trim().toUpperCase()
  if (designated === 'HR' || designated === 'TOP' || designated === 'TOP15') return false
  const hr = hrScore(p)
...
export function tierRole(p) {
  // DE-EMOJIFIED AT THE SOURCE (2026-08-14). The bot publishes this tier with
  // a pictograph baked in — "🏆 HR Bet", "⛔ True Avoid" — and roughly ten
  // components render the string straight through. Stripping it here fixes
  // every one of them at once instead of ten times over, and keeps the bot
  // and the data branch untouched.
  //
  // Order matters with tierColor below: colour used to be sniffed off the
  // glyph, so removing the glyph without moving colour onto a semantic token
  // first would have quietly turned every badge orange. See lib/roleBadge.js.
  const raw = stripGlyphs(clean(p?.final_hr_role, ''))
...
export function tierColor(role, C) {
  // Was: six `s.includes(<emoji>)` checks. That made the pictograph
  // load-bearing rather than decorative — the moment the glyph left the
  // string, every tier fell through to the default orange. Resolution now
  // runs on a semantic token that matches the glyph OR the text, so it
  // survives the emoji being gone and also handles values that never had one.
  const badge = tierTone(role, C)
  return badge || roleColor(stripGlyphs(role), C)
}

export function scoreFor(p, type = 'hr') {
  // TOP board (2026-08-08): the bot's own overall ranking, unadjusted —
  // top_board_score_v2 is the number its Top-30 sheet sorts by.
  if (type === 'top') return n(p?.top_board_score_v2 ?? p?.overall_score, 0)
  ...
  // ── HR RANKING IS THE BOT'S RAW SCORE (2026-08-09, Donovan: "use the raw
  // HR score") ────────────────────────────────────────────────────────────
  //
  // HISTORY, kept because it explains why the adjustment existed. On
  // 2026-08-04 this returned isoAdjustedHr(p, hrScore(p)): the 39-day graded
  // archive (3,973 slots) had shown season ISO out-predicting hr_score for
  // home runs — 8.2% to 22.2% across ISO bands against +4.7 points across
  // score quartiles — so the site multiplied the bot's score by the measured
  // relative HR rate of the hitter's ISO band. That finding was real and the
  // derivation still lives in lib/scoring_additions.js.
  //
  // TWO THINGS MADE IT THE WRONG PLACE TO APPLY IT, and both are checkable:
  //
  //   1. hr_score ALREADY CONTAINS ISO. Corrected 2026-08-09 after checking
  //      the bot rather than trusting the first thing that looked like proof:
  //      I originally cited build_game_pick_role_map()'s _power_rank, but that
  //      only decides WHICH hitter gets designated, not what hr_score is
  //      worth. The real evidence is inside the score. hr_score = hr_score_v2,
  //      whose weight table carries season_power at 0.12, and season_power is
  //      100 × (0.50 × ISO + 0.30 × HR/PA + 0.20 × SLG) — half ISO outright,
  //      the rest power measures. The trap penalty gates on ISO too
  //      (mlb_dashboard.py:6473, :6698, weights at :158).
  //
  //      So the site was multiplying an ISO band rate onto a number ISO had
  //      already shaped. Worth being precise about the sizes, because it cuts
  //      both ways: the bot's internal term is 12% of the blend, while the
  //      site's multiplier ran ×0.56 to ×1.52 — a ±50% swing. Removing it is
  //      not neutral. It takes real ISO weight out of the ranking.
  //
  //   2. IT BROKE THE ONE CALIBRATED SURFACE. ProjectedOutput's CALIB bands
  //      are copied verbatim from the Streamlit build and were MEASURED
  //      against the bot's published score bands — raw hr_score. Feeding them
  //      the site-adjusted number looked up a hitter's rate in a table built
  //      for a different scale. Proj HR is the only genuine prediction on the
  //      site, and it was the surface the adjustment corrupted.
  //
  // So the site now ranks on the bot's own number, and the ISO story is told
  // where it belongs: as its own visible ISO column beside the score, so a
  // reader can see it rather than have it silently folded in.
  //
  // isoAdjustedHr stays exported — this is a change of WHICH NUMBER THE SITE
  // RANKS ON, not a deletion of the research behind it.
  return hrScore(p)
}
...
// ── 🔒 THE HR RANK — one ordering, everywhere, forever ─────────────────────
//
// 2026-08-11, Donovan: "give me the ranking on the hr board that will show me
// the order the players are in on the results page. and don't change it ever
// again."
//
// The drift he was seeing was real and structural. Gone Yard (Scoreboard)
// ranked the FULL slate by raw hrScore; the HR board ranked the FILTERED list
// and numbered it i+1 — so touching a team chip or the search box renumbered
// every hitter, and the two pages could not agree even with no filters if
// either ever changed its sort.
//
// This is now the ONLY place the HR rank is computed. Both surfaces call it,
// it always ranks the FULL slate, and scripts/check-rank-lock.mjs fails the
// build if either surface stops calling it or this function stops ranking on
// raw hr_score. That is what "don't change it ever again" means in a codebase:
// not a promise, a check.
//
// Tie-break is the league's numeric id, ascending — arbitrary but PERMANENT,
// so two hitters on the same score land in the same order on every page,
// every render, every device.
// 2026-09-25: the rank is the bot's board_rank (lib/boardOrder.js), not a
// sort on hr_score. Same map, same callers; only the order changed.
export function hrRank(players) {
  const ranked = boardOrder(players)
  const m = new Map()
  ranked.forEach((p, i) => {
    const id = mlbId(p)
    if (id != null && !m.has(id)) m.set(id, i + 1)
  })
  return m
}
...
export function bestBet(p, type = 'hr') {
  // Same treatment as tierRole: best_bet_type arrives glyph-first from the bot
  // and is rendered raw on the player card.
  const raw = stripGlyphs(clean(p?.best_bet_type || p?.bet_type || p?.best_use, ''))
...
export function isAligned(p) {
  // REBUILT ON MEASURED SIGNALS (2026-08-04). The old definition honoured the
  // bot's 🧩 tag and otherwise required weak-spot + pitch-match + a soft
  // "real contact" bar. Audited against the graded archive:
  //
  //   🧩-tagged slots homered 15.4% vs 14.6% untagged (n=39) — nothing.
  //   weak_spot_flag        18.0% vs 13.9%  — validated
  //   pitch_type_match_flag 18.4% vs 13.6%  — validated
  //   BOTH flags            23.3% (n=219)
  //   BOTH + ISO ≥ .18      29.2% (n=154)  — more than 2× the 12.9% baseline
  //
  // So Aligned now means the measured stack: both validated matchup flags
  // plus real season power. The 🧩 tag is dropped as an input (39 samples of
  // nothing), and the fuzzy "real contact" bar is replaced by the one power
  // input the archive keeps validating. This is the strongest composite
  // signal on the site, and for once that's a measured sentence.
  const weakSpot = p?.weak_spot_flag === true
  const pitchMatch = p?.pitch_type_match_flag === true || n(p?.pitch_type_match_score, 0) > 0
  const power = n(p?.season_iso, 0) >= 0.18
  return weakSpot && pitchMatch && power
}
