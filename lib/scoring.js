...
export function compactRole(p) {
  const r = roleRaw(p).toLowerCase()
  if (avoidHRCandidate(p)) return 'Avoid HR'
...
export function bestBet(p, type = 'hr') {
  ...
  if (role === 'Avoid HR') return 'Avoid HR'
  return 'HR'
}
...
export function isAligned(p) {
  return (p?.top_board_tags || []).some((t) => String(t).includes('🧩'))
}
