...
// ── WHAT IT COST WHEN IT WORKED (2026-09-01) ────────────────────────────────
//
// Donovan, confirming the board: "streaks and scores and price hit rate like
// the avg price pregame when they cash." `avg_price` is what he has been
// offered on EVERY night; this is what he was offered on the nights he
// actually cleared the bar. The two diverge in a useful direction — a hitter
// whose cash nights were all +900 and whose misses were all +400 is a man the
// book prices right when it's short and wrong when it's long. Averaged as
// probability and converted back, same convention the bot uses for avg_price,
// so the two columns are comparable.
//
// The log is [date, price, got] newest first. Nothing here is a model: it is
// the same receipts the row already shows, summed.
export function cashSummary(log) {
  const rows = (Array.isArray(log) ? log : [])
    .filter((e) => Array.isArray(e) && e.length >= 3)
    .map(([date, price, got]) => ({ date: String(date), price: Number(price), got: Number(got) > 0 }))
    .sort((a, b) => (a.date < b.date ? 1 : a.date > b.date ? -1 : 0))
  if (!rows.length) return { streak: 0, cashPrice: null, lastCash: null }
  // Current run, newest first: +3 = cleared his last three priced nights,
  // -4 = missed his last four. Sign carries the direction.
  let streak = 0
  const first = rows[0].got
  for (const r of rows) { if (r.got !== first) break; streak += 1 }
  if (!first) streak = -streak
  const cashed = rows.filter((r) => r.got && Number.isFinite(r.price) && r.price !== 0)
  let cashPrice = null
  if (cashed.length) {
    const probs = cashed.map((r) => (r.price < 0 ? -r.price / (-r.price + 100) : 100 / (r.price + 100)))
    const p = probs.reduce((a, b) => a + b, 0) / probs.length
    cashPrice = p >= 0.5 ? -Math.round((100 * p) / (1 - p)) : Math.round((100 * (1 - p)) / p)
  }
  return { streak, cashPrice, lastCash: cashed[0]?.date || null }
}
...
export function flatten(hist, { minN = 5 } = {}) {
  ...
  Object.entries(players).forEach(([pid, p]) => {
    Object.entries(p?.markets || {}).forEach(([key, b]) => {
      ...
      const cash = cashSummary(b.log)
      out.push({
        streak: cash.streak,
        cashPrice: cash.cashPrice,
        lastCash: cash.lastCash,
        id: `${pid}|${key}`,
        pid: Number(pid),
        name: p.name || `#${pid}`,
        team: p.team || '',
...
// ── 💵 THE REALITY CHECK ─────────────────────────────────────────────────────
//
// 2026-08-15, from Donovan's own panel and the line under it: "hit rate alone
// is not the finish line. A +700 HR can miss often and still profit; a -170
// prop can hit often and still bleed."
//
// He is right and it indicts everything else on this page. `rate`, `edge`,
// `true_price` are all percentages, and a percentage does not tell you whether
// a unit came back. bots/odds_history.py now settles every priced prop at flat
// stake and publishes the ROI; this reads it.
//
// THE VERDICT IS GATED ON THE ERROR BAR, and the bar here is wider than
// intuition says. ROI's spread comes from the RETURNS, not the win rate — one
// +900 winner moves a small book more than twenty -150 winners — so a cohort
// can post +30% and be indistinguishable from zero. Nothing here calls a
// number profitable until it clears two of its own standard errors.

export const ROI_MARKET = MARKET_LABEL

export function roiVerdict(r) {
  if (!r || !r.n) return { key: 'none', label: 'no bets yet', tone: '#8b8b95' }
  if (r.thin) return { key: 'thin', label: 'too few bets', tone: '#8b8b95' }
  const z = r.roi_se ? r.roi / r.roi_se : 0
  if (z >= 2) return { key: 'up', label: 'profitable', tone: '#4ade80' }
  if (z <= -2) return { key: 'down', label: 'losing', tone: '#f87171' }
  return { key: 'flat', label: 'inside the noise', tone: '#FCD34D' }
}

/** Market rows for the reality-check band, biggest book first. */
export function roiRows(hist) {
  const roi = hist?.roi
