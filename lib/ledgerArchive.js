 ...
 */
/**
 * Fetch + transform one night off the branch's graded file — PURE. No
 * localStorage, no caching, no side effects: same date in, same shape out,
 * every time. This is the function to call when a caller wants the real
 * answer rather than a browser's copy of one from weeks ago (see
 * lib/calledLedger.js — Path to Victory A10's Called Ledger keeps its own
 * in-memory-only cache on top of this precisely so a fresh page load always
 * re-derives the truth from the branch, not from whatever this device
 * happened to store).
 *
 * `hasCapture` says whether THIS night's file actually carries
 * hr_capture_report.all_homer_entries at all — confirmed by hand
 * (2026-09-15) that the schema is NOT stable across the season: files before
 * roughly late May use several older shapes (a bare array of picks, or an
 * object with `hrs_caught`/`results` and no `graded_slots` at all) with no
 * capture report, so `all` silently being `[]` on those nights means "this
 * format predates the report", not "nobody homered". Callers that build a
 * season total MUST gate on this flag rather than trust a zero — the exact
 * "zero is a join bug until proven otherwise" rule the rest of this repo
 * already follows.
 */
export async function fetchGradedNight(date) {
  if (!date) return null
  let payload = null
  try {
  ...
  const hasCapture = Boolean(payload.hr_capture_report)
  ...
  const all = allEntries
    .map((e) => {
      ...
      return {
        ...
        onSheet: Boolean(g || c),
        role: role || null,
        // CALLED = TOP, HR, HIT, HRR or CONTACT (2026-09-15, Donovan; CONTACT
        // added 2026-09-26 -- lib/callStatus.js). Each of those
        // four badges is its own lane with its own hit-rate scoreboard
        // elsewhere on the site (Path to Victory A1's PICKS lane, A9's Hits/
        // HRR lanes) -- a real designated call, not a ranking band. WATCH is
        // documented in A1 as explicitly "not a call"; CONTACT and TOP15 are
        // ranking bands, not designated picks. Both stay ON BOARD, not CALLED.
        badged: isCalledRole(role),
...
/**
 * The same fetch, cached to this device's localStorage — the shape every
 * existing caller here (LedgerLab, SeasonRecord) already expects. Kept as a
 * thin wrapper over fetchGradedNight so there is exactly one place that
 * parses a graded file, not two that can drift.
 */
