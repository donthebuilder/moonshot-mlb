...
const SCHED_FIELDS = 'dates,date,games,gamePk,status,abstractGameState,detailedState,teams,home,away,team,id,score,linescore,currentInning,inningState,outs,offense,batter,onDeck,inHole,first,second,third,fullName'
...
export function liveSlateStatus() {
  return { at: _at || 0, failedAt: _failedAt, stale: Boolean(_failedAt && _failedAt > _at), reason: _failReason }
}
...
export function fetchLiveSlate({ force = false } = {}) {
  ...
  _inflight = pullLiveSlate()
    .then((s) => {
      if (s) { _snap = s; _at = Date.now(); _failedAt = 0; _failReason = '' }
    ...
    .catch((err) => {
      _failedAt = Date.now()
      _failReason = `threw: ${err?.message || err}`
      console.error('[liveSlate] pullLiveSlate threw', err)
      return _snap
    })
...
async function pullLiveSlate() {
  ...
  const schedUrl = `https://statsapi.mlb.com/api/v1/schedule?sportId=1&startDate=${yday}&endDate=${today}&hydrate=linescore&fields=${SCHED_FIELDS}`
  const schedRes = await fetch(schedUrl).catch((err) => { console.error('[liveSlate] schedule fetch threw', err); return null })
  const sched = schedRes ? (schedRes.ok ? await schedRes.json().catch(() => null) : null) : null
  ...
  if (!sched) {
    _failReason = schedRes ? `schedule ${schedRes.status} ${schedRes.statusText}` : 'schedule fetch threw or returned unparseable JSON'
    console.error(`[liveSlate] ${_failReason} (${schedUrl})`)
    return null
  }
  const dated = []
  ...
  if (!games.length) {
    // The schedule call SUCCEEDED — this is a genuine zero, not a dropped
    // request — unless every game's gameDate came back empty, which means
    // the whitelist stripped the `date` sub-field again (2026-09-08: it had
    // never been in SCHED_FIELDS at all, so this filter silently zeroed the
    // slate on every pregame hour with no game yet Live). Logged loudly
    // either way, because a true off day and a broken filter must never look
    // identical again.
    const anyDated = dated.some((d) => d.date)
    _failReason = anyDated ? 'genuine off day — no games matched etDay and none Live' : 'every game.date came back empty — SCHED_FIELDS whitelist likely stripped it again'
    console.error(`[liveSlate] zero games after filter: ${_failReason} (etDay=${etDay}, raw dates=${dated.length})`)
    return null
  }
    ...
    ;['home', 'away'].forEach((side) => {
      ...
      Object.values(box.teams[side]?.players || {}).forEach((pl) => {
        ...
        lines[id] = {
          ...
          // WHICH SIDE, DIRECT FROM THE BOX (2026-09-06, Donovan: a pinch-hit
          // homer came back with no team/opponent -- "the bot's had him"
          // details were blank). The old path made a homer's team a SECOND
          // lookup, later, against g.lineup[side] -- the posted STARTING
          // lineup, keyed off battingOrder. A pinch hitter's own at-bat is
          // right here in this same loop, already inside the correct side's
          // players object; battingOrder is a separate field on the same
          // record and has no bearing on whether this stat line resolves.
          // Recording the side we are ALREADY iterating removes that whole
          // second lookup -- and the dependency on batting-order data ever
          // being complete or timely -- for every batter with a real line.
          side, teamId: sideTeamId,
