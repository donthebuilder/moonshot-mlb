...
//
// D/ST used to be partial unconditionally -- sacks, interceptions, fumble
// recoveries and defensive touchdowns were absent from the payload by design.
// The bot publishes them now (team_defense in nfl_week.json), so this asks the
// same question of a defence it asks of a quarterback: does THIS row carry the
// term. A payload from before that change, or an archived one, still says
// partial and still says it truthfully.
export function projectionIsPartial(player) {
  // Nothing to be partial ABOUT: no NFL history exists for him yet, so the
  // projection is not incomplete, it is absent. Flagged so the UI can say that
  // instead of printing 0.0 next to a first-round rookie.
  if (player?.source_payload?.no_data) return true
  const stats = player?.source_payload?.stats || {}
  if (player?.position === 'QB') return !Number.isFinite(Number(stats.PATD))
  if (player?.position === 'DEF') return !Number.isFinite(Number(stats.def_sacks))
  return false
}
...
// ── A D/ST HAS NO PROP MARKET, AND 50 IS NOT A SCORE ────────────────────────
//
// `dashScore` falls back to 50 for a man no market priced. For a skill player
// that is rare. For a DEFENCE it is EVERY row, always: the feed carries no
// defensive props, so all 32 D/ST entries score exactly 50 and the Wire --
// which sorts by this number under a header saying "Ranked by this week's
// market score" -- fell back to its alphabetical tiebreak. Filter the Wire to
// DEF and you get Arizona, Atlanta, Buffalo, Carolina: the alphabet, printed
// as if it were a ranking, with the best defence on the board (Chicago, 6.6
// projected) five rows below the worst (Arizona, 3.0).
//
// The 50 stays where it is -- it is load-bearing for sorting a mixed list --
// but callers can now ask whether a number was MEASURED or merely defaulted,
// and print a dash instead of a fake score when it was defaulted.
export function hasMarketScore(player) {
  if (player?.source_payload?.on_bye) return false
  return Object.values(player?.source_payload?.scores || {}).map(Number).filter(Number.isFinite).length > 0
}
...
export function projectedFantasyPoints(player, scoring = 'ppr') {
  ...
  if (position === 'DEF') {
    ...
    // SAFETIES (2026-09-11, item 8). The other four D/ST terms here all have
    // one -- this was the one category with no term at all, so a D/ST that
    // notches a safety scored the same as one that didn't. Worth 0.06/game
    // league-wide (rare, but real when it happens, and worth a full 2 like
    // an interception or fumble recovery in every scoring system this app
    // uses). Read defensively like every other bot-fed key here: absent
    // until the bot publishes `def_safeties`, contributes 0 until then, no
    // crash either way.
...
export function fantasyPointsFromStats(stats = {}, scoring = 'ppr') {
  ...
  points += number(stats.two_point_conversions) * 2 - number(stats.fumbles_lost) * 2 + number(stats.return_touchdowns) * 6
  ...
  points += number(stats.def_sacks) + number(stats.def_interceptions) * 2 + number(stats.def_fumble_recoveries) * 2 + number(stats.def_touchdowns) * 6 + number(stats.def_safeties) * 2
...
// -- THE LINEUP NOBODY HAS SET YET (2026-09-12, DASH COACH GAME PLAN) -------
//
// fantasy_lineup_slots only ever gets a row when a manager calls
// set_fantasy_lineup_slot (202608260001) -- there is no default and no
// carry-forward, so "next week's lineup" usually does not exist as data at
// all, for either side of a matchup. Planning a week ahead means computing
// what the best lineup COULD be, not reading one that was set.
//
// This is the same fill-dedicated-then-flex order replacementLevels already
// uses, run per team instead of league-wide -- and it is exactly optimal
// here, not a heuristic. Every slot but FLEX accepts exactly one position
// (eligibleForSlot), so the position pools behind QB/RB/WR/TE/K/DEF never
// compete with each other: taking each pool's best players first can never
// cost a better assignment elsewhere. FLEX is the one slot that reaches
// across RB/WR/TE, and by the time the dedicated slots are filled, the only
// players left in those three pools are -- by definition -- each pool's
// next-best. The single highest projection among THEM is the single highest
// projection FLEX could possibly hold. No swap of two filled slots can ever
// raise the total, so this is the assignment, not an estimate of one.
//
// `roster` should already have `.id` and a numeric `.projection` attached
// (as every caller here does via dashScore/projectedFantasyPoints) -- and
// should already exclude anyone who can't play this week (a bye), since the
// optimizer has no way to know that on its own.
export function bestPossibleLineup(roster, requiredSlots) {
  const list = requiredSlots || []
  const pool = (roster || []).filter((player) => player && Number.isFinite(Number(player.projection)))
  const used = new Set()
  const queues = {}
  for (const slot of new Set(list)) {
    if (slot === 'FLEX') continue
    const count = list.filter((s) => s === slot).length
    const picks = pool.filter((player) => !used.has(player.id) && eligibleForSlot(player, slot))
      .sort((a, b) => b.projection - a.projection).slice(0, count)
    for (const player of picks) used.add(player.id)
    queues[slot] = picks
  }
  const flexCount = list.filter((s) => s === 'FLEX').length
  queues.FLEX = pool.filter((player) => !used.has(player.id) && eligibleForSlot(player, 'FLEX'))
    .sort((a, b) => b.projection - a.projection).slice(0, flexCount)
  for (const player of queues.FLEX) used.add(player.id)
  const cursor = {}
  const starters = list.map((slot) => {
    const i = cursor[slot] || 0
    cursor[slot] = i + 1
    return { slot, player: queues[slot]?.[i] || null }
  })
  const totalProjection = starters.reduce((sum, row) => sum + (row.player?.projection || 0), 0)
  const filled = starters.filter((row) => row.player).length
  return { starters, totalProjection, filled, total: starters.length }
}
