...
// 10 MINUTES, NOT ONE (2026-09-24). Supabase flagged the project EXCEEDING
// USAGE LIMITS: 5.58 GB of egress against the Free plan's 5 GB this cycle.
// Most of it was draft week (Sep 5-7, ~2.4 GB, before this cache existed);
// since then it runs ~70-300 MB a day, and this read -- ~0.23 MB each time a
// server instance's cache is cold -- is the largest repeated one on every
// Franchise page. Nothing in it moves faster than the scoring cron that
// writes it, which runs every ten minutes, so a one-minute TTL was re-reading
// data that could not have changed. Injury tags can lag the cron by up to one
// more cycle; lineups, rosters and scores are never cached here.
