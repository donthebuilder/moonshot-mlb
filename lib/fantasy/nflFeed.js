...
export async function loadFranchiseNflFeed() {
  ...
  return {season,games,players:[...players,...defenceWeekRows(games,season,raw?.team_defense?.weeks)],catalog,builtAt:raw?.built_at||null,source:raw?.source||games[0]?.source||'dash'}
}
...
// before.
function defenceWeekRows(games, season, weekly = null) {
  const rows = []
  for (const game of (games || [])) {
    ...
    for (const [team, allowed] of [
      ...
      const line = weekly?.[String(game.week)]?.[team] || null
      const stats = scored
