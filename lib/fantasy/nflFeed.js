...
export async function loadFranchiseNflFeed() {
  ...
  const games=(raw?.games||[]).filter((game)=>game?.game_id&&game?.kickoff&&Number(game?.week)>=1&&Number(game?.week)<=22).map((game)=>({
  ...
  const gameByTeam=new Map(games.flatMap((game)=>[[game.homeTeam,game],[game.awayTeam,game]]))
  const players=(raw?.players||[]).map((player)=>{const game=gameByTeam.get(String(player.team||'').toUpperCase());const week=Number(game?.week||raw.week);if(!game||week<1||week>22)return null;return {
    ...
    stats:normalizeStats(player.game_stats||player.fantasy_stats||{}),
