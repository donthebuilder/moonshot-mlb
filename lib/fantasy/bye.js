 ...
 *
 * THE BOT'S OWN ANSWER COMES FIRST (2026-09-06). The arithmetic above is a
 * good inference and it is still the fallback, but it is an inference: it
 * cannot tell a real fourteen-game bye week from a feed halfway through
 * publishing, and the floor it uses to protect against that is a guess about
 * how partial a partial slate looks. The slate now publishes `on_bye` per
 * player -- computed where the schedule actually is, by the same join that
 * decides whether he gets scored at all -- so when the row carries it, it is
 * the answer and no arithmetic is needed.
 */
export function isOnBye(player, byeTeams) {
  const flagged = player?.source_payload?.on_bye
