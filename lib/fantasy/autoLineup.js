...
export async function benchUnavailableStarters(db, { season, week, now = Date.now(), commit = true, projector = averageProjector, slateIds = null } = {}) {
  ...
  try {
    ...
    // Once every game this week has kicked off no swap is possible, so the
    // whole-league roster read below (every rostered man's payload, every
    // tick) is skipped until the next week's slate arrives. Supabase egress.
    if (![...kickoffByClub.values()].some((at) => now < at)) return { ...result, skipped: 'all_kicked_off' }
