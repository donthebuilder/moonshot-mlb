...
export async function autoFillLineups(db, { season, week, now = Date.now(), commit = true, projector = averageProjector } = {}) {
  ...
  try {
    ...
    for (const team of liveTeams) {
      ...
      const bench = roster
        ...
        .filter((p) => !usedPlayers.has(p.id) && !onBye(p) && !started(p) && !isUnavailable(p))
...
export async function benchUnavailableStarters(db, { season, week, now = Date.now(), commit = true, projector = averageProjector } = {}) {
  ...
  try {
    ...
    for (const team of liveTeams) {
      ...
      for (const row of slots.filter(isStarter)) {
        ...
        if (!out || row.locked_at || started(out) || !isUnavailable(out)) continue
        const benchRowOf = new Map(slots.filter((r) => !isStarter(r)).map((r) => [r.player_id, r]))
        const candidates = [...roster.values()]
          .filter((p) => !startersNow.has(p.id) && eligible(p, row.slot) && !isUnavailable(p) && !onBye(p) && !started(p))
        ...
        result.swaps.push({
          ...
          benched: `${out.name} (${out.injury_status})`,
