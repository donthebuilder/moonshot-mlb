...
export function buildGroupTickets(players = [], {
  ...
  const pinKey = pinnedId == null || pinnedId === '' ? null : String(pinnedId)
  const isPinned = (leg) => pinKey != null && String(legId(leg.player)) === pinKey
  ...
  if (pinKey != null) {
    out = tickets.filter((t) => {
      const has = (t.legs || []).some((l) => String(legId(l.player)) === pinKey)
      if (!has) pinnedDropped += 1
      return has
    })
  }
  return { tickets: out, counts, collapsed, emptyGroups, signalMen, pinnedDropped, pinned: pinKey }
}
