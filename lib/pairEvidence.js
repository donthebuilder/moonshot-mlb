 ...
 */
/**
 * ── SIX CARDS, SIX DIFFERENT PAIRS (2026-08-17) ────────────────────────────
 *
 * Donovan: "names also duplicated idk whats thats about."
 *
 * The strip showed six cards all reading 5.3% / 3.1 over random, with Miguel
 * Vargas in three of them, Matt Olson in three and Eduardo Valencia in three.
 * Not a data error — an inevitable consequence of the ranking.
 *
 * `rate` comes from the single strongest matching rule, so EVERY pair of two
 * TOP-designated bats returns the identical 5.3%. Inside that flat band the
 * tiebreak was `hrA + hrB`, i.e. pure score sum. The six highest score sums
 * among the top bats are necessarily the same three or four men recombined:
 * Vargas+Olson, Vargas+Valencia, Olson+Valencia, then each of them against the
 * next bat down. Taking the head of a band ordered by score sum GUARANTEES
 * repetition.
 *
 * The fix is not a dedupe filter bolted on the end — it is admitting that the
 * list's job is to show six *usable* pairs, and a sixth card naming a man
 * already on three others is not a sixth option. So: greedy selection with an
 * appearance cap. Walk the ranked list and take a pair only if neither half has
 * already been used MAX_APPEARANCES times. Two, not one — a genuinely
 * exceptional bat should be allowed to anchor a second pair, because forbidding
 * that would hide the real answer to "who do I build around".
 *
 * Nothing is invented and nothing is reordered: the ranking is untouched, the
 * measured rate on each card is the same measured rate it always was. Cards are
 * only ever SKIPPED, and if the cap starves the list the leftovers are appended
 * in rank order rather than returning fewer than asked for — a short list would
 * be a silent cap, and silent caps read as "there was nothing else".
 */
export const MAX_APPEARANCES = 2

export function buildPairs(players = [], { limit = 12, maxAppearances = MAX_APPEARANCES } = {}) {
  const pool = players
  ...
  const ranked = out.sort((x, y) => y.sort - x.sort)

  const seen = new Map()
  const count = (p) => seen.get(String(p?.player_id)) || 0
  const bump = (p) => seen.set(String(p?.player_id), count(p) + 1)

  const picked = []
  const skipped = []
  for (const cand of ranked) {
    if (picked.length >= limit) break
    if (count(cand.a) >= maxAppearances || count(cand.b) >= maxAppearances) {
      skipped.push(cand)
      continue
    }
    bump(cand.a); bump(cand.b)
    picked.push(cand)
  }
  // Backfill in rank order if the cap left the list short of `limit`. Better a
  // repeated name at the tail than a list that quietly claims the slate had
  // only four pairs worth showing.
 ...
 */
/**
 * ── ONE ENGINE, WITH A MAN YOU NAMED IN IT (2026-08-17) ─────────────────────
 *
 * Donovan: "why cant buuld from groups enging and the pair builder engine wokr
 * on the same build does that make isnes"
 *
 * It makes complete sense, and until now the answer was only that nobody had
 * joined them. The two engines were never in conflict — they take the SAME
 * slate rows, produce the SAME leg objects, collapse a two-designation hitter
 * the same way and honour the same one-leg-per-game rule. The anchor builder
 * simply started from a name and the group builder started from the markets,
 * and the site made you pick which end to start from.
 *
 * `pinnedId` removes the choice. Name a hitter and he holds a leg; the group
 * engine fills the rest of the ticket around him under all its normal rules.
 * Leave it null and this behaves exactly as it always did.
 *
 * Implemented by RANK, not by a second code path: the pinned man is sorted to
 * the front of every group pool he belongs to, and any ticket that fails to
 * contain him is dropped rather than shown. That matters — a "build around
 * Alonso" that quietly returns tickets without Alonso is worse than returning
 * none, because you would have to check every row to notice.
 */
export function buildGroupTickets(players = [], {
  ...
  reserve = true,
  pinnedId = null,
  // ── ONE OR MORE (2026-08-17) ─────────────────────────────────────────────
  // Donovan: "pair builder should be able to build around one player OR MORE."
  // pinnedIds supersedes pinnedId (kept for compatibility); every pinned man
  // takes a slot before the reserve, none is consumed across tickets, and a
  // ticket missing ANY of them is dropped.
  pinnedIds = null,
} = {}) {
  const pinKeys = new Set(
    (Array.isArray(pinnedIds) ? pinnedIds : (pinnedId == null || pinnedId === '' ? [] : [pinnedId]))
      .filter((v) => v != null && v !== '').map(String),
  )
  const pinKey = pinKeys.size ? true : null      // truthiness only, below
  const isPinned = (leg) => pinKeys.size > 0 && pinKeys.has(String(legId(leg.player)))
  ...
  picked.forEach((g) => {
    ...
    // The pinned man is exempt from the SIGNAL filter — you asked for him by
    // name, so a signal switch must not silently drop him — and he sorts first
    // in every group he holds.
    const kept = all.filter((l) => carries(l) || isPinned(l)).sort(rankLegs)
    if (pinKey != null) {
      kept.sort((a, b) => (isPinned(b) ? 1 : 0) - (isPinned(a) ? 1 : 0))
    }
  ...
  for (let t = 0; t < limit; t += 1) {
    ...
    //    false comparison.
    // ── THE PINNED MAN TAKES HIS SLOT FIRST (2026-08-17) ──────────────────
    // He has to be placed BEFORE the reserved-signal leg, not after. Sorting
    // him to the front of his group pool was not enough: the reserve step
    // scans every group for the best signal-carrying leg and claims that
    // group's slot, so a HIT reserve that was not the pinned man filled the
    // HIT slot and the pinned man never entered the ticket at all. Every
    // ticket then failed the contains-him filter and the panel reported "not
    // designated in the groups you picked" about a hitter who plainly was.
    // Found by testing the POSITIVE path after the refusal path passed —
    // a guard firing correctly is not evidence that the thing it guards works.
    if (pinKeys.size) {
      // EVERY pinned man gets his slot, not just the first — walk until all
      // are placed or no empty slot in a group any of them holds remains.
      const placed = new Set()
      for (let i = 0; i < seq.length; i += 1) {
        if (slots[i]) continue
        const leg = pool.get(seq[i]).find((l) => isPinned(l)
          && !placed.has(String(legId(l.player)))
          && !takenMen.has(legId(l.player)) && !takenGames.has(l.game))
        if (!leg) continue
        slots[i] = leg
        placed.add(String(legId(leg.player)))
        takenMen.add(legId(leg.player))
        if (leg.game) takenGames.add(leg.game)
        if (placed.size >= pinKeys.size) break
      }
    }
    ...
    if (reserve) {
      ...
      if (anchor) {
        // Find a slot of the anchor's own group that is still EMPTY. indexOf
        // returned the first slot of that group unconditionally, which on a
        // pinned build would overwrite the pinned man where they share a group.
        let at = -1
        for (let i = 0; i < seq.length; i += 1) {
          if (seq[i] === anchor.group && !slots[i]) { at = i; break }
        }
        if (at >= 0 && !takenMen.has(legId(anchor.player)) && !takenGames.has(anchor.game)) {
          slots[at] = anchor
          takenMen.add(legId(anchor.player))
          if (anchor.game) takenGames.add(anchor.game)
        } else {
          anchor = null      // no room for it; do not claim a reserve that isn't there
        }
      }
    }
    ...
    // The pinned man is deliberately NOT added to usedMen: he is supposed to be
    // on every ticket, so consuming him after the first one would leave tickets
    // 2..N without him and they would all be dropped by the filter below.
    legs.forEach((l) => {
      if (!isPinned(l)) usedMen.add(legId(l.player))
      if (l.game) usedGames.add(l.game)
    })
    tickets.push(finishTicket(legs, t, anchor))
  }

  // A ticket built "around" a named hitter that does not contain him is not the
  // thing that was asked for. Drop rather than display — and report how many
  // were dropped, so an empty result says why instead of looking broken.
  let pinnedDropped = 0
  let out = tickets
  if (pinKeys.size) {
    out = tickets.filter((t) => {
      const ids = new Set((t.legs || []).map((l) => String(legId(l.player))))
      const hasAll = [...pinKeys].every((k) => ids.has(k))
      if (!hasAll) pinnedDropped += 1
      return hasAll
    })
  }
  return { tickets: out, counts, collapsed, emptyGroups, signalMen, pinnedDropped, pinned: pinKeys.size ? [...pinKeys] : null }
}
