...
import { n, mlbId, nameOf } from './player'
...
//
// Four booleans, and not one of them is invented here.
//
...
export const LEG_SIGNALS = [
  ...
  },
  {
    id: 'weak',
    label: 'weak spot',
    say: () => 'he bats in the lineup slot this starter is worst against',
...
/**
 * The signals worth SAYING, as opposed to the ones worth filtering on.
 *
...
function makeLeg(p, group, ctx) {
  return {
    ...
    signals: signalsOn(p, ctx),
    alsoGroups: [],
  }
}

// A leg is better than another leg when it carries more of the signals asked
// for; the category score only breaks the tie. Deliberately that way round —
// the signals are measured against outcomes and the score is a model output.
const rankLegs = (a, b) => (b.signals.length - a.signals.length) || (b.score - a.score)
...
function finishTicket(legs, index) {
  ...
  return {
    ...
    signalCount: legs.reduce((s, l) => s + l.signals.length, 0),
 ...
 *
 * @returns { tickets, counts, collapsed, emptyGroups }
 */
export function buildGroupTickets(players = [], {
  ...
  const blank = { tickets: [], counts: {}, collapsed: 0, emptyGroups: [] }
  ...
  picked.forEach((g) => {
    ...
    counts[g] = { total: all.length, kept: kept.length }
    pool.set(g, kept)
  })
  ...
  if (shape === 'game') {
    ...
    byGame.forEach((legs) => {
      ...
      built.push(finishTicket(merged.slice(0, Math.max(2, size)), built.length))
    })
    built.sort((a, b) => (b.signalCount - a.signalCount)
      || (b.legs.reduce((s, l) => s + l.score, 0) - a.legs.reduce((s, l) => s + l.score, 0)))
    return { tickets: built.slice(0, limit), counts, collapsed, emptyGroups }
  }
  ...
  for (let t = 0; t < limit; t += 1) {
    ...
    for (let i = 0; i < size; i += 1) {
      const g = picked[i % picked.length]
    ...
    tickets.push(finishTicket(legs, t))
  }

  return { tickets, counts, collapsed, emptyGroups }
}
