...
import { solveScale, auditRamp } from './rampSolver'

// 🎨 PALETTE — four heat scales, switchable, per device. Ember by default.
//
...
//   SIGNAL   the props sheet's own four colours, stretched to eight. Deep
//            tinted cell, lit number — the only ramp here that puts the
//            brightness in the TEXT, and the only one that ships paired inks.
//            Says HOW GOOD. The one for ranking a column.
...
//   signal      4.64:1           Δ23           0.012 → 0.087 fill, 0.312 → 0.584 ink
...
//
// Each NEUTRAL-INK ramp switches ink exactly once, on the far side of the gap.
// Asserted. Signal is exempt because it ships its own inks — the dead zone is
// a fact about white and near-black and says nothing about a ramp that brings
// its own colour. It is held to a stricter rule instead: every fill measured
// against its own ink, plus every ink measured against the page.
//
...
//   signal   .71 ↗ .89 ↘ .71   arch, peaks middle  a sweep you can rank
//   verdict  .51 ↗ .74 ↓ .10 ↗ .70   collapses     extremes only
//
// ── COLOUR BLINDNESS ───────────────────────────────────────────────────────
// Red/green is the worst pair for deuteranopia (~8% of men), so luminance
// rises monotonically on all four — and on Signal, on BOTH its channels: strip
// the colour out and each is still correctly ordered dark → light. Hue is the
// fast read, lightness is the fallback that cannot lie. Signal's top two stops
// lean on that fallback entirely: they are the same green and only lightness
// separates them. Verdict leans on red/green hardest, which is why
...
export const RAMPS = {
  ember: {
    id: 'ember',
    label: 'Moonshot',
    blurb: 'The site-standard amber magnitude scale.',
  ...
  traffic: {
    id: 'traffic',
    label: 'Signal',
    blurb: 'The props colours. Deep tinted cells, lit numbers.',
    hint: 'Every step between weak and strong is visible — the one to use when ranking a whole column.',
    // ── REBUILT FROM THE PROPS SHEET (2026-08-10, eighth pass) ──────────
    // Donovan sent two screenshots and said "make Signal this palette."
    //
    // The screenshots are OUR OWN — components/ThresholdGrid.js, the props
    // matrix. So this was not a guess for once: I read the four colours out
    // of the file he was looking at rather than eyedropping a picture.
    //
    //   #f87171  hue 0    under 25%
    //   #f97316  hue 25   25-39%
    //   #FCD34D  hue 46   40-59%
    //   #4ade80  hue 142  60%+
    //
    // WHY THIS RAMP IS BUILT DIFFERENTLY FROM THE OTHER THREE. Those four
    // are TEXT colours sitting on a 7-13% tint of themselves. The brightness
    // lives in the number, not the cell — which is the exact construction I
    // called wrong on 2026-08-09 and I was half right and half wrong about
    // why. What made the neon attempt unreadable was that every cell was the
    // same near-black, so good and bad were equally dark and the ramp had no
    // order left. The props sheet does not have that problem, because it only
    // has four tiers and the hue does the whole job.
    //
    // Stretch four tiers to eight and the problem comes back. So this ramp
    // keeps his construction — deep tinted fill, lit number — and puts BOTH
    // channels on rising ladders:
    //
    //   fill  0.012 → 0.083   the cell still gets lighter as it gets better
    //   ink   0.301 → 0.560   and so does the number
    //
    // Greyscale ordering therefore survives, which is the thing the whole
    // palette file exists to protect.
    //
    // THE PAIRED INK IS THE NEW PART. Every other ramp picks white or near-
    // black per cell by contrast. This one ships its own inks and inkOn()
    // looks up the pair. I argued against per-stop ink in this very file and
    // the argument was that a ramp could use a bright custom ink to dodge the
    // dead-zone rule. That is still true, and the answer is not to ban the
    // mechanism but to check the thing that actually matters: the guard now
    // measures EVERY fill against ITS OWN ink, so a pair that does not clear
    // 4.5:1 fails the build. Worst pair here is 4.55:1. The dead zone is a
    // fact about the two NEUTRAL inks and simply does not apply to a ramp
    // that brings its own.
    //
    // THE SPEC IT WAS SOLVED AGAINST — fills:
    //
    //   @lum .012 .018 .026 .037 .047 .060 .070 .087   rising
    //   @sat .71 .79 .87 .89 .89 .86 .79 .71             arch
    //   @hue 0 14 28 43 57 101 142 142                    holds at the top
    //
    // and inks:
    //
    //   @ilum .312 .332 .362 .399 .440 .489 .533 .584   rising
    //   @isat .71 .80 .86 .90 .90 .86 .80 .72             arch
    //   @ihue 0 14 28 42 58 100 142 142                    holds at the top
    //
    // The ink at each end lands within a few units of the colours he pointed
    // at: #e87575 against his #f87171, #50e386 against his #4ade80.
    //
    // ONE COLOUR AT THE GOOD END (2026-08-10). Donovan: "end in one colour."
    // The hue path used to still be climbing at the top — 101° then 142° — so
    // the two best cells were a lime and a green, which reads as two different
    // verdicts rather than one getting stronger. Hue now HOLDS at 142 for the
    // last two stops and only luminance separates them, so the good end is a
    // single green that gets brighter.
    //
    // That costs something and it is worth naming: two stops at the same hue
    // have only lightness to tell them apart, and inside this construction's
    // narrow fill band there is barely enough. The ladder is a power curve
    // (t^1.3) rather than a straight line for exactly that reason — it spends
    // its steps where the hue has stopped helping. Closest fill pair is Δ23
    // against a floor of 22, the tightest margin in the file.
    stops: ['#3c0a0a', '#431608', '#472405', '#463304', '#423f04', '#1e5006', '#0a5726', '#10602d'],
    inks: ['#e87575', '#ed7c5a', '#ef882e', '#dd9e0c', '#bbb60a', '#51d410', '#19de61', '#50e386'],
  },
...
// The alternative experiments remain readable so an old saved preference can
// be migrated safely, but only the site-standard ramp is offered in controls.
export const RAMP_IDS = ['ember']
// EMBER IS THE DEFAULT AGAIN (2026-08-10). Donovan, after a night with the
// rebuilt Signal on screen: "just do ember instead."
//
// Worth recording rather than just flipping: Signal is the more INFORMATIVE
// ramp — eight steps between weak and strong, every one of them visible — and
// Ember is the more READABLE one on a page this dense, because one hue means
// a column of cells reads as a single gradient instead of eight decisions.
// Denser boards want fewer hues. Signal is one click away and unchanged.
//
// A device that has already CHOSEN a ramp keeps it: this only moves the
// starting point, and overriding a saved choice because I changed my mind
// about the default would be the wrong kind of tidy.
export const DEFAULT_RAMP = 'ember'
...
export const DEFAULT_KNOBS = {
  hueFrom: 2, hueTo: 142, sat: 0.6, brightness: 0.6, satShape: 'arch', stops: 9,
  greyBottom: 0,
  // litNumbers picks the CONSTRUCTION, not a colour: false = bright cell with
  // neutral ink (Ember, Verdict), true = deep tinted cell with a lit number
  // (Signal, and the props sheet it came from). It is the one knob that is a
  // fork rather than a dial.
  litNumbers: false,
}
...
export function setKnobs(next, { persist = true } = {}) {
  const k = { ..._knobs, ...next }
  const { stops, inks } = solveScale(k)
  if (!auditRamp(stops, inks || undefined).ok) return false
  _knobs = k
  RAMPS.custom.stops = stops
  // Deleted rather than set to null, so `r.inks` in inkOn() and edgeOn() is a
  // plain truthiness test and a custom ramp that leaves lit mode goes back to
  // neutral ink cleanly instead of carrying a dead array.
  if (inks) RAMPS.custom.inks = inks
  else delete RAMPS.custom.inks
...
/**
 * What to paint a LEGEND CHIP or a bare dot with.
 *
 * Not the same question as "what fills a table cell". On a lit ramp the fill
 * is deliberately near-black and the number on top carries the colour — right
 * in a table, invisible as an 8px square in a legend with nothing written on
 * it. So anything drawing the ramp without text on it asks for these.
 */
export function activeChips() {
  const r = RAMPS[_current] || RAMPS[DEFAULT_RAMP]
  return r.inks || r.stops
}
 ...
 * to locate on the page. Ember, Verdict and Yours all climb into genuinely
 * bright territory at the good end, so they draw no ring and this returns null
 * for them.
 *
 * SIGNAL IS THE EXCEPTION, and on purpose (2026-08-10). Its whole construction
 * is a deep tinted cell with a lit number, so the cell edge is where the tier
 * actually reads — and the props sheet Donovan pointed at draws exactly this:
 * a faint ring in the cell's own colour. Without it eight dark fills on a near
 * black page have no shape and the table stops looking like a grid.
 */
export function edgeOn(bg) {
  const r = RAMPS[_current]
  if (!r || !r.inks || typeof bg !== 'string') return null
  const i = r.stops.indexOf(bg.toLowerCase())
  if (i < 0) return null
  const [a, b, c] = [1, 3, 5].map((k) => parseInt(r.inks[i].slice(k, k + 2), 16))
  return `rgba(${a},${b},${c},0.30)`
}


// ── ink ─────────────────────────────────────────────────────────────────────
export const INK_DARK = '#0a0a0b'
// #f8f8f8 UNTIL 2026-08-22, and the drift was invisible and real: every
// contrast number in the block comment above, the whole dead-zone derivation
// (0.170-0.189), scripts/check-palette.mjs's `L`, and rampSolver's default
// inkLight were ALL computed against #f4f4f5, while the ink that actually
// painted was #f8f8f8. The contrast that shipped was not the contrast that was
// asserted. Nothing looked wrong — #f8f8f8 is slightly brighter, so on a dark
// fill it read BETTER than the test claimed — but the ink-switch threshold in
// inkOn() sits where the two ratios cross, and that crossing point moves with
// the ink. Aligned to the value everything else in the system already used,
// which is also C.text. scripts/check-scales.mjs now asserts they agree.
export const INK_LIGHT = '#f4f4f5'
...
export function inkOn(bg) {
  ...
  // PAIRED INK, IF THE RAMP SHIPS ONE (2026-08-10).
  //
  // Signal now carries its own `inks` — a lit number on a deep tinted cell,
  // which is the construction Donovan pointed at in the props sheet. The pair
  // is looked up by exact hex, which is safe because rampColor() returns a
  // stop verbatim rather than interpolating between them.
  //
  // I ARGUED AGAINST THIS EXACT MECHANISM in this file and the reasoning was:
  // a ramp with a custom ink can dodge the dead-zone rule and go quietly
  // unreadable. Still true. But the dead zone is a fact about the two NEUTRAL
  // inks — it is the band where neither white nor near-black clears 4.5:1 —
  // and it says nothing about a fill that brings its own. The right fix was
  // never to ban the mechanism; it was to check the thing that matters.
  // scripts/check-palette.mjs now measures every fill against ITS OWN ink and
  // fails the build under 4.5:1, which is a stronger guarantee than the rule
  // it replaces because it holds no matter what colour a ramp chooses.
  const r = RAMPS[_current]
  if (r && r.inks) {
    const i = r.stops.indexOf(bg.toLowerCase())
    if (i >= 0 && r.inks[i]) return r.inks[i]
  }

  // EVERY OTHER RAMP: whichever neutral ink wins on this fill. Computed from
  // the colour rather than from an index, because every time a ramp changed
  // here a hard-coded "switch to dark ink at step N" was left behind.
