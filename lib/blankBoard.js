...
// `num` is the after-a-blank numerator, `base` his normal rate over every
// batted game, `ctl` the clean complement (games following a game he DID get a
// hit in). All three count the same bar over the same PA-gated denominator —
// see compute_blank_profile in the bot, which walks them in one helper for
// exactly that reason.
export const BLANK_MARKETS = [
  { key: 'hit', label: '1+ hit', short: 'HIT', row: 'hit', threshold: 1, num: 'after_blank_hit', base: 'overall_hit', ctl: 'after_hit_hit' },
  { key: 'hrr1', label: '1+ H+R+RBI', short: 'HRR 1+', row: 'hrr', threshold: 1, num: 'after_blank_hrr1', base: 'overall_hrr1', ctl: 'after_hit_hrr1' },
  { key: 'hrr2', label: '2+ H+R+RBI', short: 'HRR 2+', row: 'hrr', threshold: 2, num: 'after_blank_hrr2', base: 'overall_hrr2', ctl: 'after_hit_hrr2' },
  { key: 'tb2', label: '2+ total bases', short: 'TB 2+', row: 'tb2', threshold: 2, num: 'after_blank_tb2', base: 'overall_tb2', ctl: 'after_hit_tb2' },
]

/**
 * Is the bot publishing the control cohorts yet?
 *
 * SEPARATE FROM blankDataPublished, deliberately. The control shipped a day
 * after the board, so the live failure mode is a perfectly good slate from a
 * bot that has after_blank_* but not overall_* / after_hit_*. Without its own
 * guard every baseline would silently read 0 of 0 and the verdict would
 * announce a colossal fake effect. Same lesson as nflMatchupLooksReal.
 */
export function controlPublished(rows = []) {
  return (rows || []).some((p) => num(p?.overall_n, 0) > 0)
}
...
export function blankRows(players = [], odds = null, marketKey = 'hit') {
  ...
  for (const p of players || []) {
    ...
    // His own baselines. `baseRate` gets no MIN_N gate of its own — it is
    // computed off every batted game, so it is always the deeper of the two
    // samples and its floor is already implied by the after-blank one.
    const baseN = num(p?.overall_n, 0)
    const baseCount = num(p?.[m.base], 0)
    const baseRate = baseN > 0 ? (100 * baseCount) / baseN : null
    const ctlN = num(p?.after_hit_n, 0)
    const ctlCount = num(p?.[m.ctl], 0)
    ...
    out.push({
      ...
      // ── THE OTHER COMPARISON (2026-08-16) ───────────────────────────────
      // Everything above measures him against the BOOK. These measure him
      // against HIMSELF, which is the only way to answer whether the blank in
      // the board's name predicts anything at all.
      //
      // `baseRate` is his normal rate over every batted game — the reference
      // tick on the chart. `ctlRate` is the clean complement (games after a
      // game he DID hit in) and is what the board-level test consumes.
      //
      // NOTE THE ABSENCE: there is no per-row "lift" field, on purpose. With
      // ~30 games after a blank against ~120 overall, an individual's
      // difference has an interval wide enough to swallow any real effect —
      // printing it per row would be inviting the reader to read noise as a
      // trend. The lift is only ever stated POOLED, in blankPool, where the
      // denominators are in the thousands.
      baseN,
      baseCount,
      baseRate,
      ctlN,
...
/**
 * ── DOES THE BLANK ACTUALLY PREDICT ANYTHING? ───────────────────────────────
 *
 * 2026-08-16, Donovan: "do the board compare side to the book as well i like
 * that." So: the same rate, against a second reference.
 *
 * Everything else on this board asks "is he mispriced" — his after-a-blank
 * rate against what the sportsbook charges. That is a real question and it is
 * the one he built the board for. But it quietly leaves the board's own NAME
 * unexamined: a hitter reading 68% after a blank looks like a bounce-back
 * story, when 68% might simply be his rate and the blank might be doing
 * nothing at all. The control for that is not a sportsbook. It is him.
 *
 * WHY THIS IS POOLED AND THE PER-ROW VERSION DOES NOT EXIST. One hitter has
 * maybe 30 games after a blank against 90 after a hit. The 95% interval on
 * that difference is roughly ±20 points — wide enough to swallow any effect
 * this size, so a per-row "lift" column would be a noise generator with a
 * heading. Pool all 80 hitters on the board and the denominators land in the
 * thousands, which is where a five-point shift becomes visible. So the board
 * gets ONE verdict, in a sentence, and no row gets a number it cannot support.
 *
 * THE CONTROL IS after_hit, NOT overall. `overall` contains the after-blank
 * games, so testing one against the other tests a group against a superset of
 * itself and attenuates whatever is really there. `after_hit` is the exact
 * complement: same follow-up rule, same bars, same PA-gated denominator,
 * opposite condition.
 *
 * Two-proportion z, unpooled variance, the same test used for the HR-gate
 * finding. Returns null when either arm is too thin to say anything.
 */
export const VERDICT_MIN_N = 200

export function blankLift(rows = []) {
  let bn = 0
  let bk = 0
  let cn = 0
  let ck = 0
  ...
  if (bn < VERDICT_MIN_N || cn < VERDICT_MIN_N) return null
  const p1 = bk / bn
  const p2 = ck / cn
  const se = Math.sqrt((p1 * (1 - p1)) / bn + (p2 * (1 - p2)) / cn)
  const z = se > 0 ? (p1 - p2) / se : 0
  return {
    afterBlank: { n: bn, k: bk, pct: 100 * p1 },
    afterHit: { n: cn, k: ck, pct: 100 * p2 },
    diff: 100 * (p1 - p2),
    z,
    // 1.96 is the 95% two-sided bar. Below it the honest word is "no", not
    // "a small effect" — an undetectable difference is not a small difference,
    // it is an unmeasured one, and this board has been implying the opposite
    // since the day it shipped.
    real: Math.abs(z) >= 1.96,
  }
}
