...
//
// A GAME is one he BATTED in (atBats >= 1). A pinch-run or walk-only night is
// skipped, not counted: it is neither an 0-fer nor a hit, and letting it break
// a streak would make the board's central claim false for exactly the players
// most likely to be on it. A BLANK is such a game with no hits. AFTER A BLANK
// counts every game whose immediately-preceding batted game was a blank.
//
...
/** Did he go hitless in his last game he batted in? Unknown is NOT false. */
export function blankedLastGame(p) {
  if (String(p?.blank_profile_status || '') !== 'ok') return false
  return num(p?.last_game_ab, 0) >= 1 && num(p?.last_game_hits, 0) === 0
}
