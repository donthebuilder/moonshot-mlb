...
export async function pitcherSituational(pid) {
  ...
  if (extra.d && extra.n && f(extra.d?.ops) != null && f(extra.n?.ops) != null) {
    rows.push({
      key: 'dn', label: 'OPS against, day vs night',
      a: f(extra.d.ops), b: f(extra.n.ops), aLabel: 'day', bLabel: 'night', dp: 3,
      worse: f(extra.d.ops) > f(extra.n.ops) ? 'a' : 'b',
    })
  }
  if (extra.i01 && f(extra.i01?.ops) != null) {
    rows.push({
      key: 'i1', label: 'First inning',
      single: `${extra.i01.ops} OPS · ${extra.i01.homeRuns ?? 0} HR against`,
      note: 'The ambush read — some arms bleed before they settle, and the top of the order gets that inning',
    })
  }
  return rows
}
...
// ── batter vs pitcher ────────────────────────────────────────────────────────
// vsPlayer — the actual head-to-head. Verified live 2026-08-06 (Kwan vs
// Scott): returns per-season splits AND a vsPlayerTotal career block, with
// PA/AB/H/HR/BB/K/TB and the slash line. The bot publishes a thinner career
// bvp_* line on the slate row; this is the richer live version of the same
// thing, and it stays context-only for the same reason BvP always should:
// the samples are almost always tiny.
export async function bvpSplits(batterId, pitcherId) {
  if (!batterId || !pitcherId) return null
  const key = `${batterId}:bvp:${pitcherId}`
  if (cache.has(key)) return cache.get(key)
  const url = `${API}/people/${batterId}/stats?stats=vsPlayer&opposingPlayerId=${pitcherId}&group=hitting`
  const p = fetch(url)
    .then((r) => (r.ok ? r.json() : null))
    .then((j) => {
      const blocks = j?.stats || []
      const seasons = (blocks.find((b) => b?.type?.displayName === 'vsPlayer')?.splits || [])
        .filter((s) => s?.stat)
        .map((s) => ({ season: s.season, stat: s.stat }))
      let total = blocks.find((b) => b?.type?.displayName === 'vsPlayerTotal')?.splits?.[0]?.stat || null
      if (!total && seasons.length) {
        // Career block absent when they first met this season — sum it ourselves.
        const sum = (k) => seasons.reduce((a, s) => a + (Number(s.stat[k]) || 0), 0)
        const ab = sum('atBats'), h = sum('hits'), tb = sum('totalBases')
        total = {
          plateAppearances: sum('plateAppearances'), atBats: ab, hits: h,
          homeRuns: sum('homeRuns'), baseOnBalls: sum('baseOnBalls'),
          strikeOuts: sum('strikeOuts'), totalBases: tb, rbi: sum('rbi'),
          avg: ab ? (h / ab).toFixed(3).replace(/^0/, '') : '—',
          slg: ab ? (tb / ab).toFixed(3).replace(/^0/, '') : '—',
        }
      }
      return seasons.length || total ? { seasons, total } : null
    })
    .catch(() => null)
  cache.set(key, p)
  return p
...
// ── strike-zone hot/cold map ─────────────────────────────────────────────────
// hotColdZones — per-zone exit velocity, SLG, OPS, AVG, OBP for the 13 Gameday
// zones (1–9 in-zone, 11–14 the out-of-zone quadrants). Verified live
// 2026-08-05 (Judge, 660271): the API returns value + its own temp label per
// zone. This is the EV log mapped onto the plate — same lane, context only.
export async function hotColdZones(pid, group = 'hitting') {
  if (!pid) return null
  const key = `${pid}:zones:${group}`
  if (cache.has(key)) return cache.get(key)
  const url = `${API}/people/${pid}/stats?stats=hotColdZones&group=${group}&season=${season()}`
  // For pitchers the same endpoint returns SLG/AVG/OPS *against* per zone,
  // plus raw pitch counts per zone — usage, from the API (verified live
  // 2026-08-06). numberOfPitches carries no temp, just values.
  const NAME_MAP = {
    exitVelocity: 'ev', sluggingPercentage: 'slg', onBasePlusSlugging: 'ops',
    battingAverage: 'avg', onBasePercentage: 'obp', numberOfPitches: 'pitches',
  }
  const p = fetch(url)
    .then((r) => (r.ok ? r.json() : null))
    .then((j) => {
      ...
      const out = {}
      splits.forEach((s) => {
        const short = NAME_MAP[s?.stat?.name]
        if (!short) return
        const zones = {}
        ;(s.stat.zones || []).forEach((z) => {
          zones[String(z.zone)] = { temp: z.temp, value: z.value }
        })
        out[short] = zones
      })
...
export async function batterSituational(pid) {
  ...
  if (s.vgb && s.vfb && f(s.vgb?.slg) != null && f(s.vfb?.slg) != null) {
    rows.push({
      key: 'gbfb', label: 'SLG vs ground-ball vs fly-ball pitchers',
      a: f(s.vgb.slg), b: f(s.vfb.slg), aLabel: 'GB arms', bLabel: 'FB arms', dp: 3,
      note: 'Tonight’s starter has a batted-ball identity — this is how he handles each kind',
      good: f(s.vfb.slg) >= f(s.vgb.slg) ? 'b' : 'a',
    })
  }
  return rows
}

// ── THE FULL SPLIT BOARD (2026-08-29) ────────────────────────────────────────
//
// Donovan, on the pitcher split pickers: "where are ALL of them. not just
// four." The bot publishes hand splits and (when its situational job has
// run) seven situational cuts — and when it hasn't, the picker offered vs
// LHB / vs RHB / Overall and stopped. The API has had every one of these
// all along; this fetches them in ONE batched call per pitcher (statSplits
// takes a comma list) and returns a keyed board the pickers can always
// offer, tagged live so nobody reads it as bot-validated.
//
// Every value comes straight off the API's own stat object — nothing is
// derived except HR/9 when the API omits homeRunsPer9 (then it is plain
// arithmetic on homeRuns and inningsPitched, the same identity the stat
// means). inningsPitched arrives as "123.1" meaning 123⅓ — parsed, not
...
const ip = (v) => {
  ...
  const outs = s.endsWith('.1') ? 1 / 3 : s.endsWith('.2') ? 2 / 3 : 0
  return whole + outs
}

export const PITCHER_SPLIT_BOARD = [
  ['h',     'Home',    'his numbers at home'],
  ['a',     'Road',    'his numbers on the road'],
  ['d',     'Day',     'day games'],
  ['n',     'Night',   'night games'],
  ['risp',  'RISP',    'with runners in scoring position'],
  ['ac',    'Ahead',   'when ahead in the count'],
  ['bc',    'Behind',  'when behind in the count — the blowup count state'],
...
export async function pitcherSplitBoard(pid) {
  if (!pid) return {}
  const codes = PITCHER_SPLIT_BOARD.map(([k]) => k).join(',')
  const s = await fetchSplits(pid, 'pitching', codes, season())
  const out = {}
  PITCHER_SPLIT_BOARD.forEach(([code]) => {
    const st = s[code]
    if (!st) return
    const innings = ip(st.inningsPitched)
    const hr = f(st.homeRuns)
    const hr9 = f(st.homeRunsPer9) ?? (innings > 0 && hr != null ? (9 * hr) / innings : null)
    const row = {
      hr9,
      whip: f(st.whip),
      era: f(st.era),
      ...
      hr,
      ip: innings,
      bf: f(st.battersFaced),
    }
    if (Object.values(row).some((v) => v != null)) out[code] = row
  })
  return out
}
