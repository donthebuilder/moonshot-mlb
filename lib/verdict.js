...
// Exported 2026-08-31: the shortlist needs the family to colour an archetype
// chip on the site's own role palette. It was private and called three times
// inside this file; a second local copy in a component is exactly how one hue
// comes to mean two things (see check-scales.mjs).
export const marketFamily = (role) => {
...
export function hitterArchetype(r, role = primaryRole(r) || 'NONE') {
  ...
  // ── THIS READ A FIELD THAT DOES NOT EXIST (fixed 2026-09-19) ────────────
  // `air_pull_rate` and `recent_air_pull_rate` are both absent at the top
  // level of a slate row -- measured on the live 270-hitter slate, 0 of 270
  // carry either. So rate() saw undefined, n() defaulted it to 0, and the
  // `airPull >= .24` branch below has been false for every hitter every night
  // since it was written. Pull-Side Threat could only ever be reached through
  // its own fallback clause.
  //
  // The number it wants IS published, twice, under a different word order:
  // `recent_pull_air_rate` at the top level (270/270) and
  // `bbe_profile.pull_air_rate` nested. Both are the share of batted balls
  // that were pulled AND in the air.
  //
  // NOT `bbe_profile.air_pull_rate`, which is a real field with a DIFFERENT
  // denominator -- the pulled share of AIR balls only. At this threshold that
  // one fires for 258 of 270 hitters, so wiring it here would have turned
  // Pull-Side Threat into the default label for 96% of the board. The
  // pull-air reading fires for 152, and only as the fourth branch of an
  // ordered cascade. The sibling threshold in lib/scoring_additions.js is
  // named 'pitcher: pull-air prone' at 0.26, which settles which quantity
  // both were always meant to read.
  const airPull = rate(r?.recent_pull_air_rate ?? r?.bbe_profile?.pull_air_rate)
  ...
  if (family === 'HR') {
    ...
    // THRESHOLD RAISED .24 -> .33 IN THE SAME PASS, and it has to be, because
    // .24 was never tested against anything: the field it gated on did not
    // exist, so the branch never ran. Measured on the live 270-hitter slate,
    // .24 sits BELOW the median pull-air rate (.250) -- switching the field on
    // at the old number made Pull-Side Threat the single biggest label on the
    // board, 114 of 270, swallowing 60 hitters from Standard Power. A label
    // that describes 42% of a slate is not describing a shape.
    // .33 is the 75th percentile of that live distribution -- 75 of 270 clear
    // it, the same order of selectivity as the branches above (Laser 27,
    // Matchup Hunter 44), and as the fourth branch of an ordered cascade it
    // sees far fewer than that. Chosen as a quartile of real data, not tuned
    // to taste; revisit it against a full season rather than one slate.
    if (airPull >= .33 || (pull >= .47 && fly >= .32)) {
