...
// ══ STREAKS, AND WHERE THEY BROKE ═══════════════════════════════════════════
//
// Donovan, 2026-08-23: "keep making better adds breask in streaks and als add
// streask to palayer model too with toggles on the props grid or somehting."
//
// Everything on this site that says "streak" says ONE number: the current run,
// positive for clears and negative for misses (thresholdRates above, the
// matrix's `stk`, the ⚡/🥶 pattern). That number answers "is he hot right
// now" and refuses every other question a streak raises:
//
//   · Is 4 straight a lot FOR HIM? (his best run this season says)
//   · How long do his cold runs usually last? (his longest drought says)
//   · Did the streak that just ended die at 3, or at 11? (the run before it)
//   · WHERE did it break — who was pitching, home or away, what date?
//
// A streak is a sequence of runs, so this returns the sequence. Newest first,
// each run carrying its length, whether it cleared, the games it spans, and —
// the ask — THE BREAK: the single game that ended the run before it, which is
// the first game of THIS run whenever the two disagree.
//
// Pure function of an already-fetched log. No new call, no cache, and it takes
// the test as an argument so it works at any line the grid is set to, not just
// the 1+ defaults MARKETS bakes in.
export function streakRuns(games, test) {
  const gs = Array.isArray(games) ? games.filter(Boolean) : []
  if (!gs.length || typeof test !== 'function') return null
  const runs = []
  for (const g of gs) {
    const ok = !!test(g)
    const head = runs[runs.length - 1]
    if (head && head.ok === ok) { head.len++; head.games.push(g) }
    else runs.push({ ok, len: 1, games: [g] })
  }
  // `broke` is the game that ENDED the previous (older) run. The log is
  // newest-first, so run i is newer than run i+1, and the game that broke run
  // i+1 is the OLDEST game of run i — the first one that went the other way,
  // chronologically. Getting this backwards yields a break date that reads
  // perfectly and points at the wrong night.
  runs.forEach((r) => { r.broke = r.games[r.games.length - 1] || null })
  const clears = runs.filter((r) => r.ok)
  const misses = runs.filter((r) => !r.ok)
  return {
    runs,
    current: runs[0] || null,
    best: clears.length ? Math.max(...clears.map((r) => r.len)) : 0,
    drought: misses.length ? Math.max(...misses.map((r) => r.len)) : 0,
    // The run that just ended — the one the current streak broke. Null on the
    // first run of the season, because there is nothing behind it to have
    // broken, and printing "broke a 0-game streak" would be a fabricated fact.
    ended: runs[1] || null,
    games: gs.length,
  }
}
