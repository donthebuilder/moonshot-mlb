...
export function sprayAngle(hcX, hcY) {
...
export async function savantBattedBalls(pid) {
  ...
  try {
    ...
    // ── STRIKEOUTS, SAME EXPORT (2026-09-08) ────────────────────────────────
    // This CSV is every pitch, not just balls in play — a strikeout's final
    // pitch is sitting right there with `events` set and `type` !== 'X'.
    // Same treatment bots/mlb_dashboard.py gives the bot's own cache: pull it
    // in as a bbe-shaped row tagged is_k, every batted-ball-only field left
    // null rather than invented, pitch/arm/velo/pitcher taken from that same
    // final pitch. Keeps a non-slate player's K rate and K rows working the
    // same as anyone the bot has cached.
    const kRaw = grid.slice(1).filter((row) => /^strikeout/i.test(col(row, 'events')))
    const pids = [...new Set([...raw, ...kRaw].map((row) => col(row, 'pitcher')).filter(Boolean))]
    ...
    const kRows = kRaw.map((row) => ({
      ...
      result: String(col(row, 'events')).replace(/_/g, ' '),
      bb_type: '',
      trajectory: '',
      stand: String(col(row, 'stand')).toUpperCase(),
      is_pull_air: 0,
      zone: parseInt(col(row, 'zone'), 10) || null,
      is_k: 1,
    }))

    const combined = [...rows, ...kRows].sort((a, b) => String(b.date).localeCompare(String(a.date)))
...
// ── PITCHER HEAT MAP: BALLS PUT IN PLAY AGAINST HIM (2026-09-21) ───────────
//
// Same CSV, same player_type=pitcher query savantPitcherArsenal already
// proved works, filtered to `type === 'X'` (ball in play) instead of
// aggregated into pitch-type usage. Every row already carries hc_x/hc_y —
// SprayField's own hitter version reads those off the identical column
// names — so classifying a ball hit off this pitcher into pull/center/oppo
// and a distance tier is the exact sprayAngle() math two functions up,
// unchanged. Nothing here is modelled: a zone with no batted balls in it
// stays empty rather than being filled in with a league number this file
// never fetched.
export async function savantPitcherBattedBalls(pid) {
  if (!pid) return []
  const key = `pbb:${pid}`
  if (_cache.has(key)) return _cache.get(key)
  try {
    ...
    const raw = grid.slice(1).filter((row) => col(row, 'type') === 'X')
    const batterIds = [...new Set(raw.map((row) => col(row, 'batter')).filter(Boolean))]
    const names = await pitcherNames(batterIds)   // same people lookup, any MLBAM id
    ...
    const rows = raw.map((row) => {
      ...
      const dist = parseFloat(col(row, 'hit_distance_sc'))
      const events = String(col(row, 'events'))
      // Same three-way split SprayField's hitter chips use (Pull/Centre/Oppo),
      // relative to the BATTER's own stand -- pull side means something
      // different for a lefty and a righty even off the same pitcher.
      const side = ang == null ? '' : (
        Math.abs(ang) <= 12 ? 'center' : (pulled ? 'pull' : 'oppo')
      )
      const lane = ang == null ? '' : (
