...
//   BOARD   per-game top-8 by hr_score, ~110 names      ->  ~59% of all HR
...
/** Map player_id -> rank inside HIS game by hr_score (1 = that game's top bat). */
export function gameRank(players) {
  ...
  for (const list of byGame.values()) {
    list.sort((a, b) => (hrScore(b) - hrScore(a)) || ((mlbId(a) || 0) - (mlbId(b) || 0)))
