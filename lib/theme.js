...
export function applyTheme(key) {
  ...
  try {
    ...
    // TUDDY reads its own C (lib/nfl/theme.js) -- same greys by design, a
    // different accent family. Until 2026-09-05 nothing ever repainted that
    // object, so picking Light on MOONSHOT and switching sport gave a pale
    // body under dark football cards. The greys follow the palette always;
    // the accents follow only for light, where the emerald/cyan brand pair
    // does not clear contrast on white and the palette's own do.
    try {
      const nfl = require('./nfl/theme').C
      const GREYS = ['bg', 'bg2', 'bg3', 'glass', 'border', 'border2', 'text', 'text2', 'text3', 'scrim', 'shadow']
      for (const k of GREYS) if (theme.C[k] !== undefined) nfl[k] = theme.C[k]
      if (key === 'light') for (const k of ['green', 'cyan', 'red', 'yellow', 'purple', 'blue', 'orange']) if (theme.C[k]) nfl[k] = theme.C[k]
    } catch { /* the NFL palette is optional here */ }
...
// ── THE TYPE SCALE (2026-09-14, MOONSHOT batch 4) ───────────────────────────
// Before this: 3,444 hardcoded fontSize numbers across components/ and lib/,
// no shared scale, values scattered across a long tail (7, 7.5, 8, 8.5, 9,
// 9.5, 10...) with no consistent meaning from one component to the next.
// Worse: dozens of those sat BELOW a comfortable phone floor.
//
// TYPE is six named steps, not a free scale -- every new fontSize should be
// one of these, not a fresh number. `micro` (10) is the hard floor: nothing
// on the site should render smaller than that on a phone. Pick by ROLE, not
// by "what looks about the same size today":
//   display   hero numbers / big score callouts (the one number on a card)
//   title     section and page headers
//   name      player/team names, list-item and card headlines
//   body      running text, descriptions, card copy
//   label     uppercase kickers, tags, column headers -- small, but tracked
//             wide enough that 10.5 reads fine where 8.5 stopped being legible
//   micro     dense table cells, fine print, timestamps -- the floor itself
//
// Rollout is deliberately staged: Home.js is the pilot (MOONSHOT batch 4,
// this commit). The other ~3,364 call sites are unmigrated and still use
// their old literal numbers -- don't assume TYPE is in force anywhere it
// hasn't been applied file-by-file. See STATUS.md for what's done vs. not.
export const TYPE = {
  display: 28,
  title: 17,
  name: 14,
...
export const TABS = [
  ...
  //
  // ── THIS WAS THE FOURTH LABEL LIST, AND NOTHING READS IT (2026-09-03) ─────
  // Found while wiring Header.js and MobileTabBar.js to lib/routes.js: this
  // array is exported and imported by nobody. It is how the drift happened --
  // four copies of the same list, three of them maintained. It now derives
  // from MLB_NAV so it cannot state a name that is false, and it is a
  // deletion candidate the next time this file is open.
  ...Object.entries(MLB_NAV).map(([k, v]) => [k, `${v.icon} ${v.label}`]),
]
