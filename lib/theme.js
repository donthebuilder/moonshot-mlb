...
// ── THE GROUPED NAV (2026-08-15) ─────────────────────────────────────────────
//
// Donovan: "theres too many tabs now. we need to start making pages full
// pages and condensing the number of main tabs." Twenty tabs became five
// groups. Every key below must exist in TABS — Dashboard still renders every
// key, so old deep links (#tab=boxes) keep working even though the folded
// pages no longer sit in the top row: Boxes lives inside Games, Patterns
// inside Boards, Pair History inside Pairs, True Price inside Results, each
// as a view pill on its host — his four folds, verbatim.
export const NAV = [
  { label: 'Tonight', keys: [
    ['home', 'Home'], ['games', 'Games'], ['atplate', 'At the Plate'], ['scoreboard', 'Scoreboard'],
  ] },
  { label: 'Boards', keys: [
    ['board', 'Boards'], ['longest', 'Power'], ['leaders', 'Leaders'], ['player', 'Player'], ['pitchers', 'Pitchers'],
  ] },
  { label: 'Picks', keys: [
    ['bot', 'Picks'], ['mypicks', 'My Picks'], ['pairs', 'Pairs'], ['pools', 'Pools'], ['watch', 'Watchlist'],
  ] },
  { label: 'Ledger', keys: [
    ['results', 'Results'], ['guide', 'Guide'],
  ] },
]

// Which group a folded page's KEY belongs to, so a deep link into one still
// lights up the right group in the header.
export const NAV_HOME = {
  boxes: 'Tonight', runs: 'Boards', pairhist: 'Picks', trueprice: 'Ledger',
  derby: 'Boards', hitshrr: 'Boards', due: 'Boards',
}
...
export const TABS = [
  ...
  ['boxes',       'Boxes'],
  // 🔥 Patterns (2026-08-15) — active streaks at a bar you pick, with the
  // strip. Hot and cold on the same board, because a drought is a position
  // too. Named by Donovan; the props grid's own section gave the word up.
  ['runs',        'Patterns'],
