...
// linescore,outs,offense,first,second,third added 2026-08-14 (Donovan:
// "i dont see the outs") — fields= is a WHITELIST (see lib/liveSlate.js's
// header for the bug that rule caused once already), so the feed's own
// liveData.linescore block was being stripped entirely. This is the fresher
// source for the one game on screen (15s poll vs the schedule's 25s).
// rbi,awayScore,homeScore added 2026-08-14 for the game timeline — per-play
// RBI and the score after each play, off result. Same whitelist rule as ever.
import { catColor } from './scales'

export const LIVE_FEED_FIELDS = 'liveData,plays,allPlays,playEvents,isPitch,pitchData,coordinates,pX,pZ,strikeZoneTop,strikeZoneBottom,startSpeed,details,type,code,description,call,isInPlay,isStrike,isBall,hitData,launchSpeed,launchAngle,totalDistance,coordX,coordY,trajectory,matchup,batter,pitcher,id,fullName,result,event,about,halfInning,inning,gameData,teams,home,away,abbreviation,linescore,outs,offense,first,second,third,rbi,awayScore,homeScore'

// The plate is 17 inches wide, so the zone runs ±0.708 ft from centre.
export const PLATE_HALF = 0.708

// ── PITCH COLOUR — ONE SOURCE, 2026-08-31 ───────────────────────────────────
//
// This file used to hold its own PITCH_COLORS table of raw hexes. It was the
// last of the three the colour audit found, and it DISAGREED with the one the
// site's own checker polices: a changeup was green here and blue in CAT.pitch,
// a cutter amber here and green there, a 4-seam red here and orange there.
// Both were shipping, on the same page, in different components.
//
// Donovan picked CAT.pitch, and it is the right pick for a reason worth
// writing down: CAT holds TOKEN NAMES, not hexes, so pitch colour follows the
// theme toggle like every other colour on the site. A table of raw hexes
// cannot — it looks correct on the day it is written and silently stops
// matching the first time a token moves. scripts/check-scales.mjs asserts
// CAT.pitch is single-valued; nothing asserted that about this table.
//
// WHAT VISIBLY CHANGED: 4-seam red → orange, sinker orange → yellow, cutter
// amber → green, changeup and splitter green → blue. Slider, sweeper, curve
// and knuckle-curve are unchanged.
//
// PITCH_COLORS is kept as an export because callers import it, but it is now
// DERIVED. There is no second place to edit.
const PITCH_CODES = [
  'FF', 'FA', 'SI', 'FT', 'FC', 'SL', 'ST', 'SV',
  'CU', 'KC', 'CS', 'CH', 'FS', 'FO', 'KN', 'EP',
]

// LIVE, not a snapshot. lib/theme.js switches themes with Object.assign(C,
// theme.C) — it MUTATES C in place — so a table built once at module load
// freezes whatever the theme was on first import and silently stops matching
// after the first toggle. That is precisely the failure this change exists to
// remove, so it would have been an embarrassing way to reintroduce it.
// Each key is a getter; every read asks catColor fresh.
export const PITCH_COLORS = Object.defineProperties({}, Object.fromEntries(
  PITCH_CODES.map((c) => [c, { enumerable: true, get: () => catColor('pitch', c) }]),
))

// An unknown code falls through catColor to C.text3, which is the same grey
// the old table used for eephus and knuckleball — the fallback is unchanged.
export const pitchColor = (code) => catColor('pitch', code)
...
export function parseLiveGame(data) {
  ...
  plays.forEach((pl, pi) => {
    ...
    if (any) {
      meta.push({
        ...who,
        event,
        // Timeline fields (2026-08-14, Donovan: "i need like a timeline of
        // what happened in the game"): each play's RBI count and the score
        // AFTER the play, straight off the feed's own result block — so a
        // scoring-plays timeline needs no arithmetic and can never disagree
        // with the scoreboard.
        rbi: num(pl?.result?.rbi) || 0,
        as: num(pl?.result?.awayScore),
        hs: num(pl?.result?.homeScore),
      })
    }
  })
  const gt = data?.gameData?.teams || {}
  // The situation (2026-08-14): outs + runners straight off the feed's own
  // linescore — fresher than the schedule snapshot for the game on screen.
  // Same null discipline as everywhere else: outs is a number only when the
  // linescore actually came through, and the runner fields carry the
  // runner's NAME when a base is occupied ('' when empty), so a tooltip can
  // say who's standing there without another lookup.
  ...
  return {
    ...
    outs: ls?.outs ?? null,
    on1: ls?.offense?.first?.fullName || (ls?.offense?.first ? '·' : ''),
    on2: ls?.offense?.second?.fullName || (ls?.offense?.second ? '·' : ''),
    on3: ls?.offense?.third?.fullName || (ls?.offense?.third ? '·' : ''),
  }
}
...
export async function fetchBattedBallLog(snap, limit = 12) {
  ...
  // 2026-08-13, Donovan, on the live page: "i dont like this format" — sent
  // right after this shipped, with a screenshot showing a 103mph groundout
  // that traveled 17ft sitting at #1, above a 381ft double. Sorting on raw
  // EV alone did that: a grounder keeps almost all its exit velo because it
  // barely leaves the bat before it's on the ground, so a routine comebacker
  // can out-EV a real double or a ball that rattled the wall. RANKED BY TIER
  // FIRST NOW — home run, then barrel/deep-fly-out/extra-base-hit (contact
  // that's notable independent of the raw number), then everything else that
  // still cleared a gate — EV only breaks ties INSIDE a tier. A double at 98
  // now outranks a 103mph roller no matter what; two barrels still sort
  // hardest-first against each other.
  const tierOf = (r) => (r.tone === 'hr' ? 0 : (r.barrel || r.deepFly || r.tone === 'xbh') ? 1 : 2)
  out.sort((a, b) => (tierOf(a) - tierOf(b)) || (b.ev - a.ev) || ((b.dist || 0) - (a.dist || 0)) || (b.inning - a.inning) || (b.pi - a.pi))
  return out.slice(0, limit)
}
