...
// ── 🔥 HARDEST HIT — every hard-hit ball on today's slate, not just yours ──
//
// 2026-08-13, Donovan: "add like a rolling ev log of the hard hits of the
// live games and todays games." A different question than fetchSkinEvents
// above: that one asks "what happened to MY guys," scoped tight on purpose
// because paying for strangers' at-bats was the whole cost problem. Here,
// strangers squaring up the ball is the entire point, so this deliberately
// takes the "most" tier from the note above (every live game, 8-15 calls a
// refresh) instead of "MIDDLE" — and adds FINAL games too, so a ball hit
// hard in a game that already wrapped tonight doesn't vanish from the log,
// it just stops updating. Shares feedFor()'s per-game cache, so any game
// the skin feed already pulled this refresh costs nothing extra here.
export async function fetchHardHitLog(snap, limit = 12) {
  ...
  // HARDEST FIRST. This is a leaderboard of contact quality, not a timeline
  // of what just happened — that question already has an answer above.
  out.sort((a, b) => (b.ev - a.ev) || (b.inning - a.inning) || (b.pi - a.pi))
  return out.slice(0, limit)
}
