...
export const MLB_MORE_GROUPS = [
  ['Boards',  ['board', 'power', 'steals', 'gap', 'spray']],
...
export const NFL_NAV = {
  // the five the rail shows. `home` left it for the same reason MOONSHOT's
  // did: the TUDDY wordmark is the home button now.
  // TOUCHDOWNS leads the rail, and `boards` moved to the drawer to make room
  // rather than growing the bar to six. Donovan, 2026-09-13: the product "has
  // to help people and me find touchdowns just like for home runs" — and the
  // MLB side leads with its HR board, not with a seven-market shell that has
  // home runs as one tab inside it. The other six markets are one tap away.
  touchdowns:     { label: 'Touchdowns',     icon: '\u{1F3C8}', blurb: 'Who scores this week, and why' },
  boards:         { label: 'Boards',         icon: '\u{1F4CA}', blurb: 'Ranked boards across the seven markets' },
  games:          { label: 'Games',          icon: '\u{1F3C8}', blurb: "Every game, its script, and the model's read on it" },
...
export const NFL_MORE_GROUPS = [
  ['Sunday',   ['live', 'boxscores']],
  ['Research', ['boards', 'matchups', 'pairs', 'streaks', 'leaders']],
  ['Players',  ['players', 'watchlist']],
  ['Yours',    ['accountability', 'report', 'tuddyledger']],
  ['Help',     ['guide']],
]
