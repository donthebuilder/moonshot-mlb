...
// ── THE BOARDS (2026-09-02). Donovan, asked what goes on the fascias:
//    "DASH logos or MOONSHOT ... or sign up for FRANCHISE, TUDDY, all
//    that." So the park advertises the network it belongs to — the three
//    products in their own colours from NetworkSwitch, and the DN mark —
//    and nothing invented. Each board is a canvas: dark ground, the word,
//    a rule, the tag line. Planes, one per ~9° of the lower fascia, facing
//    the plate, cut against the open sectors like everything on the bowl.
const BOARDS = [
  { text: 'MOONSHOT', tag: 'MLB · EVERY PICK GRADED IN PUBLIC', color: '#f97316' },
  { text: 'TUDDY', tag: 'NFL · THE SAME DNA', color: '#22c55e' },
  { text: 'FRANCHISE', tag: 'SIGN UP · FANTASY FOOTBALL', color: '#ff633e' },
  { text: 'DASH NETWORK', tag: 'ONE NETWORK · THREE WAYS TO PLAY', color: '#f4f4f5', logo: true },
]
function boardTex(b, logoImg) {
  const cv = document.createElement('canvas')
  ...
  let x0 = 28
  if (b.logo && logoImg) { g.drawImage(logoImg, 22, 16, 96, 96); x0 = 136 }
  g.font = '900 62px SF Mono, Menlo, monospace'
  g.textBaseline = 'middle'
  g.fillStyle = b.color
  g.fillText(b.text, x0, 52)
  g.font = '700 20px SF Mono, Menlo, monospace'
  g.fillStyle = 'rgba(244,244,245,.72)'
  g.fillText(b.tag, x0, 96)
  const tex = new THREE.CanvasTexture(cv)
  tex.anisotropy = 4
  return tex
}
...
export function buildPark(scene, { dims, heights, venue, P, wallD, wallH, maxD, SEG }) {
  ...
  const grassMat = (hex) => new THREE.MeshLambertMaterial({ color: hex, map: grassTex, side: THREE.DoubleSide })
  const dirtMat = (hex) => new THREE.MeshLambertMaterial({ color: hex, map: dirtTex, side: THREE.DoubleSide })
  ...
  //    -45°..45°-about-+Z wedge is θ 45°..135°.
  {
    ...
    // THE MOW (design pass, 2026-09-02). Rings alone read as a target; a
    // real outfield is cut in a checker — rings crossed by lanes from the
    // plate — and it is the single strongest "this is a ballpark" cue on
    // the grass. Every other cell of a 28-ft ring × 10° sector grid is a
    // half-step lighter. Still a band you have to look for; the same
    // midpoint green, only its grain alternates.
    const lightMat = grassMat(lift(0x2c5637))
    const rings = []
    for (let r0 = 30; r0 < minD - 16; r0 += 28) rings.push([r0, Math.min(r0 + 28, minD - 16)])
    rings.forEach(([r0, r1], i) => {
      for (let j = 0; j < 9; j++) {
        if ((i + j) % 2) continue
        const th0 = -3 * Math.PI / 4 + (j * Math.PI) / 18
        const cell = new THREE.Mesh(planarUV(new THREE.RingGeometry(r0, r1, 12, 1, th0, Math.PI / 18)), lightMat)
  ...
  let dressing = null
  {
    ...
    // the boards, riding the fascia's face above the wall top
    {
      const logo = new Image()
      const texes = BOARDS.map((b) => boardTex(b))
      // the DN mark arrives async; redraw its board when it lands
      logo.onload = () => { BOARDS.forEach((b, i) => { if (b.logo) { texes[i].image = boardTex(b, logo).image; texes[i].needsUpdate = true } }) }
      logo.src = '/icon-192.png'
      let k = 0
      for (let a = -66; a <= 66; a += 9) {
        if (isOpenSector(bowl, a - 3) || isOpenSector(bowl, a + 3)) continue
        // a tall wall (the Monster, the Crawford Boxes) is its own fascia
        if (wallH(clamp(a)) > 12) continue
        const b = BOARDS[k++ % BOARDS.length]
        const tex = texes[BOARDS.indexOf(b)]
        const h0 = wallH(clamp(a))
        const w = 2 * Math.PI * (wallD(clamp(a)) + 15.6) * (8 / 360)
        ...
        const v = P(wallD(clamp(a)) + 15.2, a)
        m.position.set(v.x, Math.max(h0, 1) + 3.2, v.z)
