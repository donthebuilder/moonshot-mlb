...
export function buildPark(scene, { dims, heights, venue, P, wallD, wallH, maxD, SEG }) {
  ...
  let dressing = null
  {
    ...
    band(-180, 180, 15.6, 0, 6, new THREE.MeshLambertMaterial({ color: bowl.fascia || 0x1c3a48, side: THREE.DoubleSide }))
    ...
    for (const u of ups) {
      band(u[0], u[1], 115.5, 46, 50, new THREE.MeshLambertMaterial({ color: bowl.fascia || 0x1f2630, side: THREE.DoubleSide }))
      {
      ...
      band(u[0], u[1], 115.4, 58.5, 60.5, new THREE.MeshLambertMaterial({
        color: 0x3a2a12, emissive: 0xc27a2a, emissiveIntensity: 0.38, side: THREE.DoubleSide,
      }))
    }
    ...
    //    lib/parkProps (now three times as many) and come in just below.
    {
      ...
      if (bowl.towers) for (const a of bowl.towers.filter((x) => Math.abs(x) <= 140)) {
        ...
        const rig = new THREE.Mesh(new THREE.PlaneGeometry(30, 9),
          new THREE.MeshBasicMaterial({ color: 0xe8dcc6, transparent: true, opacity: 0.32, fog: false, side: THREE.DoubleSide }))
