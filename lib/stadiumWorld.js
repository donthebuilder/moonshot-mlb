...
export function buildPark(scene, { dims, heights, venue, P, wallD, wallH, maxD, SEG }) {
  ...
  let dressing = null
  {
    ...
    const ups = !bowl.up ? [] : Array.isArray(bowl.up[0]) ? bowl.up : [bowl.up]
    const tint = (hex) => bowl.seat ? new THREE.Color(hex).lerp(new THREE.Color(bowl.seat), 0.55).getHex() : hex
    deck(-72, 72, 16, 92, 6, 46, tint(0x2a323f), tint(0x3a4453), true)
    if (bowl.bleach) deck(bowl.bleach[0], bowl.bleach[1], 14, 58, 4, 26, tint(0x252d38), tint(0x353e4c), true)
    for (const u of ups) deck(u[0], u[1], 116, 96, 62, 112, tint(0x2d3441), tint(0x323a48), true)
    if (bowl.tiers > 2) {
      for (const u of (ups.length ? ups : [[-46, 46]])) deck(u[0], u[1], 220, 84, 128, 172, tint(0x1d232c), tint(0x212832), true)
    }
    ...
    band(-72, 72, 15.6, 0, 6, new THREE.MeshLambertMaterial({ color: bowl.fascia || 0x1c3a48, side: THREE.DoubleSide }))
    band(-72, 72, 15.6, 5.4, 6, new THREE.MeshBasicMaterial({ color: 0x8a5a2a, side: THREE.DoubleSide }))
    for (const u of ups) {
      band(u[0], u[1], 115.5, 46, 62, new THREE.MeshLambertMaterial({ color: bowl.fascia || 0x1f2630, side: THREE.DoubleSide }))
    ...
    //    lib/parkProps (now three times as many) and come in just below.
    {
      ...
      const slab = (a0, a1, off0, off1, y, mat) => {
        ...
        for (let a = a0; a < a1; a += 2.5) {
          const b = Math.min(a1, a + 2.5)
          if (isOpenSector(bowl, a) || isOpenSector(bowl, b)) continue
          const i0 = P(wallD(clamp(a)) + off0, a), i1 = P(wallD(clamp(b)) + off0, b)
          const o0 = P(wallD(clamp(a)) + off1, a), o1 = P(wallD(clamp(b)) + off1, b)
      ...
      for (const u of ups) {
        ...
        for (let a = u[0]; a <= u[1]; a += 7.5) {
          ...
          const v2 = P(wallD(clamp(a)) + 152, a)
          const bank = new THREE.Mesh(new THREE.PlaneGeometry(11, 3.2),
            new THREE.MeshBasicMaterial({ color: 0xf3ead6, transparent: true, opacity: 0.55, fog: false, side: THREE.DoubleSide }))
      ...
      for (const a of [-21, 21]) {
        if (isOpenSector(bowl, a)) continue
        const base = P(wallD(clamp(a)) + 232, a)
        ...
        const rig = new THREE.Mesh(new THREE.PlaneGeometry(34, 10),
          new THREE.MeshBasicMaterial({ color: 0xe8dcc6, transparent: true, opacity: 0.3, fog: false, side: THREE.DoubleSide }))
    ...
    addParkProps(scene, { venue, P, wallD, wallH, steps, extra })
