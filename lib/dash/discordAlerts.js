...
//   DISCORD_LIVE_WEBHOOKS    optional, same list format. LIVE_CATEGORIES below
...
// LIVE CATEGORIES. The ones that only mean something while a game is
// actually being played -- a followed hitter going deep, on deck, clutch, at
// the plate, bases loaded, a lead swinging, extra bases, a cold streak, the
// final score -- as opposed to the pregame four (boardup/lastcall/lineup/
// gameoff/scratched), a one-time roster event (callup/dropout), or next-day
// housekeeping (finalline, slate). These go to DISCORD_LIVE_WEBHOOKS when
// it's set (see the CONFIG comment above); everything else keeps using the
// sport-wide list it always has.
//
// 'homer' HERE IS THE PERSONAL PUSH EVENT (pushRules.js's own "a followed
// hitter goes deep", the one that reaches this phone as a push notification),
// NOT the flagship "Called It" graphic post -- that one is a separate
// pipeline (app/api/dash/homers/tick + lib/dash/xPost.js) with its own
// dedicated DISCORD_HOMER_WEBHOOK and is untouched by anything in this file.
// Donovan confirmed 2026-09-07, from an actual phone notification screenshot
// (on deck / HRR cleared / doubles / goes yard, all from the same feed):
// these belong together as "live action."
const LIVE_CATEGORIES = new Set([
  'homer', 'ondeck', 'clutch', 'atbat', 'slam', 'leadchg', 'xbh', 'bigbases', 'hrr', 'cold', 'gamefinal',
])

function webhooksFor(sport, category) {
  if (sport === 'mlb' && LIVE_CATEGORIES.has(category)) {
    const live = list(process.env.DISCORD_LIVE_WEBHOOKS)
...
// PUSH-ONLY CATEGORIES. Personal enough that they belong on one phone, not in
// front of a whole Discord server -- multihit was the first ask (2026-09-06,
// via the Alert Box Score board): a nightly "he had two hits" is worth having
// as a push and noise for anyone else watching the server. Add a category
// here to keep it off Discord without touching pushRules or the sender.
const PUSH_ONLY = new Set(['multihit'])
...
export async function fanOutToDiscord(events) {
  const worth = (events || []).filter((e) => (e?.sport === 'mlb' || e?.sport === 'nfl') && !PUSH_ONLY.has(e.category))
  if (!discordAlertsOn() || !worth.length) return { sent: 0, failed: 0 }

  let sent = 0
  let failed = 0
  await Promise.all(worth.map(async (e) => {
    const webhooks = webhooksFor(e.sport, e.category)
