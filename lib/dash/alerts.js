...
export const CATEGORIES = [
  ...
  // callup and dropout, added 2026-09-06: `lineup` fires every night a
  // regular starts, which is most nights — Donovan wanted that reserved for
  // someone NEW, and its mirror, someone who's suddenly not there. See
  // pushRules.js's mlbEventsFrom for how each is actually detected.
  { key: 'callup', sport: 'mlb', group: 'Moonshot', scope: 'personal', priority: 1, push: true, live: false,
    label: 'Call-up', detail: 'First time we have seen him in a posted lineup', kinds: [] },
  { key: 'dropout', sport: 'mlb', group: 'Moonshot', scope: 'personal', priority: 1, push: true, live: false,
    label: 'Dropped from the lineup', detail: 'His team is playing and the card is up without him', kinds: [] },
  { key: 'ondeck', sport: 'mlb', group: 'Moonshot', scope: 'personal', priority: 1, push: true, live: false,
    label: 'On deck', detail: 'He is up next — time to go and watch', kinds: [] },
  ...
  { key: 'slate', sport: 'mlb', group: 'Moonshot', scope: 'everyone', priority: 4, push: true, live: false,
    label: 'Any slate homer', detail: 'Anyone goes deep, with where he sat on the board', kinds: ['anyhr'] },
  ...
  { key: 'scratched', sport: 'mlb', group: 'Moonshot', scope: 'personal', priority: 0, push: true, live: false,
    label: 'Scratch', detail: 'A board hitter whose card is posted without him', kinds: [] },

  // ── THE BIG MOMENTS ───────────────────────────────────────────────────────
  // slam is priority 0 with the homer, because it expires in one at-bat and is
  // the most on-topic thing a home-run site can say. The two game-level ones
  // are about the SCORE rather than a player, so they carry no single name and
  // are off by default -- they are the ones most able to turn into noise.
  { key: 'slam', sport: 'mlb', group: 'Moonshot', scope: 'personal', priority: 0, push: true, live: false,
    label: 'Bases loaded, he is up', detail: 'Your hitter at the plate with the bases full', kinds: [] },
  { key: 'leadchg', sport: 'mlb', group: 'Moonshot', scope: 'personal', priority: 2, push: true, live: false,
    // Donovan named this one himself (2026-09-06): "iLive" -- his label for
    // the live score-swing feed, not a generic description of what it does.
    label: 'iLive', detail: '7th on, within three runs, in a game you have somebody in', kinds: [] },
  { key: 'gamefinal', sport: 'mlb', group: 'Moonshot', scope: 'personal', priority: 3, push: true, live: false,
    label: 'Final score', detail: 'How the game ended, once, per game you had somebody in', kinds: [] },
  ...
  { key: 'nflred', sport: 'nfl', group: 'Tuddy', scope: 'personal', priority: 0, push: true, live: false,
    label: 'His offence is in the red zone', detail: 'Your man\u2019s team has the ball inside the twenty', kinds: [] },
  ...
  // priority 0, not 3 (2026-09-06, Alert Box Score): Donovan wants kickoff
  // itself, not held behind anything quieter -- see pushRules.js's producer.
  { key: 'nflkick', sport: 'nfl', group: 'Tuddy', scope: 'personal', priority: 0, push: true, live: false,
  ...
  { key: 'frbye', sport: 'fantasy', group: 'Franchise', scope: 'personal', priority: 0, push: true, live: false,
    label: 'Starting a player on bye', detail: 'A starter has no game this week and cannot score', kinds: [] },
  // Added 2026-09-10. Same shape as the four above -- owned, never a follow
  // list -- but not one-time: these fire all season, so they are not
  // priority-0-because-it-expires like the draft three, they are
  // priority-0/2 for the same reason TUDDY's own nfltd/nflbig are (see
  // lib/dash/franchise.js's starterScoreEventsFrom for the producer).
  { key: 'frtd', sport: 'fantasy', group: 'Franchise', scope: 'personal', priority: 0, push: true, live: true,
    label: 'Your starter scores', detail: 'A player in your starting lineup finds the end zone', kinds: [] },
  { key: 'frbig', sport: 'fantasy', group: 'Franchise', scope: 'personal', priority: 2, push: true, live: false,
    label: 'Your starter has a big day', detail: '100 rushing or receiving, 300 passing', kinds: [] },
]
...
export const PRESETS = [
  {
    // Kept in sync with pushRules.js's own DEFAULTS, which is what a
    // saved-settings-less account actually gets. 2026-09-14 (notification
    // audit): multihit and callup LEFT this set -- multihit was ~36 events a
    // night on a channel already at its throttle ceiling, callup was firing
    // for regulars (fixed, off until a night of ledger proves it). dropout
    // stays: it is the only lineup alert for a follow who is not on the board.
    key: 'essentials', label: 'Essentials', detail: 'The pregame four, a lineup change, homers, slam chances, red zone, kickoffs and touchdowns. About 4-6 a night.',
    on: ['homer', 'slam', 'nfltd', 'nflred', 'nflkick', 'boardup', 'lastcall', 'gameoff', 'scratched',
         'dropout', 'frdraft', 'frclock', 'frauto', 'frlineup', 'frbye', 'frtd'],
  },
  {
    key: 'closely', label: 'Following closely', detail: 'How your guys are doing, without opening the site. Around 10 a night.',
    on: ['homer', 'multihit', 'slam', 'nfltd', 'nflred', 'nflkick', 'boardup', 'lastcall', 'gameoff', 'scratched',
         'callup', 'dropout', 'frdraft', 'frclock', 'frauto', 'frlineup', 'frbye', 'frtd',
         'lineup', 'ondeck', 'xbh', 'bigbases', 'nflbig', 'finalline',
         'leadchg', 'gamefinal', 'frbig'],
  },
  {
