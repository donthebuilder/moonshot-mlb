...
//   3. ONLY THE FIELDS THAT MATTER. The row has 472 of them. Twenty-one are
...
import { slatePaths, runMetaPaths } from '../dataSource'

const FETCH_WINDOW_MS = 5 * 60 * 1000

// Twenty of four hundred and seventy-two.
const KEEP = [
  ...
  // The designation itself (2026-09-05): the homer feed tags every home run
  // with the role the board gave the man. Eighteen of four hundred and seventy-two.
  'game_pick_role',
  // 2026-09-15, for the new "Tonight's Board" role-based post: a HIT or HRR
  // pick has to be ranked by ITS OWN market's score, same house rule as
  // lib/verdict.js's LANES ("A PICK ALWAYS WEARS ITS OWN MARKET'S SCORE") --
  // ranking a HIT-tagged row by hr_score would put a below-average hitter's
  // bat above the board's actual best 1+ hit lean just because his homer odds
  // happen to be decent. Twenty of four hundred and seventy-two.
  'hit_score', 'hrr_score', 'contact_score',
  // 2026-09-15, for the matchup-history trivia posts ("has a HR vs tonight's
  // starter" / "who owns him"): pitcher_name alone can't drive a StatsAPI
  // vsPlayer lookup -- that endpoint takes numeric person ids
  // (opposingPlayerId), and the bot already publishes pitcher_id right next
  // to pitcher_name on every row (confirmed live, 2026-09-15: 607536 for
  // Kyle Freeland). Twenty-one of four hundred and seventy-two.
  'pitcher_id',
]
...
export async function claimBoardWindow(db) {
  ...
  try {
    const { data, error } = await db
      ...
      .select('event_key')
    if (error) { console.error(`[board] window claim failed: ${error.message}`); return false }
...
// This mode's run_meta.json (generated_at/run_id/slate_date/trigger), or
// null when the bot hasn't published one for this mode yet. Written by the
// SAME bot run that writes the slate file (build_run_meta() /
// sync_model_foundation_outputs_to_website_repo() in bots/mlb_dashboard.py),
// so slate_date is an honest answer to "which day does the board I just
// fetched actually belong to" -- see the pregame gate in homers/tick/route.js
// (2026-09-12) for why that question matters and board.size alone couldn't
// answer it.
export async function fetchRunMeta(mode = 'today') {
  for (const url of runMetaPaths(mode)) {
    try {
      const res = await fetch(url, { cache: 'no-store' })
      if (!res.ok) continue
      const body = await res.json()
