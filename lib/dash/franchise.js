...
import { FANTASY_SEASON, resolveFantasyWeek } from '../fantasy/week'
import { fetchNflLive, lineFor } from '../nfl/liveSlate'
...
// -- A STARTER FINDS THE END ZONE --------------------------------------------
//
// frbye's other half: not "he can't score" but "he just did." Reads this
// week's starting lineups -- the same fantasy_lineup_slots table
// byeStarterEventsFrom already reads -- against the live NFL box scores
// TUDDY's own nflEventsFrom watches (lib/nfl/liveSlate.js), and asks one
// question of each row: is this player, right now, STARTING for somebody's
// team.
//
// STARTERS ONLY, same line byeStarterEventsFrom draws. A touchdown from your
// bench is not fantasy points and not news.
//
// MATCHING IS BY NAME, NOT ID. lineFor() is the same normalized-name-plus-team
// join TUDDY's own touchdown alert relies on (there is no gsis<->ESPN
// crosswalk in the live payload) -- reused rather than re-implemented, so
// there is exactly one place on this site that knows how to find a player in
// an ESPN box score. A miss is a player who gets no alert, never the wrong
// player getting one.
//
// OWNED, like every other Franchise event: goes to the team's owner and
// nobody else, follow list not consulted.
//
// RUNS EVERY SWEEP, not gated behind `full` the way the draft/lineup/bye
// producers above are. A touchdown is priority 0 -- the whole reason
// nflEvents() gets swept up to three times a tick instead of once -- and a
// Franchise owner's phone should not lag TUDDY's by forty seconds for the
// same play. This costs one small roster query per sweep (one league's
// current-week starters, at most a few hundred rows) and no second trip to
// ESPN: fetchNflLive() below is called without `force`, so it reads the same
// in-process snapshot nflEvents() already warmed this tick.
export async function starterScoreEventsFrom(db) {
  if (!db) return []
  try {
    const week = await resolveFantasyWeek(db, null)
    if (!week) return []

    const { data: matchups } = await db
      .from('fantasy_matchups')
      .select('league_id,home_team_id,away_team_id')
      .eq('season', FANTASY_SEASON).eq('week', week)
    if (!matchups?.length) return []
    ...
    const [{ data: teams }, { data: slots }] = await Promise.all([
      ...
      db.from('fantasy_lineup_slots')
        .select('team_id,slot,player_id,player:nfl_players(name,team)')
        .eq('season', FANTASY_SEASON).eq('week', week).in('team_id', teamIds),
    ])
    if (!slots?.length) return []

    const teamById = new Map((teams || []).map((t) => [t.id, t]))

    // No force: nflEvents() already pulled this snapshot moments earlier in
    // the same sweep, and asking ESPN again for the same second would be the
    // exact kind of second trip fetchNflLive()'s TTL exists to avoid.
    ...
    for (const s of slots) {
      ...
      const teamName = txt(team.name) || 'your team'
      const url = `${URL_FOR(team.league_id)}/team`

      // Rushing and receiving TDs are the ATD-style score everybody on the
      // roster can have; passing is its own trigger below so a QB's line
      // reads "throws" rather than "scores."
      const rushRecTds = num(line.rushing_tds) + num(line.receiving_tds)
      if (rushRecTds >= 1) {
        out.push(frEvent({
          key: `fr:${team.league_id}:${team.id}:${FANTASY_SEASON}:${week}:td:${s.player_id}:${rushRecTds}`,
          category: 'frtd', priority: 0, owner: team.owner_id,
          title: `\u{1F3C8} ${name} scores${rushRecTds > 1 ? `, ${rushRecTds} today` : ''}`,
          body: `${num(line.receptions)} rec, ${num(line.receiving_yards)} yds${num(line.rushing_yards) ? ` \u{00B7} ${num(line.rushing_yards)} rush` : ''} \u{00B7} ${teamName}`,
          short: `${name}${rushRecTds > 1 ? ` (${rushRecTds})` : ''}`, group: 'scored for you',
          url,
        }))
      }

      const passTds = num(line.passing_tds)
      if (passTds >= 1) {
        out.push(frEvent({
          key: `fr:${team.league_id}:${team.id}:${FANTASY_SEASON}:${week}:passtd:${s.player_id}:${passTds}`,
          category: 'frtd', priority: 0, owner: team.owner_id,
          title: `\u{1F3C8} ${name} throws a TD${passTds > 1 ? `, ${passTds} today` : ''}`,
          body: `${num(line.passing_yards)} pass yds \u{00B7} ${teamName}`,
          short: `${name}${passTds > 1 ? ` (${passTds} TD)` : ''}`, group: 'threw for you',
          url,
        }))
      }

      // Same three thresholds as TUDDY's own nflbig (100 rush/rec, 300 pass),
      // rounded to the nearest 50 in the key for the same reason nflbig's
      // does: a yard-by-yard key would refire every play once the threshold
      // is cleared.
      const big = num(line.receiving_yards) >= 100 ? ['rec', num(line.receiving_yards)]
        : num(line.rushing_yards) >= 100 ? ['rush', num(line.rushing_yards)]
        : num(line.passing_yards) >= 300 ? ['pass', num(line.passing_yards)]
        : null
      if (big) {
        out.push(frEvent({
          key: `fr:${team.league_id}:${team.id}:${FANTASY_SEASON}:${week}:big:${s.player_id}:${big[0]}:${Math.floor(big[1] / 50) * 50}`,
          category: 'frbig', priority: 2, owner: team.owner_id,
          title: `\u{1F4C8} ${name}: ${big[1]} ${big[0]} yds`,
          body: teamName,
          short: `${name} ${big[1]}`, group: 'are having big days for you',
          url,
        }))
      }
    }
