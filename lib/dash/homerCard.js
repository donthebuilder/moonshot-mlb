...
export async function homerCard(row, { site = 'dashnetwork.vercel.app' } = {}) {
  ...
  return frame([
      ...
      <div key="emblem" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', position: 'absolute', right: 40, top: 92, width: 208, padding: '16px 12px 14px 12px', borderRadius: 4, border: `1px solid ${called ? 'rgba(249,115,22,0.55)' : 'rgba(255,255,255,0.14)'}`, background: called ? 'rgba(249,115,22,0.09)' : 'rgba(255,255,255,0.04)' }}>
        ...
        {row?.board_rank ? <span style={{ fontSize: 13, fontWeight: 800, letterSpacing: 1.4, color: INK, marginTop: 6 }}>{`#${row.board_rank} ON THE BOARD`}</span> : null}
      </div>,
      ...
      <div key="id" style={{ display: 'flex', alignItems: 'flex-end', padding: '22px 268px 0 40px', gap: 22 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, paddingBottom: 10 }}>
          ...
          <span style={{ fontSize: 21, fontWeight: 800, color: INK }}>{where}</span>
        </div>
      </div>,
