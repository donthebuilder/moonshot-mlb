...
// ── the headshot (2026-09-18) ──────────────────────────────────────────────
// Donovan: "the one for elly sept ... can be alot better color and more stuff
// kinda like how the home run cards are make its like a show case card."
//
// A face is the single biggest thing separating a showcase card from a stat
// list, and MLB publishes one for every player at a stable URL. Verified live
// 2026-09-18: /v1/people/682829/spots/240 returns a 21KB PNG, 200.
//
// This is the ONE network fetch at render time in this file, and every other
// asset here was deliberately moved to disk to avoid exactly that (see the
// mark and grain loaders above). Two things make it acceptable: it is cached
// per player for the life of the instance, and THE HOT STRETCH posts twice a
// day, so it is at most two fetches a day rather than one per homer. It is
// also hard-timed out and falls back to no photo -- a slow CDN must never cost
// the post, same rule as the mark.
const HEADSHOT_TIMEOUT_MS = 4000
const _headshots = new Map()
async function loadHeadshot(playerId) {
  const id = String(playerId || '').trim()
  if (!/^\d+$/.test(id)) return ''
  if (_headshots.has(id)) return _headshots.get(id)
  let out = ''
  try {
    const ctl = new AbortController()
    const t = setTimeout(() => ctl.abort(), HEADSHOT_TIMEOUT_MS)
    const res = await fetch(`https://midfield.mlbstatic.com/v1/people/${id}/spots/240`, { signal: ctl.signal })
    clearTimeout(t)
    if (res.ok) {
      const buf = Buffer.from(await res.arrayBuffer())
      if (buf.length) out = `data:image/png;base64,${buf.toString('base64')}`
    }
  } catch { /* no photo, card still renders */ }
  _headshots.set(id, out)
  return out
}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column' }}>
      <div style={{ display: 'flex', alignItems: 'center', padding: '24px 40px 0 40px', gap: 14 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column' }}>
          <span style={{ fontSize: 20, fontWeight: 800, letterSpacing: -0.3 }}>CALLED IT</span>
          <span style={{ fontSize: 10.5, fontWeight: 800, letterSpacing: 2.6, color: '#b9b2a8' }}>POWERED BY DASH NETWORK</span>
        </div>
...
export async function homerCard(row, { site = 'dashnetwork.vercel.app' } = {}) {
  ...
  const called = isCalledRole(row?._roles || row?.role) // 2026-09-24: lib/callStatus.js
  ...
  return frame([
      <Header label="HOME RUN" sub={`${prettyDay(String(row?.day || ''))} · every home run, graded in public`} pills={[called && Number(row?.odds_over) >= 700 ? `🎯 LONGSHOT ${fmtOdds(row.odds_over)}` : '', nth, 'TONIGHT'].filter(Boolean)} key="h" />,
      ...
      <div key="id" style={{ display: 'flex', alignItems: 'flex-end', padding: '22px 268px 0 40px', gap: 22 }}>
        <div style={{ display: 'flex', flexDirection: 'column' }}>
          <span style={{ fontSize: 14, fontWeight: 800, letterSpacing: 6, color: ORANGE }}>THE BOT CALLED IT</span>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, paddingBottom: 10 }}>
          ...
          {/* The inning came off (2026-09-07, Donovan: "just remove time of
              hr"). It is on the row and it is in the tweet; on the card it
              was one more thing competing beside a 19-character name for the
              strip between the chip and the emblem. The matchup stays. */}
      ...
      hasStats ? (
        <div key="stats" style={{ display: 'flex', gap: 12, margin: '16px 40px 0 40px' }}>
          ...
          {ball.length ? (
            <div style={{ display: 'flex', flexDirection: 'column', width: 208, border: `1px solid ${RULE}`, background: PANEL, padding: '4px 16px' }}>
              {ball.map((b) => (
...
export async function recapCard(day, rows = [], history = [], { site = 'dashnetwork.vercel.app' } = {}) {
  ...
  const called = rows.filter((r) => isCalledRole(r?.role)).sort((a, b) => (a.board_rank || 999) - (b.board_rank || 999)).slice(0, 8)
  ...
  return frame([
      <Header label="THE NIGHT" sub={`${prettyDay(day)} · every home run, graded in public`} pills={['RECAP']} key="h" />,
      ...
      <div key="body" style={{ display: 'flex', padding: '26px 40px 0 40px', gap: 32 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', flex: 1, position: 'relative' }}>
          ...
          <span style={{ fontSize: 14, fontWeight: 800, letterSpacing: 6, color: ORANGE }}>T H E   B O T   C A L L E D</span>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 14, marginTop: 4 }}>
            <span style={{ fontFamily: DISPLAY, fontSize: 132, fontWeight: 900, color: ORANGE, lineHeight: 1, transform: 'skewX(-8deg)' }}>{String(c.called)}</span>
            <span style={{ fontFamily: DISPLAY, fontSize: 44, color: '#b9b2a8', transform: 'skewX(-8deg)' }}>{`of ${c.total} home runs`}</span>
          </div>
          <span style={{ fontSize: 24, fontWeight: 800, color: INK, marginTop: 8 }}>{`${c.pct ?? 0}% tonight${c.rated ? ` · ${c.rated} more on the board, no call` : ''}${c.off ? ` · ${c.off} off the board` : ''}`}</span>
          <div style={{ display: 'flex', gap: 10, marginTop: 16, flexWrap: 'wrap' }}>
            {roles.map(([r, n]) => (
              <span key={r} style={{ fontSize: 15, fontWeight: 800, letterSpacing: 1.2, color: ORANGE, border: `1px solid ${ORANGE}88`, borderRadius: 999, padding: '5px 14px' }}>{`${roleWord(r).toUpperCase()} ${n}`}</span>
            ))}
          </div>
          {called.length ? (
            <span style={{ fontSize: 17, fontWeight: 800, color: '#b9b2a8', marginTop: 18, lineHeight: 1.4 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', width: 380, padding: '18px 20px', background: PANEL, border: `1px solid ${RULE}` }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
            <span style={{ fontSize: 13, fontWeight: 800, letterSpacing: 1.6 }}>LAST 10 NIGHTS</span>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-end', gap: 8, height: 280, marginTop: 14 }}>
            {days.map((d) => (
              <div key={d.day} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flex: 1, height: '100%', justifyContent: 'flex-end' }}>
                <span style={{ fontSize: 12, fontWeight: 800, color: '#b9b2a8', marginBottom: 4 }}>{d.total ? `${d.pct}%` : '—'}</span>
                {d.day === day ? <div style={{ width: 6, height: 6, borderRadius: 3, background: ORANGE, display: 'flex', marginBottom: 4 }} /> : null}
                <div style={{ display: 'flex', width: '100%', height: `${Math.max(2, d.pct || 0)}%`, background: d.day === day ? ORANGE : 'rgba(244,88,31,0.38)' }} />
                <span style={{ fontSize: 10, color: d.day === day ? INK : '#b9b2a8', fontWeight: d.day === day ? 800 : 400, marginTop: 6 }}>{d.day.slice(5).replace('-', '/')}</span>
              </div>
            ))}
          </div>
        </div>
...
export async function pregameCard(day, picks = [], { site = 'dashnetwork.vercel.app' } = {}) {
  ...
  // 712, not 760 (2026-09-07). The rows lost their pill padding in the
  // style pass and ten of them no longer fill a 760px sheet -- the footer
  // pins to the bottom, so the difference showed up as dead field.
  const ROWS_HEIGHT = 712
  return frame([
    <Header label="TONIGHT'S CALLS" sub={`${prettyDay(day)} · posted before first pitch`} pills={['PREGAME']} key="h" />,

    <div key="body" style={{ display: 'flex', flexDirection: 'column', padding: '18px 40px 0 40px' }}>
      <span style={{ fontSize: 14, fontWeight: 800, letterSpacing: 6, color: ORANGE }}>THE BOT’S HR CALLS</span>
      <div style={{ display: 'flex', flexDirection: 'column', marginTop: 8 }}>
        {list.map((p, i) => {
          const col = teamColor(p.team)
          const price = p.odds_over && p.odds_book ? `${fmtOdds(p.odds_over)} · ${p.odds_book}` : ''
          // 2026-09-18 (Donovan: "make sure we know pitcher hr/9 recent"). On a
          // night with no published price -- which is every row of tonight's
          // board -- this column rendered a bare em-dash ten times down the
          // card: a whole column saying nothing. The arm's HR/9 over his last
          // three starts is on every row (verified 270/270), is the number this
          // list is implicitly about, and is never empty. Price still wins when
          // there is one; the dash is gone either way.
          const armL3 = fin(p.pitcher_l3_hr9) ? `${Number(p.pitcher_l3_hr9).toFixed(2)} HR/9 L3` : ''
          const podium = i < 3
          return (
            <div key={p.player_id || i} style={{
              ...
              <div style={{
                ...
                borderRadius: 4, fontFamily: DISPLAY, fontSize: podium ? 26 : 24, fontWeight: 900,
                color: podium ? '#0a0908' : ORANGE, transform: 'skewX(-8deg)',
                background: podium ? ORANGE : 'rgba(244,88,31,0.12)',
              }}>{String(i + 1)}</div>
              ...
              <span style={{ fontSize: 16, fontWeight: 800, color: '#b9b2a8', width: 290, whiteSpace: 'nowrap' }}>{p.opponent ? `vs ${p.opponent}${p.pitcher ? ` · ${p.pitcher}` : ''}` : ''}</span>
              <span style={{ marginLeft: 'auto', fontSize: 16, fontWeight: 800, color: '#b9b2a8', whiteSpace: 'nowrap' }}>{p.hr_score != null ? `MOONSHOT Score ${Math.round(Number(p.hr_score))}` : ''}</span>
              {/* 150px could not hold '+680 · DraftKings' at 17px bold -- the
                  text ran to the border on every four-digit price and the box
                  sat hard against the card edge. 196 and 16px fits the widest
                  book name in the feed with room, and marginLeft keeps it off
                  the score. */}
              <span style={{
                width: 196, marginLeft: 18, justifyContent: 'center', display: 'flex', fontSize: 16, fontWeight: 800, whiteSpace: 'nowrap',
                color: price ? ORANGE : (armL3 ? INK : DIM), padding: price ? '5px 10px' : 0,
                borderRadius: 4, background: 'transparent',
                border: price ? `1px solid ${ORANGE}88` : 'none',
              }}>{price || armL3 || ''}</span>
            </div>
          )
        })}
    ...
    <Footer key="f" site={site} note={`${site}/called · CALLED IT — graded live as they land`} />,
  ], fonts, ROWS_HEIGHT)
}
  ...
  return frame([
    ...
    <div key="body" style={{ display: 'flex', flexDirection: 'column', padding: '50px 50px 0 50px', flex: 1, justifyContent: 'center' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', marginTop: 28, gap: 14 }}>
        {detail.map((l, i) => (
          <span key={i} style={{ fontSize: 24, fontWeight: 800, color: '#b9b2a8' }}>{String(l)}</span>
        ))}
      </div>
    </div>,
...
/**
 * THE HOT STRETCH -- the showcase card (2026-09-18).
 *
 * Donovan, on the first version (a statCard: a headline and four grey lines
 * in the left half of a 1200x675 canvas): "i just saw the one for elly sept
 * that can be alot better color and more stuff kinda like how the home run
 * cards are make its like a show case card or soemthign unque."
 *
 * So it is built in homerCard's language rather than statCard's: his team
 * color on the edge and washed behind the emblem, the name in the display
 * face at homer-card size, one HERO number (OPS -- the number that decides
 * who gets this card in the first place), and the slash line as a row of
 * big numerals instead of a sentence. The two things that make it its own
 * card rather than a recolored homer card are the headshot and the fact that
 * every number on it covers a WINDOW rather than one swing.
 *
 * `pick` is exactly what hotStretchPicks() returns; `window` is 'month' or
 * 'week' and only decides the label.
 */
export async function hotStretchCard(day, pick, { site = 'dashnetwork.vercel.app', window = 'month', windowLabel = '' } = {}) {
  const [base, , display] = await Promise.all([loadFonts(), loadMark(), loadDisplay(), loadGrain()])
  const fonts = [...base, ...display]
  const name = String(pick?.name || 'Unknown')
  const team = String(pick?.team || '').toUpperCase()
  const col = teamColor(team)
  const face = await loadHeadshot(pick?.player_id)
  const label = String(windowLabel || (window === 'week' ? 'LAST 7 DAYS' : 'THIS MONTH')).toUpperCase()
  const arm = String(pick?.pitcher || '').trim().split(/\s+/).slice(-1)[0] || ''

  // The four supporting numerals. OPS is deliberately NOT here -- it is the
  // hero in the emblem, and printing it twice would flatten the hierarchy
  // this card exists to create.
  const tiles = [
    { label: 'AVG', value: String(pick?.avg || '') },
    { label: 'OBP', value: String(pick?.obp || '') },
    { label: 'SLG', value: String(pick?.slg || '') },
    { label: 'HR', value: String(pick?.hr ?? '') },
    { label: 'RBI', value: String(pick?.rbi ?? '') },
  ].filter((t) => t.value !== '' && t.value !== 'null' && t.value !== 'undefined')

  // season vs window. Rate stats only -- HR and RBI over a 15-game window
  // against a 150-game season is not a comparison, it is two different
  // questions, and putting them side by side would read as a decline.
  const se = pick?.season || null
  const cmp = (key) => {
    const a = String(se?.[key] || '')
    const b = String(pick?.[key] || '')
    if (!a || !b) return null
    return { label: key.toUpperCase(), season: a, now: b, up: Number(b) >= Number(a) }
  }
  const seasonRows = se ? ['avg', 'obp', 'slg', 'ops'].map(cmp).filter(Boolean) : []

  return frame([
    <Header label="THE HOT STRETCH" sub={`${prettyDay(day)} · the hottest bat on tonight's board`} pills={[label]} key="h" />,

    // his team behind the emblem, same wash homerCard uses
    <div key="wash" style={{ display: 'flex', position: 'absolute', right: 0, top: 0, width: 520, height: 420, background: `radial-gradient(circle at 100% 0%, ${col}38 0%, ${col}00 70%)` }} />,

    // THE EMBLEM -- face, window, and the one number that earned the card.
    <div key="emblem" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', position: 'absolute', right: 40, top: 92, width: 250, padding: '14px 14px 12px 14px', borderRadius: 4, border: `1px solid ${col}99`, background: 'rgba(249,115,22,0.07)' }}>
      {face
        ? <img src={face} width={112} height={112} style={{ borderRadius: 4, border: `2px solid ${col}`, objectFit: 'cover' }} />
        : <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: 112, height: 112, borderRadius: 4, border: `2px solid ${col}`, background: col, color: inkOn(col), fontSize: 34, fontWeight: 800, letterSpacing: 1 }}>{team || 'MLB'}</div>}
      <span style={{ fontSize: 12, fontWeight: 800, letterSpacing: 3, marginTop: 10, color: INK }}>{label}</span>
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginTop: 8, paddingTop: 8, width: '100%', borderTop: `1px solid ${col}66` }}>
        <span style={{ fontFamily: DISPLAY, fontSize: 64, fontWeight: 900, lineHeight: 1, color: ORANGE, transform: 'skewX(-8deg)' }}>{String(pick?.ops || '')}</span>
        <span style={{ fontSize: 12, fontWeight: 800, letterSpacing: 3.4, color: INK, marginTop: 2 }}>OPS</span>
      </div>
    </div>,

    // identity
    <div key="id" style={{ display: 'flex', flexDirection: 'column', padding: '32px 320px 0 40px' }}>
      <span style={{ fontSize: 14, fontWeight: 800, letterSpacing: 6, color: ORANGE }}>ON A HEATER</span>
      <span style={{ fontFamily: DISPLAY, fontSize: 76, fontWeight: 900, lineHeight: 0.98, marginTop: 4, transform: 'skewX(-8deg)', letterSpacing: -1 }}>{name}</span>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 12 }}>
        {team ? <span style={{ background: col, color: inkOn(col), border: `1px solid ${col}`, fontSize: 15, fontWeight: 800, letterSpacing: 1.4, borderRadius: 999, padding: '4px 14px' }}>{team}</span> : null}
        <span style={{ fontSize: 17, fontWeight: 800, color: INK }}>{`${pick?.games || 0} GAMES · ${pick?.ab || 0} AB`}</span>
      </div>
    </div>,

    // THE LINE -- the slash line as numerals, not a sentence.
    tiles.length ? (
      <div key="tiles" style={{ display: 'flex', gap: 12, margin: '26px 320px 0 40px' }}>
        {tiles.map((t) => (
          <div key={t.label} style={{ display: 'flex', flexDirection: 'column', flex: 1, alignItems: 'center', border: `1px solid ${RULE}`, background: PANEL, padding: '16px 6px 20px 6px' }}>
            <span style={{ fontSize: 10.5, fontWeight: 800, letterSpacing: 2.6, color: '#b9b2a8' }}>{t.label}</span>
            <span style={{ fontFamily: DISPLAY, fontSize: 44, fontWeight: 900, lineHeight: 1.05, color: ORANGE, transform: 'skewX(-8deg)', marginTop: 2 }}>{t.value}</span>
          </div>
        ))}
      </div>
    ) : null,

    // VS HIS OWN SEASON (2026-09-18). The card's whole claim is "this window is
    // hotter than he normally is" -- this is the evidence for it, rather than
    // 230px of empty canvas under the tiles. Both rows are MLB's own splits;
    // nothing here is computed except which arrow to draw. Dropped entirely
    // when the season call failed, rather than shown half-empty.
    seasonRows.length ? (
      <div key="vs" style={{ display: 'flex', flexDirection: 'column', margin: '22px 40px 0 40px', border: `1px solid ${RULE}`, background: PANEL }}>
        <div style={{ display: 'flex', alignItems: 'center', padding: '9px 16px', borderBottom: `1px solid ${RULE}` }}>
          <span style={{ fontSize: 13, fontWeight: 800, letterSpacing: 1.6 }}>{`${label} vs HIS SEASON`}</span>
          <span style={{ marginLeft: 'auto', fontSize: 11, fontWeight: 800, letterSpacing: 2.2, color: DIM }}>{`${pick?.season?.games || 0} GAMES ON THE YEAR`}</span>
        </div>
        <div style={{ display: 'flex', padding: '16px 16px 20px 16px' }}>
          {seasonRows.map((r) => (
            <div key={r.label} style={{ display: 'flex', flexDirection: 'column', flex: 1 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, marginTop: 3 }}>
                <span style={{ fontSize: 20, fontWeight: 800, color: DIM }}>{r.season}</span>
                <span style={{ fontSize: 14, fontWeight: 800, color: r.up ? ORANGE : DIM }}>{r.up ? '▲' : '▼'}</span>
                <span style={{ fontSize: 22, fontWeight: 800, color: INK }}>{r.now}</span>
              </div>
            </div>
          ))}
        </div>
    ...
    // the closing strip -- why this is on tonight's account and not a trivia card
    <div key="tonight" style={{ display: 'flex', alignItems: 'center', margin: '26px 40px 0 40px', padding: '19px 22px', borderRadius: 4, background: `linear-gradient(90deg, ${col}33 0%, rgba(249,115,22,0.06) 100%)`, border: `1px solid ${col}88` }}>
      <span style={{ fontSize: 22, fontWeight: 800, letterSpacing: 0.3, color: INK }}>{arm ? `ON TONIGHT'S BOARD — FACING ${arm.toUpperCase()}` : "ON TONIGHT'S BOARD"}</span>
      <span style={{ marginLeft: 'auto', fontSize: 13, fontWeight: 800, letterSpacing: 2.6, color: ORANGE }}>MOONSHOT</span>
    </div>,
...
/**
 * THE PAIR TRAP -- two men already on tonight's board with a real same-day
 * history. Was a one-line callout (names + a stat sentence); each man now
 * gets his own row -- recent form, who he's facing, his own price tonight --
 * off the same board row the Hot Zone / Danger Combos posts already read.
 */
export async function pairsCard(day, hits, { site = 'dashnetwork.vercel.app' } = {}) {
  const [base, , display] = await Promise.all([loadFonts(), loadMark(), loadDisplay(), loadGrain()])
  const fonts = [...base, ...display]
  const main = hits[0]
  const rest = hits.slice(1)

  const playerRow = (p, last) => (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 9, padding: '20px 0', borderBottom: last ? 'none' : `1px solid ${RULE}` }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
        <span style={{ background: teamColor(p.team), color: inkOn(teamColor(p.team)), fontSize: 15, fontWeight: 800, letterSpacing: 1.2, borderRadius: 999, padding: '5px 14px' }}>{p.team || '—'}</span>
        <span style={{ fontFamily: DISPLAY, fontSize: 36, fontWeight: 900, transform: 'skewX(-8deg)', color: INK }}>{p.name}</span>
        <span style={{ marginLeft: 'auto', fontSize: 16, fontWeight: 800, color: DIM }}>{p.opponent ? `vs ${p.opponent}` : ''}</span>
      </div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
        {p.blastPct != null ? <span style={{ fontSize: 15, fontWeight: 800, color: ORANGE }}>{`🔥 ${p.blastPct}% blast rate`}</span> : null}
        {p.pitcher ? <span style={{ fontSize: 15, fontWeight: 800, color: DIM }}>{`facing ${p.pitcher}${p.pitcherTeam ? ` (${p.pitcherTeam})` : ''}`}</span> : null}
      </div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
        {(p.over != null && p.book) ? <span style={{ fontSize: 16, fontWeight: 800, color: ORANGE }}>{`HR odds tonight: ${fmtOdds(p.over)} · ${p.book}`}</span> : null}
        {p.hr_score != null ? <span style={{ fontSize: 15, fontWeight: 800, color: DIM, marginLeft: 'auto' }}>{`Moonshot Score ${Math.round(p.hr_score)}`}</span> : null}
      </div>
    </div>
  )

  return frame([
    <Header label="THE PAIR TRAP" sub={`${prettyDay(day)} · pregame`} pills={['PAIRS']} key="h" />,

    <div key="body" style={{ display: 'flex', gap: 24, padding: '30px 40px 30px 40px', flex: 1 }}>
      <div style={{ display: 'flex', flexDirection: 'column', flex: 1, justifyContent: 'center', border: `1px solid ${RULE}`, background: PANEL, padding: '8px 26px' }}>
        <span style={{ fontSize: 13, fontWeight: 800, letterSpacing: 5, color: ORANGE, padding: '14px 0 4px 0' }}>BOTH ON THE BOARD TONIGHT</span>
        {playerRow(main.a, false)}
        {playerRow(main.b, true)}
        {rest.length ? (
          <div style={{ display: 'flex', flexDirection: 'column', padding: '12px 0', gap: 6, borderTop: `1px solid ${RULE}` }}>
            {rest.map((r) => (
              <span key={`${r.a.player_id}-${r.b.player_id}`} style={{ fontSize: 15, fontWeight: 800, color: DIM }}>{`+ ${r.a.name} & ${r.b.name} — ${r.count}x same-day`}</span>
            ))}
          </div>
        ) : null}
      </div>
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', width: 220, padding: '20px 16px', borderRadius: 4, border: `1px solid rgba(249,115,22,0.55)`, background: 'rgba(249,115,22,0.09)' }}>
        <span style={{ fontSize: 36, lineHeight: 1 }}>👥</span>
        <span style={{ fontFamily: DISPLAY, fontSize: 80, fontWeight: 900, lineHeight: 1, color: ORANGE, marginTop: 12, transform: 'skewX(-8deg)' }}>{String(main.count)}</span>
        <span style={{ fontSize: 12, fontWeight: 800, letterSpacing: 2, color: INK, marginTop: 4, textAlign: 'center' }}>SAME-DAY HRs THIS SEASON</span>
        {main.rate != null ? <span style={{ fontSize: 14, fontWeight: 800, color: DIM, marginTop: 14 }}>{`${main.rate}% of games`}</span> : null}
        {main.sameGame ? (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginTop: 18, paddingTop: 18, width: '100%', borderTop: '1px solid rgba(249,115,22,0.30)' }}>
            <span style={{ fontFamily: DISPLAY, fontSize: 36, fontWeight: 900, color: ORANGE, transform: 'skewX(-8deg)' }}>{String(main.sameGame)}</span>
            <span style={{ fontSize: 11, fontWeight: 800, letterSpacing: 1.5, color: INK, marginTop: 2 }}>SAME GAME</span>
          </div>
        ) : null}
      </div>
    </div>,
...
/**
 * THE MOONSHOT -- tonight's single longest-priced TOP/HR call on the board.
 * Was odds + a Moonshot Score line; now a proper mini profile -- identity,
 * the odds bar, and a recent-form / who-he's-facing row underneath, off the
 * same board row the Hot Zone / Danger Combos posts already read.
 */
export async function longshotCard(day, pick, { site = 'dashnetwork.vercel.app' } = {}) {
  const [base, , display] = await Promise.all([loadFonts(), loadMark(), loadDisplay(), loadGrain()])
  const fonts = [...base, ...display]
  const col = teamColor(pick.team)
  // roleWord() here is the same one every tweet uses ("TOP pick", "HRR
  // pick", ...) -- an earlier pass invented its own lock/top/hrr labels
  // that didn't match the account's real pick-type vocabulary anywhere else.
  const badgeWord = pick.role ? roleWord(pick.role).toUpperCase() : 'ON THE BOT'
  const battingRow = [
    pick.blastPct != null ? { label: 'BLAST RATE', value: `${pick.blastPct}%` } : null,
    pick.barrelPct != null ? { label: 'BARREL %', value: `${pick.barrelPct}%` } : null,
    pick.hardHitPct != null ? { label: 'HARD-HIT %', value: `${pick.hardHitPct}%` } : null,
  ].filter(Boolean)

  return frame([
    <Header label="THE MOONSHOT" sub={`${prettyDay(day)} · pregame`} pills={['LONGSHOT']} key="h" />,

    <div key="wash" style={{ display: 'flex', position: 'absolute', right: 0, top: 0, width: 480, height: 340, background: `radial-gradient(circle at 100% 0%, ${col}30 0%, ${col}00 68%)` }} />,

    <div key="emblem" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', position: 'absolute', right: 40, top: 96, width: 220, padding: '18px 14px', borderRadius: 4, border: `1px solid rgba(249,115,22,0.55)`, background: 'rgba(249,115,22,0.09)' }}>
      <span style={{ fontSize: 40, lineHeight: 1 }}>🎯</span>
      <span style={{ fontSize: 15, fontWeight: 800, letterSpacing: 2, marginTop: 8, color: ORANGE }}>{badgeWord}</span>
      {pick.hr_score != null ? (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginTop: 12, paddingTop: 12, width: '100%', borderTop: '1px solid rgba(249,115,22,0.30)' }}>
          <span style={{ fontSize: 11, fontWeight: 800, letterSpacing: 3, color: INK }}>MOONSHOT SCORE</span>
          <span style={{ fontFamily: DISPLAY, fontSize: 60, fontWeight: 900, lineHeight: 1, color: ORANGE, transform: 'skewX(-8deg)' }}>{String(Math.round(pick.hr_score))}</span>
        </div>
      ) : null}
    </div>,

    <div key="body" style={{ display: 'flex', flexDirection: 'column', flex: 1, justifyContent: 'center', gap: 22 }}>
      <div key="id" style={{ display: 'flex', flexDirection: 'column', padding: '0 280px 0 40px' }}>
        <span style={{ fontSize: 15, fontWeight: 800, letterSpacing: 6, color: ORANGE }}>TONIGHT'S LONGEST CALL</span>
        <div style={{ display: 'flex', alignItems: 'flex-end', gap: 18, marginTop: 8 }}>
          <span style={{ fontFamily: DISPLAY, fontSize: 80, fontWeight: 900, lineHeight: 0.98, transform: 'skewX(-8deg)', letterSpacing: -1 }}>{pick.name}</span>
          <span style={{ background: col, color: inkOn(col), fontSize: 16, fontWeight: 800, letterSpacing: 1.4, borderRadius: 999, padding: '5px 16px', marginBottom: 10 }}>{pick.team || ''}</span>
        </div>
        {pick.opponent ? <span style={{ fontSize: 18, fontWeight: 800, color: INK, marginTop: 4 }}>{`vs ${pick.opponent}`}</span> : null}
      </div>

      <div key="call" style={{ display: 'flex', alignItems: 'center', margin: '0 280px 0 40px', padding: '24px 30px', borderRadius: 4, background: 'linear-gradient(90deg, rgba(249,115,22,0.20) 0%, rgba(249,115,22,0.05) 100%)', border: '1px solid rgba(249,115,22,0.55)' }}>
        <span style={{ fontSize: 14, fontWeight: 800, letterSpacing: 2, color: INK }}>HR ODDS TO GO DEEP</span>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, marginLeft: 'auto' }}>
          <span style={{ fontFamily: DISPLAY, fontSize: 58, fontWeight: 900, lineHeight: 1, color: ORANGE, transform: 'skewX(-8deg)' }}>{fmtOdds(pick.over)}</span>
          <span style={{ fontSize: 18, fontWeight: 800, color: INK }}>{pick.book}</span>
        </div>
      </div>

      {battingRow.length ? (
        <div key="batting" style={{ display: 'flex', margin: '0 40px', border: `1px solid ${RULE}`, background: PANEL }}>
          {battingRow.map((s, i) => (
            <div key={s.label} style={{ display: 'flex', flexDirection: 'column', flex: 1, padding: '16px 24px', borderRight: i < battingRow.length - 1 ? `1px solid ${RULE}` : 'none' }}>
              <span style={{ fontSize: 11, fontWeight: 800, letterSpacing: 2, color: '#b9b2a8' }}>{s.label}</span>
              <span style={{ fontSize: 28, fontWeight: 800, color: ORANGE, marginTop: 4 }}>{s.value}</span>
            </div>
          ))}
        </div>
      ) : null}

      {(pick.pitcher || pick.pitcherHr9 != null) ? (
        <div key="pitching" style={{ display: 'flex', margin: '0 40px', border: `1px solid ${RULE}`, background: PANEL }}>
          {pick.pitcher ? (
            <div style={{ display: 'flex', flexDirection: 'column', flex: 1, padding: '16px 24px', borderRight: pick.pitcherHr9 != null ? `1px solid ${RULE}` : 'none' }}>
              <span style={{ fontSize: 11, fontWeight: 800, letterSpacing: 2, color: '#b9b2a8' }}>FACING TONIGHT</span>
              <span style={{ fontSize: 26, fontWeight: 800, color: INK, marginTop: 4 }}>{`${pick.pitcher}${pick.pitcherTeam ? ` (${pick.pitcherTeam})` : ''}`}</span>
            </div>
          ) : null}
          {pick.pitcherHr9 != null ? (
            <div style={{ display: 'flex', flexDirection: 'column', flex: 1, padding: '16px 24px' }}>
              <span style={{ fontSize: 11, fontWeight: 800, letterSpacing: 2, color: '#b9b2a8' }}>PITCHER HR/9</span>
              <span style={{ fontSize: 28, fontWeight: 800, color: ORANGE, marginTop: 4 }}>{Number(pick.pitcherHr9).toFixed(2)}</span>
            </div>
          ) : null}
        </div>
      ) : null}
...
/**
 * THE NUMEROLOGY MOMENT -- trifecta / jersey-match / cluster. Was a comma-
 * joined name string plus one summary line; jersey-match and cluster tiers
 * (2+ players) now get a real per-player table -- jersey, opponent, and
 * whether that man was also a bot pick tonight, tying every numerology post
 * back to the same "the bot called it" brand the rest of the account runs
 * on, instead of raw physical stats that don't serve the numerology angle.
 */
export async function numerologyCard(day, moment, { site = 'dashnetwork.vercel.app' } = {}) {
  const [base, , display] = await Promise.all([loadFonts(), loadMark(), loadDisplay(), loadGrain()])
  const fonts = [...base, ...display]
  const label = moment.tier === 'trifecta' ? 'TRIFECTA' : moment.tier === 'jersey' ? 'JERSEY MATCH' : 'CLUSTER'
  const root = moment.tier === 'jersey' ? null : moment.root
  const shown = moment.players.slice(0, 5)
  const rest = moment.players.length - shown.length

  const playerRow = (p, i) => (
    <div key={p.player_id || i} style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '16px 20px', borderBottom: i < shown.length - 1 ? `1px solid ${RULE}` : 'none' }}>
      <span style={{ fontFamily: DISPLAY, fontSize: 24, fontWeight: 900, color: ORANGE, width: 50, transform: 'skewX(-8deg)' }}>{`#${p.jersey}`}</span>
      <span style={{ fontSize: 26, fontWeight: 800, color: INK, flex: 1 }}>{p.name}</span>
      {p.team ? <span style={{ background: teamColor(p.team), color: inkOn(teamColor(p.team)), fontSize: 13.5, fontWeight: 800, borderRadius: 999, padding: '4px 12px' }}>{p.team}</span> : null}
      {p.opponent ? <span style={{ fontSize: 14, fontWeight: 800, color: DIM, marginLeft: 8 }}>{`vs ${p.opponent}`}</span> : null}
      {p.role ? <span style={{ fontSize: 14, fontWeight: 800, color: ORANGE, marginLeft: 14 }}>{`🤖 ${roleWord(p.role).toUpperCase()}`}</span> : null}
      <span style={{ fontSize: 15, fontWeight: 800, color: DIM, marginLeft: 12 }}>{p.nth ? `HR #${p.nth}` : ''}</span>
    </div>
  )

  return frame([
    <Header label={label} sub={`${prettyDay(day)} · live`} pills={['NUMEROLOGY']} key="h" />,

    <div key="body" style={{ display: 'flex', gap: 20, padding: '28px 40px 0 40px', flex: 1, alignItems: 'center' }}>
      <div style={{ display: 'flex', flexDirection: 'column', flex: 1, border: `1px solid ${RULE}`, background: PANEL }}>
        {shown.map((p, i) => playerRow(p, i))}
        {rest > 0 ? <span style={{ fontSize: 14, fontWeight: 800, color: DIM, padding: '12px 20px' }}>{`+ ${rest} more`}</span> : null}
      </div>
      {root != null ? (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: 220, padding: '20px 16px', borderRadius: 4, border: '1px solid rgba(249,115,22,0.55)', background: 'rgba(249,115,22,0.09)' }}>
          <span style={{ fontSize: 12, fontWeight: 800, letterSpacing: 2.5, color: INK }}>DIGIT ROOT</span>
          <span style={{ fontFamily: DISPLAY, fontSize: 92, fontWeight: 900, lineHeight: 1, color: ORANGE, marginTop: 6, transform: 'skewX(-8deg)' }}>{String(root)}</span>
          <span style={{ fontSize: 13, fontWeight: 800, color: DIM, marginTop: 10, textAlign: 'center' }}>{moment.tier === 'trifecta' ? 'jersey · HR count · birthday' : 'shared jersey digit'}</span>
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: 220, padding: '20px 16px', borderRadius: 4, border: '1px solid rgba(249,115,22,0.55)', background: 'rgba(249,115,22,0.09)' }}>
          <span style={{ fontSize: 38, lineHeight: 1 }}>🔢</span>
          <span style={{ fontFamily: DISPLAY, fontSize: 62, fontWeight: 900, lineHeight: 1, color: ORANGE, marginTop: 10, transform: 'skewX(-8deg)' }}>{`#${moment.jersey}`}</span>
          <span style={{ fontSize: 12, fontWeight: 800, letterSpacing: 2, color: INK, marginTop: 8, textAlign: 'center' }}>SAME NUMBER, SAME NIGHT</span>
        </div>
      )}
    </div>,
