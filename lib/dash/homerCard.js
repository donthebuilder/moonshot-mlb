...
async function frame(children, fonts) {
  return new ImageResponse(
    (
      <div style={{
        width: 1200, height: 675, display: 'flex', flexDirection: 'column',
...
export async function pregameCard(day, picks = [], { site = 'dashnetwork.vercel.app' } = {}) {
  const fonts = await loadFonts()
  const list = (Array.isArray(picks) ? picks : []).slice(0, 5)
  return frame([
    <Header label="CALLED IT · TONIGHT'S CALLS" sub={`${prettyDay(day)} · posted before first pitch`} pills={['PREGAME']} key="h" />,

    <div key="body" style={{ display: 'flex', flexDirection: 'column', padding: '22px 40px 0 40px' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', marginTop: 10 }}>
        {list.map((p, i) => {
          ...
          return (
            <div key={p.player_id || i} style={{ display: 'flex', alignItems: 'center', gap: 18, padding: '11px 0', borderBottom: i < list.length - 1 ? '1px solid rgba(255,255,255,0.08)' : 'none' }}>
              <span style={{ width: 44, fontSize: 30, fontWeight: 800, color: ORANGE }}>{String(i + 1)}</span>
              <span style={{ fontSize: 32, fontWeight: 800, width: 400 }}>{String(p.name || '')}</span>
              {p.team ? <span style={{ background: col, color: inkOn(col), fontSize: 15, fontWeight: 800, borderRadius: 8, padding: '4px 11px' }}>{String(p.team)}</span> : null}
              <span style={{ fontSize: 19, color: '#d4d4d8', width: 330, whiteSpace: 'nowrap' }}>{p.opponent ? `vs ${p.opponent}${p.pitcher ? ` · ${p.pitcher}` : ''}` : ''}</span>
              <span style={{ marginLeft: 'auto', fontSize: 19, color: DIM, whiteSpace: 'nowrap' }}>{p.hr_score != null ? `HR score ${Math.round(Number(p.hr_score))}` : ''}</span>
              <span style={{ width: 200, justifyContent: 'flex-end', display: 'flex', fontSize: 22, fontWeight: 800, color: price ? INK : DIM, whiteSpace: 'nowrap' }}>{price || '—'}</span>
            </div>
          )
        })}
      </div>
