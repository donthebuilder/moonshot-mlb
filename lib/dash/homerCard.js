...
export async function homerCard(row, { site = 'dashnetwork.vercel.app' } = {}) {
  const [fonts] = await Promise.all([loadFonts(), loadMark()])
  ...
  return frame([
      ...
      score != null ? (
        <div key="ghost" style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', position: 'absolute', right: 40, top: 76 }}>
          <span style={{ fontSize: 176, fontWeight: 800, color: 'rgba(249,115,22,0.14)', lineHeight: 0.8 }}>{String(score)}</span>
          <span style={{ fontSize: 12, fontWeight: 800, letterSpacing: 4, color: 'rgba(249,115,22,0.34)', marginTop: 2 }}>HR SCORE</span>
        </div>
      ) : null,
      ...
      // the call
      <div key="call" style={{ display: 'flex', alignItems: 'center', gap: 22, margin: '16px 40px 0 40px', padding: '14px 22px', borderRadius: 14, background: called ? 'linear-gradient(90deg, rgba(249,115,22,0.20) 0%, rgba(249,115,22,0.05) 100%)' : 'rgba(255,255,255,0.05)', border: `1px solid ${called ? 'rgba(249,115,22,0.55)' : 'rgba(255,255,255,0.10)'}` }}>
        ...
        <div style={{ display: 'flex', gap: 22, marginLeft: 'auto', fontSize: 17, color: DIM }}>
          ...
          {score != null ? <span>{`MOONSHOT Score ${score}`}</span> : null}
...
export async function statCard(day, { pill = '', label = '', headline = '', lines = [] } = {}, { site = 'dashnetwork.vercel.app' } = {}) {
  ...
  return frame([
    ...
    <div key="body" style={{ display: 'flex', flexDirection: 'column', padding: '50px 50px 0 50px', flex: 1, justifyContent: 'center' }}>
      <span style={{ fontSize: 44, fontWeight: 800, lineHeight: 1.3 }}>{String(headline || '')}</span>
