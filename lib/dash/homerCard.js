  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', marginTop: 'auto' }}>
      <div style={{ display: 'flex', alignItems: 'center', padding: '0 40px 16px 40px', fontSize: 14, color: '#71717a' }}>
        <span>{note || `${site}/called — was he on the bot before the ball left?`}</span>
...
export async function homerCard(row, { site = 'dashnetwork.app' } = {}) {
  ...
  return frame([
      <Header label="HOME RUN" sub={`${prettyDay(String(row?.day || ''))} · every call graded in public`} pills={[nth, 'TONIGHT'].filter(Boolean)} key="h" />,
 ...
 */
export async function recapCard(day, rows = [], history = [], { site = 'dashnetwork.app' } = {}) {
  const fonts = await loadFonts()
  ...
  return frame([
      <Header label="THE NIGHT" sub={`${prettyDay(day)} · every home run, on the record`} pills={['RECAP']} key="h" />,
      ...
      <div key="body" style={{ display: 'flex', padding: '26px 40px 0 40px', gap: 40 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', flex: 1 }}>
          ...
          <span style={{ fontSize: 26, marginTop: 8 }}>{`${c.pct ?? 0}% tonight${c.rated ? ` · ${c.rated} more rated, not picked` : ''}${c.off ? ` · ${c.off} off the board` : ''}`}</span>
      ...
      <Footer key="f" site={site} note={`${site}/called — tonight's list, and every night before it`} />,
  ], fonts)
}
