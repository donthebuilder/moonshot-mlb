...
import { fmtOdds, hrQuoteFor, l5Line } from './homerFeed'
...
// A PROJECTED ARM IS NOT A FACT (2026-09-24, F3). The bot's
// resolve_probable_pitcher() guesses a rotation arm when MLB has posted no
// probable, and bots/mlb_dashboard.py publishes that guess as
// `pitcher_projected: true` on every row that carries one. Every post below
// that names the man on the mound states a matchup as a fact, so a projected
// arm reads as no arm: builders that require a pitcher drop the row, builders
// that only mention one leave it blank. (The 09-18 slate-level freshness
// gate, boardPitchersFresh, answers a different question -- whether the whole
// column is yesterday's -- and stays.)
const pitcherFact = (r) => (r?.pitcher_projected === true ? '' : txt(r?.pitcher_name))
...
function shrinkToFit(head, lines, tail, { spaced = false, lead = '', closer = '', max = 0 } = {}) {
...
// Surname only, for the second line of a two-line entry where the full name
// is already above it. lib/matchupStory.js has its own armName() but does not
// export it, and one import for one surname is not worth the coupling.
const surname = (v) => String(v || '').trim().split(/\s+/).slice(-1)[0] || ''
...
export function hottestContactPicks(rows, limit = 8) {
  ...
  for (const r of list) {
    ...
    if (!pitcherFact(r)) continue   // no game tonight (or a guessed arm), nothing to face
    seen.add(pid)
    out.push({
      player_id: pid, name: txt(r.name), team: txt(r.team) || null,
      pitcher: pitcherFact(r), pitcherTeam: txt(r.pitcher_team) || null,
      blastPct: pct1(blast),
    })
  }
...
export function hottestContactText(picks, { day = '', site = '', handle = '', variant = 'pregame' } = {}) {
  ...
  // 2026-09-18, Donovan's review: "🔥 HOT CONTACT" -- standard baseball
  // language first, proprietary second, the same SEO rule the TUDDY plan is
  // written on. Nobody searches "the hot zone."
  const head = variant === 'mid'
    ? `🔥 HOT CONTACT: STILL LIT`
    : `🔥 HOT CONTACT`
  ...
  // The pitcher came OFF each line and became the closer: it was the same
  // fact ("vs tonight's starter") repeated on every row at ~14 characters a
  // go. Said once, it costs 26 characters total instead of 70 and the rows
  // get shorter, which is more names.
  const lines = picks.map((p) => `${p.name}${p.team ? ` (${p.team})` : ''} — ${p.blastPct}%`)
  ...
  // max 5: his own draft ran four. Eight names of one rate is a leaderboard;
  // five is a post.
  return shrinkToFit(head, lines, tail, { spaced: true, max: 5, closer: "Against tonight's starters." })
}
...
export function dangerComboText(picks, { day = '', site = '', handle = '', variant = 'pregame' } = {}) {
  if (!Array.isArray(picks) || !picks.length) return ''
  const tail = [site, handle].filter(Boolean).join(' · ')
  // 2026-09-18: "THE KILL LIST" said nothing about baseball and nothing a
  // person would search. The pairing IS the post, so the head now names it.
  const head = variant === 'mid'
    ? `🔥 HOT BAT vs HITTABLE ARM: STILL LIVE`
    : `🔥 HOT BAT vs HITTABLE ARM`
  ...
  // Two lines per entry -- the hitter, then the pairing -- instead of one
  // 75-character row that ran off a phone screen. Same two rates, readable.
  const lines = picks.map((p) =>
    `${p.name}${p.team ? ` (${p.team})` : ''}\n${p.blastPct}% contact · ${surname(p.pitcher)}: ${p.pitcherHrBbePct}% HR/BBE`)
  // max 3: every entry is TWO lines, so three is already six rows on a phone.
  ...
  return shrinkToFit(head, lines, tail, { spaced: true, max: 3 })
}
...
export function hrLeadersByDowText(leaders, dow, { day = '', site = '', handle = '' } = {}) {
  ...
  // 2026-09-18, Donovan's review: the weekday belongs IN the headline
  // ("FRIDAY HR LEADERS" is a thing people search; "HR LEADERS · LAST 5
  // FRIDAYS" is a column header), the window moves to a lead line under it,
  // and the closer says the honest thing out loud rather than leaving a
  // reader to work out that five Fridays is a small sample.
  const head = `📅 ${dow.toUpperCase()} HR LEADERS`
  const lines = leaders.map((p, i) =>
    `${p.name}${p.team ? ` (${p.team})` : ''} — ${p.hr}${i === 0 ? ' HR' : ''}`)
  ...
  return shrinkToFit(head, lines, tail, {
    spaced: true,
    max: 5,
    // dow arrives already capitalised ("Friday") -- toLowerCase() here printed
    // "Last 5 fridays:" in the live render. Use it as given.
    lead: `Last 5 ${dow}s:`,
    closer: 'Small sample. Still interesting.',
  })
}
...
// pickers above.
// RECENCY BEFORE SCORE (2026-09-18). Donovan, 2026-09-13: "posting the wrong
// back to back watch players." Diagnosed then, not fixed: this sorted purely
// on hr_score, so a hitter whose last game was FIVE DAYS AGO could outrank
// last night's actual homers. lib/b2bCore.js computes the gap
// (`_b2bGapDays`) and this function was throwing it away.
//
// It became a correctness problem TODAY, when the post was renamed from
// "BACK-TO-BACK WATCH" to "HOMERED LAST TIME OUT" -- a hedge became a claim,
// and a five-day-old homer headlining a post that says "last time out" is the
// account stating something that reads false. The rename is right; this is
// what has to be true for it to be right.
//
// Nobody is FILTERED OUT: `games_since_last_hr <= 0` upstream already means
// the homer came in his most recent game, so the claim holds for everyone
// here -- a five-day gap means he hasn't played since, not that the post is
// wrong. But the freshest go first, and on a capped list of five that is the
// whole difference.
export function backToBackPicks(rows, day, limit = 10) {
  ...
  for (const p of list) {
    ...
    out.push({
      ...
      gapDays: Number.isFinite(Number(p?._b2bGapDays)) ? Number(p._b2bGapDays) : 1,
      hr_score: num(p?.hr_score) || 0,
      last5_avg: p.last5_avg, last5_hr: p.last5_hr, last5_rbi: p.last5_rbi, last5_status: p.last5_status,
    })
  }
  // Gap ascending, then score descending -- yesterday's homers ahead of last
  // week's, and within a day the board's own ranking decides.
  out.sort((a, b) => (a.gapDays - b.gapDays) || (b.hr_score - a.hr_score))
  return out.slice(0, limit)
}
...
export function backToBackText(picks, { day = '', site = '', handle = '' } = {}) {
  if (!Array.isArray(picks) || !picks.length) return ''
  const tail = [site, handle].filter(Boolean).join(' · ')
  // 2026-09-18: "BACK-TO-BACK WATCH" is our phrase for a thing that already
  // has a name. The headline now states the fact the post is built on.
  const head = `💥 HOMERED LAST TIME OUT`
  const lines = picks.map((p) => {
    // The HR count is the relevant half of the L5 line on a post about power;
    // the average is the relevant half on a post about contact. Falls back to
    // the full L5 line when he has no homers in the window.
    const hr = Number(p?.last5_hr)
    const tail5 = p?.last5_status === 'ok' && Number.isFinite(hr) && hr > 0 ? `${hr} HR L5` : l5Line(p)
    return `${p.name}${p.team ? ` (${p.team})` : ''}${tail5 ? ` — ${tail5}` : ''}`
  })
  ...
  // the twice-a-day pair that shipped first). max 5 (2026-09-18).
  return shrinkToFit(head, lines, tail, { spaced: true, max: 5 })
}
...
export function theFourPicks(rows) {
  ...
  const topBy = (field) => list
    .filter((r) => txt(r?.player_id) && num(r?.[field]) != null && pitcherFact(r))
  ...
  for (const { key, field } of cats) {
    ...
    if (!r) return []
    out.push({
      key, name: txt(r.name), pitcher: pitcherFact(r), score: Math.round(num(r[field]) * 10) / 10,
...
export function theFourText(picks, { day = '', site = '', handle = '' } = {}) {
  ...
  const tail = [site, handle].filter(Boolean).join(' · ')
  const head = `🎯 THE FOUR`
  // 2026-09-18, first pass (Donovan: "the four should have all the players
  // written out not just 3"). THE FOUR is four categories by definition -- a
  // post that silently drops one is failing at its own name. At 270 the L5
  // slash line ran each row to 60-70 characters and only three ever fit, so
  // it came off.
  //
  // 2026-09-18, second pass ("all the tweet should be full"). With the shared
  // budget it fits again, and picks already carry it. Tried WITH the L5 first
  // and fall back to without -- so on a tight budget this is still four names
  // before it is four names with stats. Four categories is never negotiable;
  // the stat line is.
  const bare = picks.map((p) => `${p.key}: ${p.name} vs ${p.pitcher} ${p.score}`)
  const rich = picks.map((p, i) => { const l5 = l5Line(p); return l5 ? `${bare[i]} · ${l5}` : bare[i] })
  const out = shrinkToFit(head, rich, tail, { spaced: true })
  return out.split('\n').filter((l) => /^[A-Z]+:/.test(l)).length === 4
    ? out
    : shrinkToFit(head, bare, tail, { spaced: true })
}
...
export function bestAirText(picks, { day = '', site = '', handle = '' } = {}) {
  if (!Array.isArray(picks) || !picks.length) return ''
  const tail = [site, handle].filter(Boolean).join(' · ')
  // 2026-09-18: the park on one line, the game and the effect under it. The
  // weather_label is a pre-built sentence ("Retractable · 85°F · 7 mph out to
  // CF/corner · +5% HR") and at 55 characters it was most of the row; the
  // HR effect is the part that answers "so what," so the line keeps that and
  // drops the rest. Falls back to the whole label if the shape ever changes
  // -- no silent blank.
  const head = `🌬️ BEST AIR TONIGHT`
  const effect = (label) => {
    const m = /([+-]\d+(?:\.\d+)?%\s*HR)\s*$/i.exec(String(label || ''))
    return m ? m[1] : String(label || '')
  }
  const lines = picks.map((p) => {
    const e = effect(p.label)
    return `${p.venue}\n${p.matchup}${e ? ` · ${e}` : ''}`
  })
  return shrinkToFit(head, lines, tail, { spaced: true, max: 3, closer: 'Wind matters.' })
}
...
// limit 3 -> 10 (2026-09-18, Donovan: "make sure for the text tweet we are
// getting as much names on there as psooible"). Measured on the live board
// first: this post was 112 of its 270 characters with three names, because
// the PICKER capped at 3, not because the post was full. Ten is a pool, not a
// promise -- shrinkToFit still decides how many fit, the same way milestone
// and matchup-history already work. Tonight's board has 30 starters with a
// real 3-start sample, ranging 0.45 to 5.73 HR/9, so there is no shortage of
// honest names below the top three.
export function storylinesPicks(rows, limit = 10) {
  const list = Array.isArray(rows) ? rows : []
  const seen = new Set()
  const out = []
  for (const r of list.slice().sort((a, b) => num(b?.pitcher_l3_hr9) - num(a?.pitcher_l3_hr9))) {
    ...
    seen.add(pid)
    out.push({ pitcher: pitcherFact(r), l3hr9: Math.round(l3 * 100) / 100, team: txt(r.pitcher_team) || null })
...
export function storylinesText(picks, { day = '', site = '', handle = '' } = {}) {
  if (!Array.isArray(picks) || !picks.length) return ''
  const tail = [site, handle].filter(Boolean).join(' · ')
  // 2026-09-18, Donovan's review. "STORYLINES" described the shape of the
  // post rather than its subject; this is a list of arms giving up homers,
  // so it says that. The window moves to a lead line and the numbers lose
  // their repeated unit -- the label above carries it for the whole column.
  const head = `⚾ ARMS GETTING HIT`
  ...
  // toFixed(2), not the raw rounded number: a 3.00 printing as "3" in a column
  // of 5.73 / 3.78 / 2.35 reads like a different kind of measurement.
  const lines = picks.map((p) => `${p.pitcher}${p.team ? ` (${p.team})` : ''} — ${Number(p.l3hr9).toFixed(2)}`)
  return shrinkToFit(head, lines, tail, {
    spaced: true,
    max: 6,
    lead: 'HR/9 over the last 3 starts:',
    closer: "Who's in the danger zone tonight?",
  })
}
...
export function callOfTheNightPick(rows, odds, day) {
  ...
  const blast = num(top.recent_blast_rate)
  return {
    name: txt(top.name), team: txt(top.team) || null, pitcher: pitcherFact(top) || null,
    edgeSd, pct, price,
    // 2026-09-18 (Donovan: "all the tweet should be full"). The post argued the
    // pick from edge, park and price alone -- three numbers ABOUT the board and
    // none about the hitter or the arm. All four below are already on the row
    // the pick was chosen from; nothing new is computed.
    score: num(top.hr_score) != null ? Math.round(num(top.hr_score) * 10) / 10 : null,
    blastPct: blast != null ? Math.round(blast * 1000) / 10 : null,
    armL3: num(top.pitcher_l3_hr9),
    last5_avg: top.last5_avg, last5_hr: top.last5_hr,
    last5_rbi: top.last5_rbi, last5_status: top.last5_status,
  }
}
...
export function callOfTheNightText(pick, { day = '', site = '', handle = '' } = {}) {
  ...
  // 2026-09-18, Donovan reading the live post: "'PARK+WEATHER: top 0% of the
  // slate' reads wrong to a human." He was right and it is a display bug, not
  // a math one. pct is the pick's PERCENTILE among tonight's park factors, so
  // the best park on the slate scores 100 -- and `100 - pct` then printed
  // "top 0%", which reads as "none" when it means "the best one there is."
  // The underlying percentile is untouched; the two ends now say what they
  // mean in words, and everything between them can never round to zero.
  const parkLine = pick.pct == null ? null
    : pick.pct >= 100 ? 'PARK+WEATHER: the best park on tonight\'s slate'
    : pick.pct <= 0 ? "PARK+WEATHER: the worst park on tonight's slate"
    : pick.pct >= 50 ? `PARK+WEATHER: top ${Math.max(1, 100 - pick.pct)}% of the slate`
    : `PARK+WEATHER: bottom ${Math.max(1, pick.pct)}% of the slate`
  const l5 = l5Line(pick)
  const lines = [
    who,
    pick.score != null ? `MOONSHOT SCORE: ${pick.score}` : null,
    pick.edgeSd != null ? `EDGE: ${pick.edgeSd} SD clear of the next name on the board` : null,
    pick.blastPct != null ? `BLAST RATE: ${pick.blastPct}% on recent contact` : null,
    l5 ? `LAST 5: ${l5.replace(' L5', '')}` : null,
    pick.armL3 != null ? `THE ARM: ${Number(pick.armL3).toFixed(2)} HR/9 over his last 3 starts` : null,
    parkLine,
...
export function matchupLinesText(stories, { day = '', site = '', handle = '' } = {}) {
  ...
  const tail = [site, handle].filter(Boolean).join(' · ')
  // 2026-09-18: "MATCHUP LINES" is ours; "HR MATCHUP HISTORY" is what a
  // person types into search. The BODY is deliberately untouched -- each line
  // is a finished sentence built by lib/matchupStory.js under that file's own
  // honesty rules, and restructuring it into "Name vs Arm: / 1 HR in 3 AB"
  // means parsing prose back into fields. That is a real change to
  // matchupStory.js, not a formatting pass here, and it is not worth doing
  // blind. Flagged, not faked.
  const head = `⚾ HR MATCHUP HISTORY`
  const lines = stories.map((s) => s?.text).filter(Boolean)
  // max 3: each line is a full sentence, so three is already a long post.
  return shrinkToFit(head, lines, tail, { spaced: true, max: 3 })
}
...
export async function vsPitcherCareerLines(rows, { exclude } = {}) {
  ...
  for (const r of Array.isArray(rows) ? rows : []) {
    ...
    if (!pid || !pitcherId || skip.has(pid) || seen.has(pid) || r?.lineup_confirmed !== true || r?.pitcher_projected === true) continue
    seen.add(pid)
    pairs.push({
      player_id: pid, name: txt(r.name), team: txt(r.team) || null,
...
export function hrVsStarterPicks(lines, limit = 24) {
...
export function careerVsStarterPicks(lines, limit = 14) {
  return (Array.isArray(lines) ? lines : [])
  ...
  for (const person of people) {
    ...
    for (const m of S_MILES) {
      ...
      for (const t of m.targets) {
        const need = t - v
        if (need > 0 && need <= m.within) // 2026-09-18: was `${m.word} this season`. Eleven characters on every
        // season row, and the header already says MILESTONE WATCH while every
        // CAREER row says "career" in its own word -- so dropping it stays
        // unambiguous and buys a name.
        miles.push({ ...base, need, t, within: m.within, word: m.word, prox: need / m.within })
      }
    }
...
export function milestoneText(picks, { day = '', site = '', handle = '', wave = 'am' } = {}) {
  if (!Array.isArray(picks) || !picks.length) return ''
  const tail = [site, handle].filter(Boolean).join(' · ')
  // 2026-09-18: says what the post is about rather than naming a feature.
  const head = wave === 'mid'
    ? `📊 CLOSE TO A MILESTONE — STILL CHASING`
  ...
  // max 6 (2026-09-18). The 2026-09-15 note above removed the pre-trim so
  // shrinkToFit alone decided the length -- correct at 270, catastrophic at
  // 890: the live render came back 847 characters and 21 names, which is a
  // spreadsheet. The picker still pulls the full pool (the payload keeps all
  // of them); the POST shows six.
  return shrinkToFit(head, lines, tail, { spaced: true, max: 6 })
}
...
export function storylineWatchText(picks, { day = '', site = '', handle = '', slot = 1 } = {}) {
  ...
  // 2026-09-18, Donovan reading the live 796-char post: "way too much
  // duplication... repeats the same Burns matchup multiple times. That's a
  // sign the generator is retrieving good information but not editorializing
  // it." He was reading BOTH halves of storylineWatchPicks -- the matchup
  // sentences and the fun facts -- interleaved in one post, while the matchup
  // sentences ALSO have their own post now (HR MATCHUP HISTORY above). So
  // this keeps only kind === 'fact': one surprising thing per line, no two
  // about the same game. The picks function is untouched, so putting the
  // lines back is deleting this filter.
  const facts = picks.filter((p) => p?.kind !== 'line')
  const lines = (facts.length ? facts : picks).map((p) => p.text).filter(Boolean)
  return shrinkToFit(head, lines, tail, {
    spaced: true,
    max: 4,
    closer: 'Different reasons to watch tonight.',
  })
}
...
// instead of presenting a guess as a fact". It now publishes that flag as
// `pitcher_projected` on the row (pitcherFact above reads it, per row); this
// gate asks the other question -- is the whole column yesterday's.
//
...
export async function hotStretchPicks(rows, day, { window = 'month', exclude } = {}) {
  ...
  for (const r of Array.isArray(rows) ? rows : []) {
    ...
    shortlist.push({ pid, name: txt(r.name), team: txt(r.team) || null, pitcher: pitcherFact(r) || null, heat })
  }
...
export function hotStretchText(pick, { day = '', site = '', handle = '', window = 'month' } = {}) {
  ...
  const out = body.join('\n')
  if (out.length <= postLimit()) return out
  ...
  return shorter.length <= postLimit() ? shorter : ''
}

// ── THE TWO BIG POSTS (2026-09-18) ──────────────────────────────────────────
//
// Donovan, marking up the post inventory: "hotcontact_mid and dangercombos_mid
// -- the same post twice... funfacts, birthday -- nothing here is yours, make
// one tweet... storyline_watch_2/3/4 -- four slots of the same format...
// milestone_mid -- combine. add more to the tweet, this is what im saying,
// like the storylines and stuff can be a really big tweet or two."
//
// So: not a cull, a MERGE. Eleven thin posts become two thick ones.
//
//   TONIGHT'S ANGLES   the arms, the parks, the matchup lines, streaks,
//                      milestones, birthdays, revenge games
//   THE HOT SHEET      the hot zone, the kill list, back-to-back, HR leaders
//
// SECTIONED, AND IT SHRINKS BY SECTION. A flat shrinkToFit would eat the last
// section whole to save four characters. This drops the LAST LINE OF THE
// LONGEST section instead, repeatedly, so a long post degrades evenly and a
// short one still carries every heading. A section that runs out of lines
// comes off entirely rather than leaving a naked title (rule #24 -- never a
// heading with nothing under it).
//
// THE LIMIT IS A PARAMETER, not a constant. These are written for X Premium's
// long posts; lib/dash/xPost.js attempts the long version and falls back to
// 280 if this app's tier refuses, so the same builder serves both worlds.
export const BIG_LIMIT = 900
// The two big posts follow the same dial, and never shrink below it.
const bigLimit = () => Math.max(BIG_LIMIT, postLimit())

function fitSections(head, sections, tail, limit = bigLimit()) {
  // EVERY SECTION IS CAPPED BEFORE THE SHRINK EVER RUNS. Without this, the
  // first section simply eats the post: the arms alone had ten qualifying
  // names on the live board and filled all 900 characters, leaving the parks,
  // the milestones, the streak and the revenge games rendering as nothing.
  // A post called TONIGHT'S ANGLES that shows one angle is not the post.
  const live = sections
    .map((s) => ({ title: s.title, lines: (s.lines || []).filter(Boolean).slice(0, s.max || 3) }))
    .filter((s) => s.lines.length)
  // Nothing to say is NOT a headline on its own. Without this an empty slate
  // renders as the bare head, which fits the limit, claims the day's slot and
  // posts a title with no post under it.
  ...
  while (out.length > limit && guard < 400) {
    guard += 1
    // THE LAST SECTION PAYS FIRST. Sections are passed in priority order, so
    // shrinking from the bottom keeps the strongest one whole.
    //
    // The first version of this popped from the LONGEST section instead, which
    // looked fair and was wrong: rendered at 270 it hollowed out the arms, the
    // matchups and the parks down to nothing and kept three one-line
    // afterthoughts. Measured before and after on the same board -- that is
    // the entire reason this comment exists.
    const at = live.length - 1
    live[at].lines.pop()
    if (!live[at].lines.length) live.splice(at, 1)
    if (!live.length) return ''
    out = render()
  }
...
/**
 * TONIGHT'S ANGLES — every "here is something interesting about tonight"
 * source in one post. Each argument is the picks array its own former post
 * already produced, so nothing here re-computes or re-defines anything.
 */
export function anglesText({
  arms = [], parks = [], matchups = [], streak = null, milestones = [], birthdays = [], revenge = null,
} = {}, { day = '', site = '', handle = '', limit = bigLimit() } = {}) {
  const tail = [site, handle].filter(Boolean).join(' · ')
  const head = `⚾ TONIGHT'S ANGLES${day ? ` — ${day.slice(5).replace('-', '/')}` : ''}`
  const rg = revenge || {}
  const sections = [
    { title: 'THE ARMS GETTING HIT', max: 4, lines: arms.map((p) => `${p.pitcher}${p.team ? ` (${p.team})` : ''} ${Number(p.l3hr9).toFixed(2)} HR/9 L3`) },
    { title: 'THE MATCHUPS', max: 3, lines: (matchups || []).map((s) => s?.text).filter(Boolean) },
    { title: 'BEST AIR', max: 2, lines: parks.map((g) => `${g.venue} · ${g.matchup}${g.label ? ` · ${g.label}` : ''}`) },
    { title: 'CLOSING IN', max: 3, lines: milestones.map((p) => `${p.name}${p.team ? ` (${p.team})` : ''} · ${p.need} from ${p.t.toLocaleString('en-US')} ${p.word}`) },
    { title: 'STREAK', max: 1, lines: streak?.text ? [streak.text] : [] },
    { title: 'REVENGE GAMES', max: 3, lines: (rg.revenge || []).map((r) => `${r.name}${r.team ? ` (${r.team})` : ''} vs ${r.opp} · ${r.span}`) },
    { title: 'BIRTHDAYS', max: 2, lines: birthdays.map((b) => `${b.name}${b.team ? ` (${b.team})` : ''} turns ${b.age}`) },
  ]
  return fitSections(head, sections, tail, limit)
}

/**
 * THE HOT SHEET — every ranked-list post in one. Same picks arrays their own
 * former posts used.
 */
export function hotSheetText({
  hot = [], danger = [], b2b = [], leaders = [], dow = '',
} = {}, { day = '', site = '', handle = '', limit = bigLimit() } = {}) {
  const tail = [site, handle].filter(Boolean).join(' · ')
  const head = `🔥 THE HOT SHEET${day ? ` — ${day.slice(5).replace('-', '/')}` : ''}`
  const sections = [
    { title: 'HOTTEST CONTACT', max: 4, lines: hot.map((p) => `${p.name} (${p.team || '?'}) ${p.blastPct}% vs ${p.pitcher}`) },
    { title: 'HOT BAT vs HITTABLE ARM', max: 3, lines: danger.map((p) => `${p.name} (${p.team || '?'}) ${p.blastPct}% vs ${p.pitcher} ${p.pitcherHrBbePct}% HR/BBE`) },
    { title: 'WENT DEEP LAST TIME OUT', max: 4, lines: b2b.map((p) => { const l5 = l5Line(p); return `${p.name}${p.team ? ` (${p.team})` : ''}${l5 ? ` · ${l5}` : ''}` }) },
    { title: dow ? `MOST HR ON ${dow.toUpperCase()}S (LAST 5)` : 'HR LEADERS', max: 4, lines: leaders.map((p) => `${p.name}${p.team ? ` (${p.team})` : ''} ${p.hr} HR · L5 ${p.l5}`) },
  ]
  return fitSections(head, sections, tail, limit)
}
