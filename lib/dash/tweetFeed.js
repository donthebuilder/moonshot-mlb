...
export function milestoneText(picks, { day = '', site = '', handle = '', wave = 'am' } = {}) {
  ...
  const tail = [site, handle].filter(Boolean).join(' · ')
  const head = wave === 'mid'
    ? `🏁 MILESTONE WATCH — STILL CHASING${day ? ` · ${day.slice(5).replace('-', '/')}` : ''}`
    : `🏁 MILESTONE WATCH${day ? ` — ${day.slice(5).replace('-', '/')}` : ''}`
  const lines = picks.slice(0, 6).map((p) =>
    `${p.name}${p.team ? ` (${p.team})` : ''} is ${p.need} away from ${p.t.toLocaleString('en-US')} ${p.word}${p.need === 1 ? ' — could land today' : ''}`)
  return shrinkToFit(head, lines, tail, { spaced: true })
}
...
export function storylineWatchText(picks, { day = '', site = '', handle = '', slot = 1 } = {}) {
  if (!Array.isArray(picks) || !picks.length) return ''
  const tail = [site, handle].filter(Boolean).join(' · ')
  const head = `📖 STORYLINE WATCH${day ? ` — ${day.slice(5).replace('-', '/')}` : ''}`
...
export function revengeGiveawayText({ revenge = [], giveaways = [] } = {}, { day = '', site = '', handle = '' } = {}) {
  const lines = [
    ...revenge.slice(0, 4).map((r) => `😤 ${r.name}${r.team ? ` (${r.team})` : ''} faces his old team, wore ${r.opp} in ${r.span} — revenge games are theater, and theater sells.`),
    ...giveaways.slice(0, 3).map((g) => `🎁 ${g.home}: ${g.name}${g.star ? ` — ${g.star.name}'s own night` : ''}`),
  ]
  if (!lines.length) return ''
  const tail = [site, handle].filter(Boolean).join(' · ')
  const head = `😤 REVENGE & GIVEAWAYS${day ? ` — ${day.slice(5).replace('-', '/')}` : ''}`
  return shrinkToFit(head, lines, tail, { spaced: true })
}
