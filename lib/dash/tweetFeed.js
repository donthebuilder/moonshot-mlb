...
export function theFourText(picks, { day = '', site = '', handle = '' } = {}) {
  ...
  const tail = [site, handle].filter(Boolean).join(' · ')
  const head = `🎯 THE FOUR`
  // 2026-09-18 (Donovan: "the four should have all the players written out not
  // just 3"). THE FOUR is four categories by definition -- a post that silently
  // drops one is failing at its own name. Measured against the live board: the
  // L5 slash line ran each row to 60-70 characters and only three ever fit.
  // The categories and the scores are the point here; L5 has its own post.
  const lines = picks.map((p) => `${p.key}: ${p.name} vs ${p.pitcher} ${p.score}`)
  return shrinkToFit(head, lines, tail, { spaced: true })
}
