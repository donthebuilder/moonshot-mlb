...
// PRIORITY — what survives when more happens in one tick than a lock screen
// should carry. 0 is the thing the person followed that player FOR and is
// never held or dropped; everything else can be bundled behind it or wait for
// the next quiet window. The sender sorts on this BEFORE it caps, because an
// unsorted cap silently throws away the home run and keeps three notices
// about a game starting.
...
export function mlbEventsFrom(snap, day) {
  ...
  for (const [id, line] of Object.entries(snap.lines)) {
    ...
    out.push({
      ...
      // For a bundle: the name alone, because five lines that all end in
      // 'goes yard' read as noise where five names read as news.
      short: `${line?.name || 'Your guy'}${hr > 1 ? ` (${hr})` : ''}`,
      group: 'went deep',
    })
  }
  return out
