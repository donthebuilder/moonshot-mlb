 ...
 */
export function mlbEventsFrom(snap, day, audience, lineupState) {
  if (!snap) return []
  const out = []
  ...
  for (const g of games) {
    ...
    for (const side of ['home', 'away']) {
      for (const row of (g.lineup?.[side] || [])) {
        ...
        // that happens once.
        //
        // HONEST LIMIT, STATED ONCE: this table has no memory before this
        // category shipped. The first night it runs, every follow already
        // starting claims the key for the first time and reads as "new" even
        // though he obviously is not. That is a one-night, self-clearing
        // cost, not a standing bug -- after that first pass, only a genuine
        // new face has an unclaimed key left to claim.
        out.push(mlbEvent({
          key: `mlb:everlineup:${id}`,
          category: 'callup', priority: 1, url: mlbPlayerUrl(id),
          title: `\u{1F195} ${lastName(name)} just made a posted lineup`,
          body: `First time we've seen him in one \u{00B7} batting ${num(row?.slot) || '?'} tonight`,
  ...
  if (lineupState) {
    for (const [id, seen] of Object.entries(lineupState)) {
      ...
      out.push(mlbEvent({
        key: `mlb:${day}:${id}:dropout`,
        category: 'dropout', priority: 1, url: mlbPlayerUrl(id),
        title: `\u{1F4E4} ${lastName(seen.name)} is out of the lineup`,
        body: `His team's card is up and he is not on it`,
  ...
  for (const [id, line] of Object.entries(lines)) {
    ...
    if (hr >= 1) {
      out.push(mlbEvent({
        ...
        title: `\u{1F4A5} ${name} goes yard${hr > 1 ? `, ${hr} tonight` : ''}`,
  ...
  for (const g of games) {
    ...
    if (deckId && follows(audience, deckId)) {
      ...
      out.push(mlbEvent({
        key: `mlb:${day}:${deckId}:deck:${g.pk}:${num(g?.inning)}`,
    ...
    if (loaded && slamId && follows(audience, slamId)) {
      ...
      out.push(mlbEvent({
        ...
        key: `mlb:${day}:${slamId}:slam:${g.pk}:${num(g?.inning)}:${outs}`,
        category: 'slam', priority: 0, url: mlbPlayerUrl(slamId),
        title: `\u{1F9E8} ${slamName} up, bases loaded`,
        body: `${outs} out${outs === 1 ? '' : 's'}${inn ? ` \u{00B7} ${inn}` : ''} \u{00B7} ${num(g?.awayScore)}-${num(g?.homeScore)}`,
  ...
  for (const g of games) {
    ...
    if (g.state === 'Live' && num(g?.inning) >= 7 && Math.abs(hs - as) <= 3) {
      out.push(mlbEvent({
        key: `mlb:${day}:${g.pk}:score:${hs}-${as}`,
        category: 'leadchg', priority: 2, playerIds: ids,
        title: ours > theirs
          ? `\u{1F53C} ${whose} lead ${line}`
          : ours < theirs ? `\u{1F53D} ${whose} trail ${line}` : `\u{2696}\u{FE0F} Tied ${line}`,
    ...
    if (g.settled) {
      out.push(mlbEvent({
        key: `mlb:${day}:${g.pk}:final`,
        category: 'gamefinal', priority: 3, playerIds: ids,
        title: ours > theirs
          ? `\u{1F3C1} ${whose} win ${line}`
          : ours < theirs ? `\u{1F3C1} ${whose} lose ${line}` : `\u{1F3C1} ${whose} tie ${line}`,
        body: who.length > 2 ? `${who.length} of your names in this one` : who.join(', '),
  ...
  for (const g of games) {
    ...
    if (g.redZone === true && g.possession) {
      ...
      if (threatening.length) {
        ...
        out.push(nflEvent({
          key: `nfl:${day}:${g.game_id}:red:${txt(g.possession)}:${num(g.period)}`,
          category: 'nflred', priority: 0,
          playerNames: threatening.map((l) => txt(l?.name)),
          title: `\u{1F6A9} ${txt(g.possession)} in the red zone`,
          body: `${who.slice(0, 3).join(', ')}${txt(g.downDistance) ? ` \u{00B7} ${txt(g.downDistance)}` : ''}${txt(g.clock) ? ` \u{00B7} ${txt(g.clock)}` : ''}`,
  ...
  for (const line of lines) {
    ...
    if (tds >= 1) {
      out.push(nflEvent({
        ...
        title: `\u{1F3C8} ${last} scores${tds > 1 ? `, ${tds} today` : ''}`,
...
const DEFAULTS = {
  ...
  // itself, priority 0.
  nflkick: true,
  // Ships ON as of 2026-09-06 (Alert Box Score) -- push only, not mirrored to
  // Discord (see discordAlerts.js's PUSH_ONLY).
  multihit: true,
  // Ships ON as of 2026-09-06 (Alert Box Score) -- the call-up/first-lineup
  // signal `lineup` itself no longer tries to be (see its producer above).
  callup: true,
  // Same pass, call-up's mirror -- needs lineupState to fire at all (see the
  // DROPPED FROM THE LINEUP section above), so it is silent wherever that
  // isn't wired up (tests, or a caller that doesn't pass it) regardless of
  // this default.
  dropout: true,
