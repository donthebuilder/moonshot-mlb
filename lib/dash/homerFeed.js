...
export function moonshotBoardRanking(rows) {
  ...
  for (const r of (Array.isArray(rows) ? rows : [])) {
    ...
    if (!Number.isFinite(Number(r?.overall_score))) continue
    seen.add(id)
    out.push(r)
  }
  out.sort((a, b) => num(b?.overall_score) - num(a?.overall_score))
  return out.map((r, i) => ({
    rank: i + 1,
    player_id: txt(r.player_id),
    name: txt(r.name),
    team: txt(r.team) || null,
    score: Math.round(num(r.overall_score) * 10) / 10,
...
export function marketForRole(role) {
  const up = String(role || '').toUpperCase()
  for (const k of ['TOP', 'HR', 'HIT', 'HRR']) {
    if (new RegExp(`\\b${k}\\b`).test(up)) return { role: k, ...NEIGHBOR_MARKETS[k] }
  }
  return null
}

export function boardNeighbors(rows, playerId, span = 2, scoreKey = 'hr_score') {
  const id = txt(playerId)
  if (!id) return []
  const seen = new Set()
  const ranked = []
  for (const r of (Array.isArray(rows) ? rows : [])) {
    ...
    if (!Number.isFinite(Number(r?.[scoreKey]))) continue
    seen.add(pid)
    ranked.push(r)
  }
  ranked.sort((a, b) => num(b?.[scoreKey]) - num(a?.[scoreKey]))
  const at = ranked.findIndex((r) => txt(r.player_id) === id)
  if (at < 0) return []
  // Clamped, not wrapped: #1 going deep shows the two BELOW him and no
  // phantom rows above, rather than a window padded to a fixed size.
  const from = Math.max(0, at - span)
  const to = Math.min(ranked.length, at + span + 1)
  return ranked.slice(from, to).map((r, i) => ({
    rank: from + i + 1,
    total: ranked.length,
    player_id: txt(r.player_id),
    name: txt(r.name),
    team: txt(r.team) || null,
    score: Math.round(num(r[scoreKey]) * 10) / 10,
    him: txt(r.player_id) === id,
  }))
}

export function boardNeighborsText(neighbors, { site = '', handle = '', market = 'HR' } = {}) {
  if (!Array.isArray(neighbors) || neighbors.length < 2) return ''
  const him = neighbors.find((n) => n.him)
  if (!him) return ''
  const tail = [site, handle].filter(Boolean).join(' · ') || null
  // The market is named because it is the honest answer to "which board?"
  // A HIT call going deep sat 4th among HIT calls; saying just "calls" would
  // imply the homer board, which he was not on.
  const head = `📊 WHERE HE SAT AMONG TONIGHT'S ${market} CALLS`
  // The arrow, not bold or caps: X strips formatting and SHOUTING ONE ROW in a
  // column of five reads as a different kind of row rather than the same kind,
  // highlighted.
  const rows = neighbors.map((n) =>
    `${n.him ? '→ ' : '   '}${n.rank}. ${n.name}${n.team ? ` (${n.team})` : ''} · ${n.score}`)
  // The denominator is the point: "#12 of 30 calls" tells a stranger both
  // where he sat AND how selective the board is, in five words.
  const word = `${market} call${him.total === 1 ? '' : 's'}`
  const closer = him.rank === 1
    ? `Top of ${him.total} ${word}. It landed.`
    : `#${him.rank} of ${him.total} ${word}. It landed.`
  for (let n = rows.length; n >= 2; n -= 1) {
    // Trim from the OUTSIDE in, keeping him centred -- dropping from the end
...
// EVERY name the board actually called, not just the ten that fit the post.
// The pregame TWEET stays at ten (pregamePicks above) -- but the receipt
// quote on a homer was keyed to that same ten-name list, so a WATCH /
// CONTACT / HIT / HRR name going deep posted as an orphan tweet with no
// proof attached. Last board: 91 rows carried a role and only 10 could ever
// quote. This is stored alongside `picks` in the pregame payload purely so
// quoteFor() in the tick route can check it; it never reaches a post, so it
// costs nothing on the X budget and does not change what the tweet says.
export function pregameCalled(rows) {
  const seen = new Set()
  for (const r of Array.isArray(rows) ? rows : []) {
    const id = txt(r?.player_id)
    if (id && isCalledRole(r?.game_pick_role)) seen.add(String(id))
  }
  return [...seen]
}
...
export function boardRolePicks(rows) {
  ...
  for (const role of ['TOP', 'HR', 'HIT', 'HRR']) {
    ...
    const best = list
      ...
      .sort((a, b) => num(b?.[scoreKey]) - num(a?.[scoreKey]))[0]
    if (best) {
      out.push({
...
export function boardRoleText(picks, { day = '', site = '', handle = '' } = {}) {
  ...
  const evidence = (p) => {
    const bits = []
    if (p.score != null) bits.push(`${p.role} score ${p.score}`)
  ...
  const build = (withEvidence) => {
    ...
    return ['🎯 TONIGHT’S BOARD', '', ...lines, '', 'CALLED IT', tail ? '' : null, tail]
  }
...
// Never hides a result -- every pick shown gets ✅, ❌, or ➖ (game postponed,
// he didn't play, or 0 at-bats: not judgeable, not a miss), same "no pick
// filtered out" rule accountabilityText already follows above.
export function boardRoleResultsText(picks, lines, { day = '', site = '', handle = '' } = {}) {
  if (!Array.isArray(picks) || !picks.length) return ''
  const tail = [site, handle].filter(Boolean).join(' · ') || null
  // 2026-09-18 (Donovan: "all the tweet should be full"). The grade used to be
  // a tick and a name -- the ONE post on the account whose whole job is proof,
  // showing none of it. The line is already in hand (it is what pickCleared
  // graded against), so printing it costs nothing and makes every ✅ and ❌
  // checkable by anyone reading. A ➖ has no line by definition.
  const fits = (l) => l.filter((x) => x != null).join('\n').length <= postLimit()
  const statLine = (ln) => {
    if (!ln) return ''
    const bits = [`${ln.h}-for-${ln.ab}`]
    if (ln.hr) bits.push(`${ln.hr} HR`)
    if (ln.rbi) bits.push(`${ln.rbi} RBI`)
    if (!ln.hr && ln.r) bits.push(`${ln.r} R`)
    return bits.join(', ')
  }
  const build = (withLine) => picks.map((p) => {
    const ln = (lines || {})[String(p.player_id)]
    const cleared = pickCleared(p.role, ln)
    const mark = cleared === true ? '✅' : cleared === false ? '❌' : '➖'
    const head = `${mark} ${p.role}: ${p.name}${p.team ? ` (${p.team})` : ''}`
    const st = withLine ? statLine(ln) : ''
    return st ? `${head} — ${st}` : head
  })
  const wrap = (rows) => ['📋 BOARD RESULTS', '', ...rows, '', 'CALLED IT', tail ? '' : null, tail]
  const rich = wrap(build(true))
  const body = fits(rich) ? rich : wrap(build(false))
  return body.filter((l) => l != null).join('\n')
}
...
export function numerologyMoment(rows) {
  const list = (Array.isArray(rows) ? rows : [])
    .map((r) => {
      const jersey = fnum(r?.stats?.jersey)
      const seasonHr = fnum(r?.stats?.season_hr)
      const nth = seasonHr != null ? seasonHr + num(r?.hr_n) : null
      return {
    ...
    .filter((r) => r.jersey != null && r.nth != null && r.nth > 0)
...
export function weeklyText(rows, { from = '', to = '', site = '', handle = '' } = {}) {
  ...
  const tail = [site, handle].filter(Boolean).join(' · ') || null
  const lines = [
    '📅 MOONSHOT WEEK IN REVIEW', '',
    `${c.called} / ${c.total} HR called`,
    `${c.pct ?? 0}% of the week's homers`,
  ]
  if (avg != null) lines.push('', `Average called price: ${fmtOdds(avg)}`)
  if (best) lines.push('', `Longest call: ${fmtOdds(best.odds_over)}`)
  if (tail) lines.push('', tail)
  return lines.join('\n')
}
...
export function monthlyText(rows, { month = '', site = '', handle = '' } = {}) {
  ...
  const tail = [site, handle].filter(Boolean).join(' · ') || null
  const lines = [
    `📆 MOONSHOT — ${(month || 'THIS MONTH').toUpperCase()}`, '',
    `${c.called} / ${c.total} HR called`,
    `${c.pct ?? 0}% of the month's homers`,
    '',
    `TOP: ${top}`,
    `HR: ${hr}`,
    `HR Watch: ${watch}`,
...
/**
 * RESULTS / ACCOUNTABILITY — last night's ten pregame picks, graded against
 * what actually went deep. `picks` is the exact array the pregame post
 * itself persisted (homer_feed_posts.payload.picks for that day) -- no new
 * table, just a read-back one day later. `hitIds` is a Set (or array) of
 * player_ids that appear anywhere in homer_feed for that same day. Never
 * hides a miss -- every pick shown gets a ✅ or a ❌, nothing filtered out.
 */
// LEAD WITH THE RECORD (2026-09-13). This used to open with "Last night's
// calls:" and then drop names off the END until the post fit 270 characters,
// with nothing saying anything had been cut.
//
// That is not a formatting detail on this particular post. Picks arrive in
// ranked order, so the names cut are always the bottom of the card — the ones
// that hit least often — which means the truncation SYSTEMATICALLY FLATTERS
// the record on the one post whose whole purpose is honesty. Measured on the
// real 2026-09-12 card: 8 of 10 picks fit, both dropped names were misses, and
// a reader counting checkmarks saw 3 of 8 where the truth was 3 of 10.
//
// So the true record is computed over EVERY pick and printed on its own line
// above the list, where nothing can truncate it, and a cut is stated outright
// when one happens. The names are now the supporting detail; the number is the
// post.
export function accountabilityText(picks, hitIds, { day = '', site = '', handle = '' } = {}) {
  if (!Array.isArray(picks) || !picks.length) return ''
  const hit = hitIds instanceof Set ? hitIds : new Set((hitIds || []).map(String))
  const tail = [site, handle].filter(Boolean).join(' · ') || null
  const fits = (lines) => lines.filter((l) => l != null).join('\n').length <= postLimit()
  const rows = picks.map((p) => `${hit.has(String(p.player_id)) ? '✅' : '❌'} ${p.name}${p.team ? ` (${p.team})` : ''}`)
  const won = picks.filter((p) => hit.has(String(p.player_id))).length
  const record = `${won} for ${picks.length} last night.`

  for (let n = rows.length; n >= 1; n -= 1) {
    const restN = rows.length - n
    // Name the cut rather than let the list imply the card was this short.
    const more = restN
      ? `+${restN} more not shown — full card on the site.`
      : null
    const body = [
      '📋 MOONSHOT RESULTS', '',
      record, '',
      ...rows.slice(0, n),
      more ? '' : null, more,
      '', 'The board is back tonight.',
      tail ? '' : null, tail,
    ]
    if (fits(body)) return body.filter((l) => l != null).join('\n')
  }
  // Even with no room for a single name, the record still goes out. It is the
  // part that matters and it is two lines long.
  return ['📋 MOONSHOT RESULTS', '', record, tail ? '' : null, tail]
...
export function botPollText({ site = '', handle = '' } = {}) {
  const tail = [site, handle].filter(Boolean).join(' · ') || null
  const lines = ['🤖 THE BOT VS THE PEOPLE', '', 'Who goes deep tonight?']
  if (tail) lines.push('', tail)
  return lines.join('\n')
}
...
export function postText(ev, { site = '', handle = '' } = {}) {
  ...
  const lines = ['🤖 CALLED IT', '', `${name.toUpperCase()} GOES DEEP.${nth}`]
  ...
  if (callStatus(ev) === 'called') {
    ...
    lines.push('', rank ? `#${rank} on the Moonshot board` : 'On the Moonshot board', `${roleWord(ev.role)}${price}`)
    ...
    lines.push('', 'Not on the Moonshot board.', '', "Some moments can't be modeled.", '', "That's baseball.")
  }
