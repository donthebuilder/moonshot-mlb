...
export function pregameText(picks, { day = '', site = '', handle = '' } = {}) {
  ...
  if (picks.length && picks.length <= 5) {
...
export function boardRoleText(picks, { day = '', site = '', handle = '' } = {}) {
  ...
  const tail = [site, handle].filter(Boolean).join(' · ') || null
  const lines = picks.map((p) => `${p.role}: ${p.name}${p.team ? ` (${p.team})` : ''}${p.opponent ? ` vs ${p.opponent}` : ''}`)
  const body = ['🎯 TONIGHT’S BOARD', '', ...lines, '', 'CALLED IT', tail ? '' : null, tail]
...
export function boardRoleResultsText(picks, lines, { day = '', site = '', handle = '' } = {}) {
  ...
  const rows = picks.map((p) => {
    const cleared = pickCleared(p.role, (lines || {})[String(p.player_id)])
    const mark = cleared === true ? '✅' : cleared === false ? '❌' : '➖'
    return `${mark} ${p.role}: ${p.name}${p.team ? ` (${p.team})` : ''}`
  })
  const body = ['📋 BOARD RESULTS', '', ...rows, '', 'CALLED IT', tail ? '' : null, tail]
  return body.filter((l) => l != null).join('\n')
}
...
export function pairsToWatch(rows, pairs, odds, day, limit = 3) {
  const list = Array.isArray(rows) ? rows : []
