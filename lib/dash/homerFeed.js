...
export function pregameText(picks, { day = '', site = '', handle = '' } = {}) {
  ...
  const head = `🎯 THE CALL SHEET${day ? ` — ${day.slice(5).replace('-', '/')}` : ''}`
  ...
  for (let n = compact.length; n >= 1; n -= 1) {
    // No "+N more on the card" here either (2026-09-07, Donovan) -- same
    // reasoning as shrinkToFit in lib/dash/tweetFeed.js. The card posted
    // alongside this text always shows the full ten regardless.
    const line = compact.slice(0, n).join(', ')
    for (const withCloser of [true, false]) {
...
export function pairsToWatchText(hits, { day = '', site = '', handle = '' } = {}) {
  ...
  const head = `👥 THE PAIR TRAP${day ? ` — ${day.slice(5).replace('-', '/')}` : ''}`
  const tail = [site, handle].filter(Boolean).join(' · ')
...
export function longshotText(pick, { day = '', site = '', handle = '' } = {}) {
  if (!pick) return ''
  const tail = [site, handle].filter(Boolean).join(' · ')
  const head = `🚀 THE MOONSHOT${day ? ` — ${day.slice(5).replace('-', '/')}` : ''}: ${pick.name}${pick.team ? ` (${pick.team})` : ''} at ${fmtOdds(pick.over)} (${pick.book})${pick.opponent ? ` to go deep vs ${pick.opponent}` : ''}. 🤖`
  return [head, tail].filter(Boolean).join('\n')
}
...
export function numerologyText(moment, { day = '', site = '', handle = '' } = {}) {
  ...
  if (moment.tier === 'trifecta') {
    ...
    head = `🔢 ${p.name}${p.team ? ` (${p.team})` : ''} went deep tonight${when} wearing #${p.jersey} — his jersey, his season HR count, and his birthday all reduce to ${moment.root}.`
  } else if (moment.tier === 'jersey') {
    const names = moment.players.map((p) => `${p.name}${p.team ? ` (${p.team})` : ''}`).join(' and ')
    head = `🔢 Same number, same night${when} — ${names} both wore #${moment.jersey} and both went deep.`
  } else {
    const shown = moment.players.slice(0, 4).map((p) => p.name)
    const rest = moment.players.length - shown.length
    head = `🔢 ${moment.players.length} homers tonight${when}, one jersey digit root (${moment.root}): ${shown.join(', ')}${rest > 0 ? `, +${rest} more` : ''}.`
  }
  return [head, tail].filter(Boolean).join('\n')
}
