...
export function boardNeighbors(rows, playerId, span = 2) {
  ...
  for (const r of (Array.isArray(rows) ? rows : [])) {
    ...
    if (!Number.isFinite(Number(r?.hr_score))) continue
    seen.add(pid)
    ranked.push(r)
  }
  ranked.sort((a, b) => num(b?.hr_score) - num(a?.hr_score))
  ...
  return ranked.slice(from, to).map((r, i) => ({
    ...
    name: txt(r.name),
    team: txt(r.team) || null,
    score: Math.round(num(r.hr_score) * 10) / 10,
    him: txt(r.player_id) === id,
  }))
}

export function boardNeighborsText(neighbors, { site = '', handle = '' } = {}) {
  ...
  const head = "📊 WHERE HE SAT AMONG TONIGHT'S CALLS"
  ...
  const closer = him.rank === 1
    ? `Top of ${him.total} calls. It landed.`
    : `#${him.rank} of ${him.total} calls. It landed.`
