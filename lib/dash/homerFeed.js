...
export function partnerFor(pairs, id, name) {
  ...
  for (const p of list) {
    ...
    if (!best || count > best.count) {
      best = { id: other.player_id != null ? String(other.player_id) : null, name: txt(other.name || other.player_name), team: txt(other.team) || null, count, sameGame: num(p?.same_game_hr_count) }
    }
  }
  return best
}
...
export function hooksFor(ev, ctx = {}) {
  ...
  if (partner) {
    const both = partner.id && ctx.todayIds?.has(String(partner.id))
    out.push(`👥 Same-day partner: ${partner.name}${partner.team ? ` (${partner.team})` : ''} — ${partner.count}x this season${both ? ' · both went deep tonight' : ''}`)
  }
  ...
  if (seasonHr != null && jersey != null) {
    const nth = seasonHr + num(ev?.hr_n)
    if (nth > 0 && digitRoot(nth) === digitRoot(jersey)) {
