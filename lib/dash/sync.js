...
// 5 MINUTES, NOT 45 SECONDS (2026-09-24). Supabase egress: every visible
// signed-in tab called /api/dash/state 80 times an hour, and each call cost
// two Auth lookups plus every saved value. A tab coming back into view still
// syncs immediately (visibilitychange below), which is the moment it matters.
const POLL_MS = 5 * 60 * 1000
// After the first full pull, ask only for keys changed since the newest stamp
// we have seen, less a margin for clocks that disagree.
const SINCE_MARGIN_MS = 10 * 60 * 1000
let newestSeen = 0
...
registerSyncedKey('mlb_watchlist_v1', { strategy: 'newerWins', event: 'mlb-watchlist-change' })
...
async function pull() {
  let payload
  try {
    const url = newestSeen ? `${ENDPOINT}?since=${encodeURIComponent(new Date(newestSeen - SINCE_MARGIN_MS).toISOString())}` : ENDPOINT
    const res = await fetch(url, { cache: 'no-store', credentials: 'same-origin' })
    if (!res.ok) return false
    payload = await res.json()
  } catch { return false }

  account = { ready: true, signedIn: Boolean(payload?.signedIn), configured: Boolean(payload?.configured), who: payload?.who || account?.who || null }
  emitAccount()
  if (!account.signedIn) { newestSeen = 0; return false }

  const cloud = payload.state || {}
  for (const entry of Object.values(cloud)) {
    const at = Date.parse(entry?.updatedAt || '')
    if (Number.isFinite(at) && at > newestSeen) newestSeen = at
  }
  if (!payload.partial && !newestSeen) newestSeen = Date.now() - SINCE_MARGIN_MS
  for (const [key, entry] of REGISTRY.entries()) {
    const remote = cloud[key]
    // A partial answer only carries keys that changed. Absent here means
    // "unchanged", not "the account has nothing" -- never push over it.
    if (remote === undefined && payload.partial) continue
