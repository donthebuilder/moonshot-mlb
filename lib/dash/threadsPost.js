 ...
 */
export async function mirrorToThreads(text, { replyTo = null, poll = null, imageUrl = null, xId = null } = {}) {
  ...
  const r = await postToThreads(text, { replyTo: parent, imageUrl })
  if (r.ok && xId) rememberThreadsParent(xId, r.id)
  _results.push(r.ok ? { ok: true, id: r.id } : { ok: false, status: r.status, error: r.error })
  return r
}
