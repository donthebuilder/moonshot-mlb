...
function mlbPulse(results, feed = null) {
...
async function buildNetworkPulse() {
  ...
  return {
    ...
    mlb: mlbPulse(results, results?.date ? await mlbSlateState(results.date) : null),
...
export const getNetworkPulse = unstable_cache(buildNetworkPulse, ['network-pulse'], { revalidate: PULSE_TTL })
