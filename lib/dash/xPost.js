 ...
 * from uploadImageToX().
 */
export async function postToX(text, { replyTo = null, mediaId = null } = {}) {
...
/**
 * One Discord webhook message. Free, no quota worth designing around.
 * `imageUrl` rides as an embed so the card shows under the text.
 */
export async function postToDiscord(text, { imageUrl = null } = {}, webhook = clean(process.env.DISCORD_HOMER_WEBHOOK)) {
  if (!webhook) return { ok: false, status: 0, error: 'DISCORD_HOMER_WEBHOOK is not set' }
  try {
