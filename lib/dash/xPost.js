...
export async function getFromX(path, query = {}) {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      return { ok: false, status: res.status, error: json?.detail || json?.title || res.statusText, json, rate }
    }
 ...
 * from uploadImageToX().
 */
export async function postToX(text, { replyTo = null, mediaId = null, quoteId = null, poll = null, kind = null, _retry = false } = {}) {
  const problem = xProblem()
  if (problem) return { ok: false, status: 0, error: problem }
  ...
  try {
    ...
    if (!res.ok) {
      ...
      if (lengthy && clientError && !_retry) {
        ...
        return postToX(fitToLimit(raw, HARD_LIMIT), { replyTo, mediaId, quoteId, poll, kind, _retry: true })
      }
    ...
    if (id) {
      await mirrorToThreads(fitted, { replyTo, poll, xId: id, kind })
 ...
 */
export async function postToDiscord(text, { imageUrl = null, png = null } = {}, webhook = process.env.DISCORD_HOMER_WEBHOOK) {
  ...
  if (imageUrl) payload.embeds = [{ image: { url: imageUrl }, color: 0xf4581f }]
