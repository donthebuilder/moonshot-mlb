 ...
 * from uploadImageToX().
 */
export async function postToX(text, { replyTo = null, mediaId = null, quoteId = null, poll = null, _retry = false } = {}) {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      if (lengthy && clientError && !_retry) {
        ...
        return postToX(fitToLimit(raw, HARD_LIMIT), { replyTo, mediaId, quoteId, poll, _retry: true })
      }
    ...
    return { ok: true, id: json?.data?.id || null, chars: Array.from(fitted).length }
  } catch (err) {
