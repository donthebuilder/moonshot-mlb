...
//
// Starring follows. UN-starring now also unfollows (2026-09-18) — see the note
// on Dashboard.js's `toggleWatch`. The original split ("different intents")
// was defensible on paper and wrong in the hand: nothing in the UI ever
// offered the in-between state, so the only thing the split produced was a
// removal that came back on the next page load. One action, one result.
//
...
// The sport half of the key comes from the registry (lib/routes.js). This was
// `sport === 'nfl' ? 'nfl' : 'mlb'`, which filed a followed LAMP skater as
// `mlb:<id>` -- and pushRules' audienceFrom() reads that prefix, so his id
// joined the MLB push audience (Batch 1, 2026-09-25). Unknown still means mlb.
export const followKey = (sport, playerId) => `${sportKey(sport)}:${playerId}`
...
/**
 * Tombstone every live row for one sport at once — the "clear all" behind
 * Your Players' own button (2026-09-18, Donovan: "where is the clear all
 * button"). Written as a tombstone per player, exactly like `unfollow`,
 * rather than by emptying the store: a bare delete loses the merge against
 * another device's stale copy and hands every name straight back, which is
 * the resurrection this file's own header was written about. Returns how
 * many rows were cleared so the caller can say so.
 */
export function unfollowAll(sport = null) {
  const store = read()
  const now = Date.now()
  let n = 0
  Object.entries(store).forEach(([key, row]) => {
    if (!row || row.removed) return
    if (sport && row.sport !== sport) return
    store[key] = { ...row, removed: true, at: now }
    n += 1
  })
  if (n) write(store)
  return n
...
export function useFollowing(sport = null) {
  ...
  const clear = useCallback(() => unfollowAll(sport || 'mlb'), [sport])

  return { rows, ids, following, toggle, unfollow: drop, clear, count: rows.length }
}
