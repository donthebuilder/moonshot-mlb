 ...
 */
// ── 6 MB EVERY 45 SECONDS, FIXED (2026-09-24 audit, PERF-1) ─────────────
// `?t=Date.now()` + cache:'no-store' made every poll tick re-download every
// payload in full: today_slim (4 MB) + results_live (2.4 MB) + six
// static-per-run files, ~0.8 MB gzipped and a ~6 MB JSON.parse, on every
// tab, 80 times an hour while games were live. raw.githubusercontent serves
// a strong ETag and answers a conditional GET with 304 and zero bytes
// (verified 09-24 on results_live.json: 1.8 MB -> 304 / 0). So the default
// is cache:'no-cache' -- the browser ALWAYS asks the server, sends
// If-None-Match, and only downloads a body that actually changed. The
// trade: the CDN in front of raw holds a copy for up to five minutes, so a
// fresh bot publish can take that long to show on a poll tick. The manual
// refresh button passes { bust: true } and keeps the old unique-URL path,
// which skips the CDN copy entirely.
export async function fetchJSON(paths, validate = null, { bust = false } = {}) {
  let fallback = null
  for (const p of paths) {
    try {
      const url = bust ? `${p}${p.includes('?') ? '&' : '?'}t=${Date.now()}` : p
      const r = await fetch(url, { cache: bust ? 'no-store' : 'no-cache' })
      if (!r.ok) continue
