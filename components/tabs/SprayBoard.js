...
function FenceLine({ playerId }) {
  ...
  const cell = (v, label, col, tip) => (
    <span title={tip} style={{ fontSize: TYPE.micro, fontFamily: NUM_FONT, color: C.text3, cursor: 'default' }}>
      <b style={{ color: col, fontSize: TYPE.body, fontWeight: 900 }}>{v}</b> {label}
    </span>
  )
  return (
    <div style={{
        ...
        <>
          <div style={{
            ...
            <button
              ...
              style={{
                marginLeft: 'auto', padding: '3px 10px', fontSize: TYPE.label, fontWeight: 700,
