...
function HRWatch({ homers }) {
  if (!homers?.length) return null
  const watchHits = homers.filter(h => {
    const tags = Array.isArray(h.tags) ? h.tags : []
    return tags.some(t => String(t || '').toUpperCase().includes('WATCH'))
  })
  if (!watchHits.length) return null
  return (
    <Card style={{ padding: 0, marginBottom: 10, overflow: 'hidden' }}>
      <SectionHeader title={`👀 HR Watch (${watchHits.length})`} color={C.cyan} />
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, padding: '10px 12px' }}>
        {watchHits.map((h, i) => {
          const base = h.base_row || {}
          const multiHR = si(base.actual_hr) > 1
