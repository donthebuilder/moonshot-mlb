'use client'
import { C, NUM_FONT } from '../lib/theme'
import {
  nameOf, teamOf, oppOf, n, clean, pct, sc,
  hrScore, hitScore, prodScore, tbScore, pitchMixScore,
  recent375, recent400, recent350, ihrVal,
  avgEV, maxEV, hardHitRate, barrelRate, launchAngle,
  babipVal, pitcherBabipVal, avgVsRHP, avgVsLHP,
} from '../lib/player''use client'
import { useState } from 'react'
import { C, NUM_FONT } from '../lib/theme'
import {
  nameOf, teamOf, oppOf, n, clean, pct, sc,
  hrScore, hitScore, prodScore, tbScore, pitchMixScore,
  recent375, recent400, recent350, ihrVal,
  avgEV, maxEV, hardHitRate, barrelRate, launchAngle,
  babipVal, pitcherBabipVal, avgVsRHP, avgVsLHP,
} from '../lib/player'
import { compactRole, roleColor, gradeFor, signalPills, bestBet } from '../lib/scoring'
import { Chip } from './ui'
import EVLog from './tabs/EVLog'
import PitchBreakdown from './tabs/PitchBreakdown'
import SprayChart from './SprayChart'
import HotZoneMap from './HotZoneMap'

function Row({ label, value, mono = true }) {
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', padding: '7px 0', borderBottom: `1px solid ${C.border}` }}>
      <span style={{ fontSize: 11, color: C.text3 }}>{label}</span>
      <span style={{ fontSize: 12, color: C.text, fontFamily: mono ? NUM_FONT : 'inherit', fontWeight: 600 }}>{value}</span>
    </div>
  )
}

function TabBtn({ active, onClick, children }) {
  return (
    <button onClick={onClick} style={{
      padding: '5px 13px', fontSize: 11, fontWeight: 700, cursor: 'pointer', borderRadius: 999,
      border: `1px solid ${active ? C.orange : C.border}`,
      background: active ? `${C.orange}22` : 'transparent',
      color: active ? C.orange : C.text3,
      whiteSpace: 'nowrap',
    }}>{children}</button>
  )
}

const BBE_RANGES = [10, 15, 25]

function RangeToggle({ value, onChange }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
      <span style={{ fontSize: 10, color: C.text3, fontFamily: NUM_FONT, whiteSpace: 'nowrap' }}>Last</span>
      <div style={{ display: 'flex', borderRadius: 6, overflow: 'hidden', border: `1px solid ${C.border}` }}>
        {BBE_RANGES.map(n => (
          <button key={n} onClick={() => onChange(n)} style={{
            padding: '3px 7px', fontSize: 10, fontWeight: 600, cursor: 'pointer', border: 'none',
            background: value === n ? C.orange : 'transparent',
            color: value === n ? '#fff' : C.text3,
          }}>{n}BBE</button>
        ))}
      </div>
    </div>
  )
}

const TABS = [
  { key: 'overview', label: 'Overview' },
  { key: 'ev',       label: '⚡ EV Log' },
  { key: 'pitch',    label: '🎯 Pitch' },
  { key: 'spray',    label: '🗺 Spray' },
  { key: 'zones',    label: '🔥 Hot Zones' },
]

export default function PlayerModal({ player, onClose }) {
  const [tab, setTab]             = useState('overview')
  const [bbeRange, setBbeRange] = useState(25)

  if (!player) return null
  const p = player

  const role = compactRole(p)
  const rc = roleColor(role, C)
  const pills = signalPills(p, C, 'hr')
  const b = babipVal(p), pb = pitcherBabipVal(p)
  const weakSide = clean(p?.pitcher_weak_side || p?.weak_side, '')
  const batsHand = clean(p?.bats || p?.handedness, '')
  const matchesWeak = weakSide && batsHand && (
    (weakSide === 'LHB' && batsHand === 'L') ||
    (weakSide === 'RHB' && batsHand === 'R')
  )
  const weakLabel = weakSide
    ? `Weak vs ${weakSide}`
    : p?.pitcher_throws ? (p.pitcher_throws === 'R' ? 'RHP' : 'LHP') : '—'

  // Documented batter-vs-pitch exploit found by the model (pitch_type_match_score > 0).
  // Backtested separator: HR_PICKS with this flag hit 23.9% vs 9.5% without it
  // across 22 days / 241 picks -- the single largest gap found in that analysis.
  const hasMatchupEdge = n(p?.pitch_type_match_score, 0) > 0

  const wideTab = tab !== 'overview'
  const modalWidth = wideTab ? 900 : 480

  return (
    <div
      onClick={onClose}
      style={{
        position: 'fixed', inset: 0, zIndex: 100,
        background: 'rgba(0,0,0,.75)', backdropFilter: 'blur(6px)',
        display: 'flex', alignItems: 'center', justifyContent: 'center',
        padding: 16,
      }}
    >
      <div
        onClick={e => e.stopPropagation()}
        style={{
          background: C.bg2, border: `1px solid ${C.border2}`, borderRadius: 18,
          width: modalWidth, maxWidth: '100%', maxHeight: '90vh', overflowY: 'auto',
          transition: 'width .15s',
        }}
      >
        <div style={{ padding: '18px 20px 22px' }}>

          {/* header */}
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
            <div>
              <div style={{ fontSize: 19, fontWeight: 900 }}>{nameOf(p)}</div>
              <div style={{ fontSize: 11, color: C.text3, fontFamily: NUM_FONT, marginTop: 3 }}>
                {teamOf(p)} vs {oppOf(p)} · Lineup #{clean(p?.lineup_spot, '?')} · {clean(p?.handedness || p?.bats, '?')}HB
                {p?.pitcher_name && <span> · vs {p.pitcher_name} ({p.pitcher_throws}HP)</span>}
              </div>
            </div>
            <button onClick={onClose} style={{ background: 'transparent', border: 'none', color: C.text3, fontSize: 22, cursor: 'pointer', lineHeight: 1 }}>✕</button>
          </div>

          {/* chips */}
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 12 }}>
            <Chip color={rc}>{role}</Chip>
            <Chip color={C.text2}>{bestBet(p, 'hr')}</Chip>
            <Chip color={C.text2}>Grade {gradeFor(p, 'hr')}</Chip>
            {hasMatchupEdge && <Chip color={C.orange}>🎯 Matchup Edge</Chip>}
            {pills.map((x, i) => <Chip key={i} color={x.color}>{x.label}</Chip>)}
          </div>

          {/* tab bar + range toggle */}
          <div style={{
            display: 'flex', justifyContent: 'space-between', alignItems: 'center',
            gap: 8, marginBottom: 16, borderBottom: `1px solid ${C.border}`, paddingBottom: 10,
            flexWrap: 'wrap',
          }}>
            <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap' }}>
              {TABS.map(t => (
                <TabBtn key={t.key} active={tab === t.key} onClick={() => setTab(t.key)}>{t.label}</TabBtn>
              ))}
            </div>
            {tab !== 'overview' && (
              <RangeToggle value={bbeRange} onChange={setBbeRange} />
            )}
          </div>

          {/* overview */}
          {tab === 'overview' && (
            <>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0 24px', marginBottom: 14 }}>
                <div>
                  <div style={{ fontSize: 10, color: C.text3, fontWeight: 800, textTransform: 'uppercase', letterSpacing: .5, padding: '8px 0 4px' }}>Model Scores</div>
                  <Row label="HR Score"  value={hrScore(p).toFixed(1)} />
                  <Row label="HRR Score" value={prodScore(p).toFixed(1)} />
                  <Row label="Hit Score" value={hitScore(p).toFixed(1)} />
                  <Row label="TB Score"  value={tbScore(p).toFixed(1)} />
                  <Row label="Pitch Mix" value={pitchMixScore(p).toFixed(1)} />
                </div>
                <div>
                  <div style={{ fontSize: 10, color: C.text3, fontWeight: 800, textTransform: 'uppercase', letterSpacing: .5, padding: '8px 0 4px' }}>Batted Ball</div>
                  <Row label="Avg EV"       value={avgEV(p) ? avgEV(p).toFixed(1) + ' mph' : '—'} />
                  <Row label="Max EV"       value={maxEV(p) ? maxEV(p).toFixed(1) + ' mph' : '—'} />
                  <Row label="Barrel %"     value={pct(barrelRate(p))} />
                  <Row label="Hard Hit %"   value={pct(hardHitRate(p))} />
                  <Row label="Launch Angle" value={launchAngle(p) ? launchAngle(p).toFixed(1) + '°' : '—'} />
                </div>
                <div>
                  <div style={{ fontSize: 10, color: C.text3, fontWeight: 800, textTransform: 'uppercase', letterSpacing: .5, padding: '14px 0 4px' }}>Recent Distance</div>
                  <Row label="350+ count" value={recent350(p)} />
                  <Row label="375+ count" value={recent375(p)} />
                  <Row label="400+ count" value={recent400(p)} />
                  <Row label="Ideal HR %" value={ihrVal(p) ? (ihrVal(p) * 100).toFixed(1) + '%' : '—'} />
                </div>
                <div>
                  <div style={{ fontSize: 10, color: C.text3, fontWeight: 800, textTransform: 'uppercase', letterSpacing: .5, padding: '14px 0 4px' }}>Season</div>
                  <Row label="AVG"    value={clean(p?.season_avg, '—')} />
                  <Row label="HR"     value={clean(p?.season_hr, '—')} />
                  <Row label="PA"     value={clean(p?.season_pa || p?.pa, '—')} />
                  <Row label="K Rate" value={pct(p?.season_k_rate)} />
                  {b > 0 && <Row label="BABIP" value={b.toFixed(3)} />}
                </div>
                <div>
                  <div style={{ fontSize: 10, color: C.text3, fontWeight: 800, textTransform: 'uppercase', letterSpacing: .5, padding: '14px 0 4px' }}>Splits</div>
                  {avgVsRHP(p) > 0 && <Row label="vs RHP" value={avgVsRHP(p).toFixed(3)} />}
                  {avgVsLHP(p) > 0 && <Row label="vs LHP" value={avgVsLHP(p).toFixed(3)} />}
                  <Row label="L5 Hits" value={n(p?.last5_hits, 0)} />
                  <Row label="L5 HR"   value={n(p?.last5_hr, 0)} />
                  <Row label="L5 XBH"  value={n(p?.last5_xbh, 0)} />
                </div>
                <div>
                  <div style={{ fontSize: 10, color: C.text3, fontWeight: 800, textTransform: 'uppercase', letterSpacing: .5, padding: '14px 0 4px' }}>Opposing Pitcher</div>
                  <Row label="Name"   value={clean(p?.pitcher_name, '—')} mono={false} />
                  <Row label="Throws" value={clean(p?.pitcher_throws, '—')} />
                  <Row label="HR/9"   value={sc(p?.pitcher_hr9)} />
                  <Row label="WHIP"   value={sc(p?.pitcher_whip)} />
                  {weakSide && (
                    <div style={{ display: 'flex', justifyContent: 'space-between', padding: '7px 0', borderBottom: `1px solid ${C.border}` }}>
                      <span style={{ fontSize: 11, color: C.text3 }}>Weak Side</span>
                      <span style={{ fontSize: 12, fontWeight: 700, fontFamily: NUM_FONT, color: matchesWeak ? C.orange : C.text }}>
                        {weakLabel}{matchesWeak ? ' ✓' : ''}
                      </span>
                    </div>
                  )}
                  {pb > 0 && <Row label="P-BABIP" value={pb.toFixed(3)} />}
                </div>
              </div>
              {clean(p?.note || p?.summary, '') !== '—' && clean(p?.note || p?.summary, '') !== '' && (
                <div style={{ background: C.bg3, border: `1px solid ${C.border}`, borderRadius: 10, padding: 12, fontSize: 12, color: C.text2, lineHeight: 1.5 }}>
                  {clean(p?.note || p?.summary)}
                </div>
              )}
            </>
          )}

          {/* ev log */}
          {tab === 'ev' && <EVLog player={p} bbeRange={bbeRange} />}

          {/* pitch breakdown */}
          {tab === 'pitch' && <PitchBreakdown player={p} />}

          {/* spray chart */}
          {tab === 'spray' && <SprayChart players={[p]} embedded={true} bbeRange={bbeRange} />}

          {/* hot zone map */}
          {tab === 'zones' && (
            <HotZoneMap
              player={p}
              onClose={null}
            />
          )}

        </div>
      </div>
    </div>
  )
}
import {
  compactRole, roleColor, gradeFor, signalPills, bestBet,
  playerState, playerStateNote, pitcherWeaknessProfile, matchesPitcherWeakness, STATE_STYLE,
} from '../lib/scoring'
import { Chip } from './ui'

function Row({ label, value, mono = true, highlight = null }) {
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', padding: '7px 0', borderBottom: `1px solid ${C.border}` }}>
      <span style={{ fontSize: 11, color: C.text3 }}>{label}</span>
      <span style={{
        fontSize: 12,
        color: highlight || C.text,
        fontFamily: mono ? NUM_FONT : 'inherit',
        fontWeight: 600
      }}>{value}</span>
    </div>
  )
}

function SectionLabel({ children, top = 14 }) {
  return (
    <div style={{
      fontSize: 10,
      color: C.text3,
      fontWeight: 800,
      textTransform: 'uppercase',
      letterSpacing: .5,
      padding: `${top}px 0 4px`,
    }}>
      {children}
    </div>
  )
}

// Arsenal table — pitcher's per-pitch usage + damage allowed
function ArsenalTable({ arsenal, matchPitch }) {
  if (!arsenal || !arsenal.length) return null
  const meaningful = arsenal
    .filter((p) => n(p.usage_pct, 0) >= 5 && n(p.bbe_allowed, 0) >= 5)
    .sort((a, b) => n(b.usage_pct, 0) - n(a.usage_pct, 0))
    .slice(0, 5)
  if (!meaningful.length) return null

  return (
    <table style={{ width: '100%', fontSize: 11, borderCollapse: 'collapse', fontFamily: NUM_FONT }}>
      <thead>
        <tr style={{ color: C.text3, fontSize: 10 }}>
          <th style={{ textAlign: 'left', padding: '4px 0', fontWeight: 600 }}>Pitch</th>
          <th style={{ textAlign: 'right', padding: '4px 0', fontWeight: 600 }}>Use</th>
          <th style={{ textAlign: 'right', padding: '4px 0', fontWeight: 600 }}>HR</th>
          <th style={{ textAlign: 'right', padding: '4px 0', fontWeight: 600 }}>HH%</th>
          <th style={{ textAlign: 'right', padding: '4px 0', fontWeight: 600 }}>BBE</th>
        </tr>
      </thead>
      <tbody>
        {meaningful.map((row, i) => {
          const pt = row.pitch_type || row.pitch_code
          const isMatch = matchPitch && pt === matchPitch
          const usage = n(row.usage_pct, 0)
          const hr = n(row.hr_allowed, 0)
          const hh = n(row.hard_hit_rate_allowed, 0) * 100
          const bbe = n(row.bbe_allowed, 0)
          const rowBg = isMatch ? 'rgba(29,158,117,0.08)' : 'transparent'
          const ptStyle = isMatch
            ? { color: '#fff', borderLeft: '2px solid #5DCAA5', paddingLeft: 5 }
            : { color: C.text }
          return (
            <tr key={i} style={{ background: rowBg }}>
              <td style={{ padding: '4px 0 4px 0' }}>
                <span style={{ ...ptStyle, fontWeight: 600 }}>{pt}</span>
                {isMatch && <span style={{ marginLeft: 6, color: '#5DCAA5', fontSize: 9, fontWeight: 600 }}>MATCH</span>}
              </td>
              <td style={{ textAlign: 'right', padding: '4px 0', color: C.text2 }}>{usage.toFixed(0)}%</td>
              <td style={{ textAlign: 'right', padding: '4px 0', color: hr > 0 ? '#F0997B' : C.text2 }}>{hr}</td>
              <td style={{ textAlign: 'right', padding: '4px 0', color: hh >= 40 ? '#F0997B' : C.text2 }}>{hh.toFixed(0)}%</td>
              <td style={{ textAlign: 'right', padding: '4px 0', color: C.text3 }}>{bbe}</td>
            </tr>
          )
        })}
      </tbody>
    </table>
  )
}

export default function PlayerModal({ player, onClose }) {
  if (!player) return null
  const p = player
  const role = compactRole(p)
  const rc = roleColor(role, C)
  const pills = signalPills(p, C, 'hr')
  const b = babipVal(p), pb = pitcherBabipVal(p)

  // NEW state-related fields
  const state = playerState(p)
  const stateStyle = STATE_STYLE[state] || STATE_STYLE.neutral
  const stateNote = playerStateNote(p, state)
  const weakness = pitcherWeaknessProfile(p)
  const weaknessMatches = weakness && matchesPitcherWeakness(p, weakness)

  // NEW pitch mix fields
  const arsenal = Array.isArray(p?.pitcher_pitch_arsenal_detail) ? p.pitcher_pitch_arsenal_detail : []
  const mixSide = clean(p?.pitcher_pitch_mix?.batter_side, 'ALL')
  const primaryMix = clean(p?.pitcher_primary_mix, '—')

  // Batter pitch totals (sum across pitch types)
  const byPitch = p?.batter_pitch_type_profile?.by_pitch || {}
  let total_hh_97 = 0, total_hr_by_pitch = 0, total_bbe = 0
  Object.values(byPitch).forEach((d) => {
    total_hh_97 += n(d?.hh_97_count, 0)
    total_hr_by_pitch += n(d?.hr, 0)
    total_bbe += n(d?.bbe, 0)
  })

  // Pitch-type match
  const matchFlag = !!p?.pitch_type_match_flag
  const matchCode = clean(p?.pitch_type_match_code, '')
  const matchNote = clean(p?.pitch_type_match_note, '')
  const matchScore = n(p?.pitch_type_match_score, 0)

  // Advanced pitcher stats
  const meatball = n(p?.pitcher_meatball_pct, 0) * 100
  const pullairAllowed = n(p?.pitcher_pullair_allowed_pct, 0) * 100
  const putaway = n(p?.pitcher_putaway_pct, 0) * 100
  const swstr = n(p?.pitcher_swstr_pct, 0) * 100
  const fps = n(p?.pitcher_first_pitch_strike_pct, 0) * 100

  return (
    <div
      className="modal-backdrop"
      onClick={onClose}
      style={{
        position: 'fixed', inset: 0, zIndex: 100,
        background: 'rgba(0,0,0,.7)', backdropFilter: 'blur(6px)',
        display: 'flex', alignItems: 'center', justifyContent: 'center',
        padding: 16,
      }}
    >
      <div
        className="modal-box"
        onClick={(e) => e.stopPropagation()}
        style={{
          background: C.bg2, border: `1px solid ${C.border2}`, borderRadius: 18,
          width: 520, maxWidth: '100%', maxHeight: '90vh', overflowY: 'auto',
          boxShadow: state !== 'neutral' && state !== 'cold' ? `0 0 0 1px ${stateStyle.ring}, 0 0 24px -8px ${stateStyle.glow}` : 'none',
        }}
      >
        <div className="modal-content" style={{ padding: '18px 20px 22px' }}>
          {/* Header */}
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
            <div style={{ minWidth: 0, flex: 1 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                <div style={{ fontSize: 19, fontWeight: 900 }}>{nameOf(p)}</div>
                {stateStyle.label && (
                  <>
                    <span style={{
                      width: 8, height: 8, borderRadius: '50%',
                      background: stateStyle.color,
                      boxShadow: stateStyle.dotShadow,
                    }} />
                    <span style={{
                      fontSize: 11, color: stateStyle.textColor,
                      fontWeight: 600, textTransform: 'uppercase', letterSpacing: 0.5,
                    }}>{stateStyle.label}</span>
                    {stateNote && (
                      <span style={{ fontSize: 11, color: stateStyle.textColor, opacity: 0.7 }}>· {stateNote}</span>
                    )}
                  </>
                )}
              </div>
              <div style={{ fontSize: 11, color: C.text3, fontFamily: NUM_FONT, marginTop: 3 }}>
                {teamOf(p)} vs {oppOf(p)} · Lineup #{clean(p?.lineup_spot, '?')} · {clean(p?.handedness || p?.bats, '?')}HB
              </div>
              {weakness && weaknessMatches && (
                <div style={{ fontSize: 10, color: '#9FE1CB', marginTop: 4 }}>
                  matches {weakness.tag}
                </div>
              )}
            </div>
            <button onClick={onClose} style={{ background: 'transparent', border: 'none', color: C.text3, fontSize: 22, cursor: 'pointer', lineHeight: 1 }}>✕</button>
          </div>

          {/* Existing chip row */}
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 14 }}>
            <Chip color={rc}>{role}</Chip>
            <Chip color={C.text2}>{bestBet(p, 'hr')}</Chip>
            <Chip color={C.text2}>Grade {gradeFor(p, 'hr')}</Chip>
            {pills.map((x, i) => (<Chip key={i} color={x.color}>{x.label}</Chip>))}
          </div>

          {/* Two-column main grid (same as before) */}
          <div className="stat-grid-two" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0 24px', marginBottom: 14 }}>
            <div>
              <SectionLabel top={8}>Model Scores</SectionLabel>
              <Row label="HR Score" value={hrScore(p).toFixed(1)} />
              <Row label="HRR Score" value={prodScore(p).toFixed(1)} />
              <Row label="Hit Score" value={hitScore(p).toFixed(1)} />
              <Row label="TB Score" value={tbScore(p).toFixed(1)} />
              <Row label="Pitch Mix" value={pitchMixScore(p).toFixed(1)} />
            </div>
            <div>
              <SectionLabel top={8}>Batted Ball</SectionLabel>
              <Row label="Avg EV" value={avgEV(p) ? avgEV(p).toFixed(1) + ' mph' : '—'} />
              <Row label="Max EV" value={maxEV(p) ? maxEV(p).toFixed(1) + ' mph' : '—'} />
              <Row label="Barrel %" value={pct(barrelRate(p))} />
              <Row label="Hard Hit %" value={pct(hardHitRate(p))} />
              <Row label="Launch Angle" value={launchAngle(p) ? launchAngle(p).toFixed(1) + '°' : '—'} />
              {total_bbe > 0 && (
                <Row label="HH 97+ (yr)" value={`${total_hh_97} · ${total_hr_by_pitch} HR`} highlight="#5DCAA5" />
              )}
            </div>
            <div>
              <SectionLabel>Recent Distance</SectionLabel>
              <Row label="350+ count" value={recent350(p)} />
              <Row label="375+ count" value={recent375(p)} />
              <Row label="400+ count" value={recent400(p)} />
              <Row label="Ideal HR %" value={ihrVal(p) ? (ihrVal(p) * 100).toFixed(1) + '%' : '—'} />
            </div>
            <div>
              <SectionLabel>Season</SectionLabel>
              <Row label="AVG" value={clean(p?.season_avg, '—')} />
              <Row label="HR" value={clean(p?.season_hr, '—')} />
              <Row label="PA" value={clean(p?.season_pa || p?.pa, '—')} />
              <Row label="K Rate" value={pct(p?.season_k_rate)} />
              {b > 0 && <Row label="BABIP" value={b.toFixed(3)} />}
            </div>
            <div>
              <SectionLabel>Splits</SectionLabel>
              {avgVsRHP(p) > 0 && <Row label="vs RHP" value={avgVsRHP(p).toFixed(3)} />}
              {avgVsLHP(p) > 0 && <Row label="vs LHP" value={avgVsLHP(p).toFixed(3)} />}
              <Row label="L5 Hits" value={n(p?.last5_hits, 0)} />
              <Row label="L5 HR" value={n(p?.last5_hr, 0)} />
              <Row label="L5 XBH" value={n(p?.last5_xbh, 0)} />
            </div>
            <div>
              <SectionLabel>Opposing Pitcher</SectionLabel>
              <Row label="Name" value={clean(p?.pitcher_name, '—')} mono={false} />
              <Row label="WHIP" value={sc(p?.pitcher_whip)} />
              <Row label="HR/9" value={sc(p?.pitcher_hr9)} />
              {pb > 0 && <Row label="P-BABIP" value={pb.toFixed(3)} />}
            </div>
          </div>

          {/* NEW: Advanced pitcher stats */}
          {(meatball > 0 || pullairAllowed > 0 || putaway > 0) && (
            <>
              <SectionLabel top={6}>Advanced Pitcher Stats</SectionLabel>
              <div style={{
                display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '8px 12px',
                padding: '10px 12px', background: C.bg3, border: `1px solid ${C.border}`,
                borderRadius: 10, marginBottom: 14, fontFamily: NUM_FONT,
              }}>
                <div>
                  <div style={{ fontSize: 10, color: C.text3 }}>Meatball</div>
                  <div style={{ fontSize: 13, color: meatball >= 8.5 ? '#F0997B' : C.text, fontWeight: 600 }}>{meatball.toFixed(1)}%</div>
                </div>
                <div>
                  <div style={{ fontSize: 10, color: C.text3 }}>Pull-Air</div>
                  <div style={{ fontSize: 13, color: pullairAllowed >= 25.5 ? '#F0997B' : C.text, fontWeight: 600 }}>{pullairAllowed.toFixed(1)}%</div>
                </div>
                <div>
                  <div style={{ fontSize: 10, color: C.text3 }}>PutAway</div>
                  <div style={{ fontSize: 13, color: C.text, fontWeight: 600 }}>{putaway.toFixed(0)}%</div>
                </div>
                <div>
                  <div style={{ fontSize: 10, color: C.text3 }}>SwStr</div>
                  <div style={{ fontSize: 13, color: C.text, fontWeight: 600 }}>{swstr.toFixed(0)}%</div>
                </div>
                <div>
                  <div style={{ fontSize: 10, color: C.text3 }}>1st-PS</div>
                  <div style={{ fontSize: 13, color: C.text, fontWeight: 600 }}>{fps.toFixed(0)}%</div>
                </div>
              </div>
            </>
          )}

          {/* NEW: Arsenal table */}
          {arsenal.length > 0 && (
            <>
              <SectionLabel top={6}>
                Arsenal {mixSide && mixSide !== 'ALL' ? `vs ${mixSide}HB` : ''} · {primaryMix}
              </SectionLabel>
              <div style={{
                padding: '8px 12px', background: C.bg3, border: `1px solid ${C.border}`,
                borderRadius: 10, marginBottom: 14,
              }}>
                <ArsenalTable arsenal={arsenal} matchPitch={matchFlag ? matchCode : null} />
              </div>
            </>
          )}

          {/* NEW: Pitch-type match callout */}
          {matchFlag && matchNote && (
            <div style={{
              padding: '10px 12px',
              background: 'rgba(29,158,117,0.08)',
              border: '1px solid rgba(29,158,117,0.25)',
              borderRadius: 10,
              marginBottom: 14,
              fontSize: 11,
              color: '#9FE1CB',
              lineHeight: 1.5,
            }}>
              <div style={{ fontWeight: 700, color: '#5DCAA5', marginBottom: 3 }}>
                Pitch-Type Match (score {matchScore.toFixed(0)})
              </div>
              {matchNote}
            </div>
          )}

          {/* Existing note / summary */}
          {clean(p?.note || p?.summary, '') !== '—' && clean(p?.note || p?.summary, '') !== '' && (
            <div style={{ background: C.bg3, border: `1px solid ${C.border}`, borderRadius: 10, padding: 12, fontSize: 12, color: C.text2, lineHeight: 1.5 }}>
              {clean(p?.note || p?.summary)}
            </div>
          )}
        </div>
      </div>
    </div>
  )
}
