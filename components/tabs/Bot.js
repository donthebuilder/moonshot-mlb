...
export default function Bot({ players = [], onPlayerClick }) {
  ...
  return (
    <div>
      ...
      {view==='log'      && <LogViewer url={dataUrl("current/today.txt")}    label="Today's Bot Log" />}
      {view==='tomorrow' && <LogViewer url={dataUrl("current/tomorrow.txt")} label="Tomorrow's Bot Log" />}
    </div>
  )
}
