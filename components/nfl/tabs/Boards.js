...
// FORM BADGE (2026-09-21, replacing the sparkline). Donovan, off "the boards
// page" feedback: the SVG line had no axis, no units, and the dotted
// market-bar line this component drew was too faint to read at this size --
// so the shape looked like decoration, not information. Same recentForm()
// data (last 8 games, hits vs the market bar, recent-half-vs-prior-half
// delta) -- this just says it in words instead of asking you to read a
// 66x27px chart.
function FormBadge({ form, color }) {
  ...
  return (
    <span
      title={`${form.hits}/${form.points.length} cleared the market bar in the last ${form.points.length} games`}
      style={{ display: 'flex', alignItems: 'baseline', gap: 5, fontFamily: NUM_FONT }}
    >
      <b style={{ color: directionColor, fontSize: 11, lineHeight: 1 }}>{direction}</b>
      <span style={{ fontSize: TYPE.micro, color: C.text2 }}>
        <b style={{ color: hot ? color : C.text2 }}>{form.hits}/{form.points.length}</b> cleared · {trend}
      </span>
    </span>
  )
}
          ...
          return (
            ...
            >
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 6 }}>
                <FormBadge form={form} color={g.color} />
