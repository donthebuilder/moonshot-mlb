  ...
  return (
    <section className="tuddy-panel">
      ...
      <div className="tuddy-ledger-list">
        {scorers.slice(0, 8).map((row) => <div key={row.playerId}><span>{row.player?.team || 'NFL'}</span><b>{row.player?.name || results?.names?.[row.playerId] || row.playerId}</b><em>{row.touchdowns} TD{row.touchdowns === 1 ? '' : 's'}</em></div>)}
  ...
  return (
    <section className="tuddy-mini-board">
      ...
      {rows.map((player, index) => {
        ...
        return <button key={player.player_id} onClick={() => onPlayerClick?.(player, market)}><span>{index + 1}</span><b>{player.name}</b><em>{player.team} · {player.position}</em><strong style={{ color: grade.color }}>{Math.round(player.scores[market])}</strong></button>
      })}
    </section>
  )
}
