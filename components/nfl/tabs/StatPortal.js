...
function Storylines({ player, market, rows, matchup }) {
  ...
  const bullets = [
    bestMarket && `${bestMarket} is his strongest DASH lane at ${Math.round(bestScore)} (${gradeFor(bestScore).label}).`,
    player.questionable && 'Injury status is questionable; the slate row should be rechecked before kickoff.',
    player.carryover && 'The current score leans on last season’s per-game baseline until current-season form has depth.',
    role && defense && `${player.opp} ranks #${defense.td_rank ?? '—'} in TDs allowed to the ${role} role (rank 1 is softest).`,
    lastFive.length && `${lastFive.length} recent published games are available for the ${MARKET_LOG[market]?.[1] || market} trend.`,
  ].filter(Boolean)
  const query = encodeURIComponent(`${player.name} ${player.team} NFL`)
  return <section className="portal-card portal-story"><div className="portal-card-head"><div><small>STORYLINE DESK</small><h2>What the data says</h2></div></div>{bullets.map((text, index) => <article key={text}><span>0{index + 1}</span><p>{text}</p></article>)}<div className="portal-news"><small>LATEST COVERAGE · LINKS ONLY</small><a href={`https://www.espn.com/search/_/q/${encodeURIComponent(player.name)}`} target="_blank" rel="noreferrer">Search ESPN for {player.name} ↗</a><a href={`https://news.google.com/search?q=${query}`} target="_blank" rel="noreferrer">Search recent headlines ↗</a></div></section>
}
