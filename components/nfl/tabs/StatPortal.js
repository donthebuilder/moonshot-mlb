...
function Splits({ player, group, setGroup }) {
  ...
  return <section className="portal-card"><div className="portal-card-head"><div><small>FILTER THE PLAYER</small><h2>Splits</h2></div><select value={group} onChange={(event) => setGroup(Number(event.target.value))}>{SPLIT_GROUPS.map((row, index) => <option value={index} key={row[2]}>{row[2]}</option>)}</select></div><div className="portal-split-labels"><b>{left}</b><span>per-game rates · sample beside each side</span><b>{right}</b></div>{keys.map((key) => { const aAbs = Number.isFinite(Number(A[key])) ? Math.abs(Number(A[key])) : 0; const bAbs = Number.isFinite(Number(B[key])) ? Math.abs(Number(B[key])) : 0; const total = aAbs + bAbs; const aPct = total ? (aAbs / total) * 100 : 50; return <div className="portal-split-row" key={key}><strong>{nice(A[key])}<small>{A.g ? `${A.g}g` : ''}</small></strong><span>{key.toUpperCase()}</span><strong>{nice(B[key])}<small>{B.g ? `${B.g}g` : ''}</small></strong>{total > 0 && <div className="portal-split-bar" aria-hidden="true"><i style={{ width: `${aPct}%` }} /><i style={{ width: `${100 - aPct}%` }} /></div>}</div> })}{!keys.length && <p className="portal-empty">This split is not published for the selected player.</p>}</section>
}
  ...
  return (
    <aside className="portal-directory">
      ...
      <div className="portal-dir-list">{rows.map((player) => {
        ...
        return <button key={player.player_id} className={selected?.player_id === player.player_id ? 'active' : ''} onClick={() => choose(player)}><span>{player.team}</span><div><b>{player.name}</b><small>{player.position} · {oppLabel(player)}</small></div><strong>{onBye(player) ? 'BYE' : noData(player) ? '—' : Math.round(best)}</strong></button>
      })}</div>
    </aside>
  )
}
