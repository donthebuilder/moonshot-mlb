  ...
  return (
    <aside className="portal-directory">
      ...
      <div className="portal-dir-list">{rows.map((player) => {
        ...
        return <button key={player.player_id} className={selected?.player_id === player.player_id ? 'active' : ''} onClick={() => choose(player)}><span>{player.team}</span><div><b>{player.name}</b><small>{player.position} · {player.roster_only ? player.roster_status : oppLabel(player)}</small></div><strong>{onBye(player) ? 'BYE' : noData(player) ? '—' : Math.round(best)}</strong></button>
      })}</div>
    </aside>
  )
}
...
function useRosterExtras(players) {
