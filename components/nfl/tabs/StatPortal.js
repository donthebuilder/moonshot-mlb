...
import { oppLabel, oppShort, onBye } from '../../../lib/nfl/oppLabel'
  ...
  return (
    <aside className="portal-directory">
      ...
      <div className="portal-dir-list">{rows.map((player) => {
        ...
        return <button key={player.player_id} className={selected?.player_id === player.player_id ? 'active' : ''} onClick={() => choose(player)}><span>{player.team}</span><div><b>{player.name}</b><small>{player.position} · {oppLabel(player)}</small></div><strong>{onBye(player) ? 'BYE' : Math.round(best)}</strong></button>
      })}</div>
    </aside>
  )
}
