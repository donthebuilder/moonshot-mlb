  ...
  return (
    <div style={{ margin: '10px 0 2px', padding: '9px 11px', borderRadius: 10, border: `1px solid ${hits ? 'rgba(0,245,173,.4)' : 'rgba(248,113,113,.35)'}`, background: hits ? 'rgba(0,245,173,.06)' : 'rgba(248,113,113,.05)' }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5 }}>
        {rows.map((r) => (
          <span key={r.m} title={`${short(r.m)}: ${r.v} against a bar of ${r.bar}`} style={{ display: 'inline-flex', alignItems: 'baseline', gap: 4, padding: '4px 8px', borderRadius: 7, border: `1px solid ${r.hit ? 'rgba(0,245,173,.45)' : C.border}`, background: r.hit ? 'rgba(0,245,173,.08)' : 'transparent', fontFamily: NUM_FONT }}>
