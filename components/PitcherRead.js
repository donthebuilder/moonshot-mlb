...
const HOT = C.orange          // good for the bats facing him
const COLD = '#60a5fa'        // his strength — same code the tiles use
...
export default function PitcherRead({ name, throws, stats, lineup }) {
  ...
  if (era != null || whip != null) {
    const traffic = whip == null ? null
      : whip >= 1.4 ? { t: 'the bases stay crowded — RBI chances ride with every knock', col: HOT }
      : whip <= 1.1 ? { t: 'he keeps the bases quiet', col: COLD }
      : null
    lines.push(
      <Line key="line" icon="🥎">
        <b style={{ color: C.text }}>The line:</b> a {throws ? `${throws}HP` : 'starter'} carrying
        {era != null && <> a <B col={era >= 5 ? HOT : era <= 3.2 ? COLD : C.text}>{era.toFixed(2)} ERA</B></>}
        {era != null && whip != null && <> on</>}
        {whip != null && <> a <B col={whip >= 1.4 ? HOT : whip <= 1.1 ? COLD : C.text}>{whip.toFixed(2)} WHIP</B></>}
  ...
  if (hr9 != null || brlAllowed != null || hhAllowed != null) {
    ...
    lines.push(
      <Line key="leak" icon={leaky ? '💣' : wall ? '🧱' : '⚾'}>
        ...
        {hr9 != null && <>gives up <B col={hr9 >= 1.3 ? HOT : hr9 <= 0.85 ? COLD : C.text}>{hr9.toFixed(2)} HR/9</B></>}
        {hr9 != null && (brlAllowed != null || hhAllowed != null || fbAllowed != null) && <>, with </>}
        {brlAllowed != null && <>barrels at <B col={brlAllowed >= 0.09 ? HOT : brlAllowed <= 0.05 ? COLD : C.text}>{(100 * brlAllowed).toFixed(1)}%</B></>}
        {brlAllowed != null && hhAllowed != null && <> and </>}
        {hhAllowed != null && <>hard contact at <B col={hhAllowed >= 0.42 ? HOT : hhAllowed <= 0.33 ? COLD : C.text}>{(100 * hhAllowed).toFixed(0)}%</B></>}
        {fbAllowed != null && <> on a <B col={fbAllowed >= 0.42 ? HOT : fbAllowed <= 0.32 ? COLD : C.text}>{(100 * fbAllowed).toFixed(0)}%</B> fly-ball diet</>}
        {leaky ? <> — <b style={{ color: HOT }}>a live power window</b></> : wall ? <> — <b style={{ color: COLD }}>a wall; power picks need another reason</b></> : null}.
      </Line>
    )
  }
  ...
  if (l3n > 0 && l3hr9 != null && hr9 != null) {
    ...
    lines.push(
      <Line key="trend" icon={d >= 0.4 ? '📈' : d <= -0.4 ? '📉' : '➖'}>
        ...
        <B col={d >= 0.4 ? HOT : d <= -0.4 ? COLD : C.text}>{l3hr9.toFixed(2)} HR/9</B> against his{' '}
        <B>{hr9.toFixed(2)}</B> season —{' '}
        {d >= 0.4 ? <b style={{ color: HOT }}>getting hit lately</b>
          : d <= -0.4 ? <b style={{ color: COLD }}>tighter than his season line lately</b>
          : <>about his norm</>}.
  ...
  if (weakSide && nBats > 0) {
    lines.push(
      <Line key="side" icon="🎯">
        <b style={{ color: C.text }}>The side:</b> he is weakest against <b style={{ color: HOT }}>{weakSide}</b>
        {sideChar && <> — <B col={onSide >= 4 ? HOT : C.text}>{onSide}</B> of tonight&apos;s <B>{nBats}</B> tracked
    ...
    lines.push(
      <Line key="threat" icon="⚔️">
        <b style={{ color: C.text }}>The threats:</b> {top.map((b, i) => (
          <span key={b.name}>{i > 0 && ' and '}<b style={{ color: C.text }}>{b.name}</b> <B col={b.hr >= 70 ? HOT : C.text}>{b.hr.toFixed(0)}</B></span>
        ))} lead the board against him
        {edges > 0 && <>, and <B col={edges >= 3 ? HOT : C.text}>{edges}</B> {edges === 1 ? 'bat carries' : 'bats carry'} the
  ...
  if (k9 != null) {
    lines.push(
      <Line key="k" icon="🥊">
        <b style={{ color: C.text }}>The K prop:</b> <B col={k9 >= 9.5 ? COLD : k9 <= 7 ? HOT : C.text}>{k9.toFixed(1)} K/9</B> —{' '}
        {k9 >= 9.5 ? <><b style={{ color: COLD }}>swing-and-miss arm; his strikeout over is the market that belongs to him</b></>
          : k9 <= 7 ? <><b style={{ color: HOT }}>contact arm — more balls in play for the bats, and a thin case for his K over</b></>
  ...
  if (venue && parkHr != null) {
    lines.push(
      <Line key="park" icon="🏟">
        ...
        <B col={parkHr >= 1.05 ? HOT : parkHr <= 0.95 ? COLD : C.text}>
