...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const paths = mode === 'today'
      ? [dataUrl('current/today_slim.json'), dataUrl('current/today.json')]
      : [dataUrl('current/tomorrow_slim.json'), dataUrl('current/tomorrow.json')]
    ...
    Promise.allSettled([
      ...
      fetchJSON([dataUrl('current/results_live.json'), dataUrl('current/results.json')]).then((j) => { if (alive) setResults(j) }),
      fetchJSON([dataUrl('current/pair_builder_latest.json')]).then((j) => { if (alive) setPairBuilder(j) }),
      fetchJSON([dataUrl('current/pair_history_summary.json')]).then((j) => { if (alive) setPairSummary(j) }),
    ]).then(() => {
  ...
  const tabsWithoutPlayers = ['pairs', 'bot', 'results', 'guide', 'watch']
