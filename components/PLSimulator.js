...
export default function PLSimulator() {
  ...
  return (
    <div>
      <div style={{ fontSize: 12.5, fontWeight: 800, marginBottom: 2 }}>💰 P/L simulator</div>
      <div style={{ fontSize: 10, color: C.text3, marginBottom: 10, lineHeight: 1.55, maxWidth: 700 }}>
        Every graded pick over {data.meta.days} days ({data.meta.from} → {data.meta.to}), bet flat at
        the odds below. <b style={{ color: C.text2 }}>Set your book&apos;s real lines</b> — the whole
        answer swings on them, and the defaults are only round numbers. This is the cost of following
        the bot blindly: no line shopping, no skipping bad spots.
      </div>

      {/* odds inputs */}
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'flex-end', marginBottom: 12 }}>
        <label style={{ fontSize: 9.5, color: C.text3 }}>
          Stake / pick ($)
          <input
      ...
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 10 }}>
          ...
          return (
            <div key={k} style={{
              ...
              <div style={{ fontFamily: NUM_FONT, fontSize: 13, fontWeight: 900, color: t.pl >= 0 ? '#4ade80' : '#f87171' }}>
                {t.pl >= 0 ? '+' : '−'}${Math.abs(t.pl).toFixed(0)}
              </div>
        ...
        <div style={{
          ...
          <div style={{
            ...
            {sim.totals.TOTAL.pl >= 0 ? '+' : '−'}${Math.abs(sim.totals.TOTAL.pl).toFixed(0)}
          </div>
      ...
      <div style={{ fontSize: 9.5, color: C.text3, marginTop: 6, lineHeight: 1.55 }}>
        White is the combined book. <b style={{ color: C.text2 }}>need vs got</b> on each tile is the
        whole verdict: the break-even win rate your odds demand against the rate the archive actually
        produced — a category can show green picks and still bleed money if the price is wrong.
        Flat stakes, archived results, your odds; past performance isn&apos;t tomorrow&apos;s slate.
      </div>
    </div>
  )
}
