...
export default function DashNetworkNav() {
  ...
  // On the board itself, switching sport is a state flip, not a navigation —
  // a full page load would throw away the slate that is already fetched. Off
  // it (the front door, Franchise) the link navigates normally.
  const switchProduct=(event,key)=>{if(pathname==='/app'&&key!=='fantasy'){event.preventDefault();setSport(key)}setOpen(false)}
