...
export default function LampDashboard({ palettePass = 0 }) {
  ...
  useEffect(() => {
    try { document.title = `${pageTitle('nhl', tab)} · DASH Network` } catch { /* ignore */ }
  }, [tab])
      ...
      <main id="board-main" className="dashboard-main" style={{ maxWidth: 1300, margin: '0 auto', padding: '14px 14px 40px', background: C.bg, color: C.text }}>
        ...
        {missingTab ? (
          <TabNotFound
            ...
            palette={C}
            onNavigate={setTab}
            doors={[['home', '\u{1F3E0} TONIGHT'], ['scores', '\u{1F4E1} SCORES'], ['schedule', '\u{1F4C5} SCHEDULE'], ['standings', '\u{1F4CA} STANDINGS'], ['guide', '\u{1F4D6} GUIDE']]}
          />
        ) : (
          <ErrorBoundary resetKey={`${tab}:${gameId || ''}`} label={`the ${tab} tab`}>
