...
export default function SprayFieldStadium({ hits = [], dims, heights, venue = '', wind = null, title = '', subtitle = '', live = false }) {
  ...
  useEffect(() => {
    ...
    if (windMph > 0 && windLabel) {
      ...
      // ── THE FLAG IS BACK (2026-09-02). Donovan: "I liked the flag, I just
      //    didn't like how it had like street lights." It was never the flag
      //    — it was the two light-tower beam cones washing across it. Those
      //    cones are now 0.009 opacity (they had become searchlights under
      //    the half-float post stack), so the cloth reads in the park's own
