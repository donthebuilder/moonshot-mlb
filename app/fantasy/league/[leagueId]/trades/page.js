...
export default async function TradesPage({params,searchParams}) {
  ...
  const [{data:league},{data:membership},{data:teams=[]},{data:rosters=[]},{data:trades=[]}]=await Promise.all([
...
function TradeCard({trade,teams,myTeam,commissioner,leagueId}) {
  ...
  return <article className={styles.tradeCard}><div className={styles.tradeCardTop}><span className={`${styles.tradeStatus} ${styles[`trade_${trade.status}`]||''}`}>{trade.status}</span><small>{new Date(trade.created_at).toLocaleDateString('en-US',{month:'short',day:'numeric'})}</small></div><div className={styles.tradeSummary}><div><b>{proposer?.name}</b>{offered.map((item)=><span key={item.id}>{item.player?.position} · {item.player?.name}</span>)}</div><em>⇄</em><div><b>{recipient?.name}</b>{requested.map((item)=><span key={item.id}>{item.player?.position} · {item.player?.name}</span>)}</div></div>{trade.note&&<p className={styles.tradeMessage}>“{trade.note}”</p>}<div className={styles.tradeActions}>{incoming&&<><TradeAction action={respondTrade} leagueId={leagueId} tradeId={trade.id} name="response" value="accepted" label="Accept"/><TradeAction action={respondTrade} leagueId={leagueId} tradeId={trade.id} name="response" value="rejected" label="Reject"/></>}{outgoing&&<TradeAction action={cancelTrade} leagueId={leagueId} tradeId={trade.id} label="Cancel offer"/>}{commissioner&&trade.status==='accepted'&&<><TradeAction action={reviewTrade} leagueId={leagueId} tradeId={trade.id} name="decision" value="approve" label="Approve trade"/><TradeAction action={reviewTrade} leagueId={leagueId} tradeId={trade.id} name="decision" value="veto" label="Veto"/></>}</div></article>
}

function TradeAction({action,leagueId,tradeId,name,value,label}) {
  return <form action={action}><input type="hidden" name="leagueId" value={leagueId}/><input type="hidden" name="tradeId" value={tradeId}/>{name&&<input type="hidden" name={name} value={value}/>}<button>{label}</button></form>
}
