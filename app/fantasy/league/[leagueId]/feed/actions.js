...
async function run(formData,name,args,message) {
  ...
  revalidatePath(`/fantasy/league/${leagueId}/feed`)
...
export async function toggleReaction(formData) {
  return run(formData,'toggle_fantasy_feed_reaction',{p_post_id:String(formData.get('postId')||''),p_reaction:String(formData.get('reaction')||'')},'Reaction updated')
}
