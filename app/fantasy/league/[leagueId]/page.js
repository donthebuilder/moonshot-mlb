...
import { addToQueue, assignDraftPick, draftPlayer, prepareDraft, removeFromQueue, setDraftState, startDraft, syncPlayerCatalog, tickAutoPick } from './actions'
        ...
        <div className={styles.draftLayout}>
          <section className={styles.playerBoard}>
            ...
            {!players.length ? <p className={styles.emptyRoom}>Commissioner: sync the NFL player catalog to begin.</p> : available.slice(0,80).map((player) => (
              <div className={styles.draftPlayer} key={player.id}><span className={styles.rankNumber}>{rankedPlayers.findIndex((ranked)=>ranked.id===player.id)+1}</span><span className={styles.positionTag} style={{ color: colorForPosition(player.position), borderColor: `${colorForPosition(player.position)}55` }}>{player.position}</span><div className={styles.playerIdentity}><NflTeamMark team={player.team}/><span><b>{player.name}</b><small>{player.team || 'FA'}{player.injury_status ? ` · ${player.injury_status}` : ''}</small></span></div><strong className={styles.playerDash} style={{ color: colorForPosition(player.position) }}>{player.dash_score}</strong>{canPick?<form action={draftPlayer}><input type="hidden" name="leagueId" value={leagueId}/><input type="hidden" name="playerId" value={player.id}/><SubmitButton pendingLabel="Drafting…">Draft</SubmitButton></form>:draft&&myTeam?<form action={addToQueue}><input type="hidden" name="leagueId" value={leagueId}/><input type="hidden" name="playerId" value={player.id}/><SubmitButton className={styles.queueButton} pendingLabel="Adding…">+ Queue</SubmitButton></form>:<span className={styles.lockedPick}>{draft?.status === 'paused' ? 'PAUSED' : draft?.status === 'live' ? 'WAIT' : 'SCOUT'}</span>}</div>
            ))}
          </section>
          <aside className={styles.draftSide}>
            <section><div className={styles.boardHead}><div><p className={styles.panelLabel}>DRAFT ORDER</p><h2>{draft?.status === 'paused' ? 'Draft paused' : 'On the clock'}</h2></div>{draft?.status==='live'&&<span className={styles.clockChip}><DraftRoomLive currentPick={draft.current_overall_pick} deadline={draft.pick_deadline} leagueId={leagueId} status={draft.status} tickAction={tickAutoPick}/></span>}</div>{picks.slice(Math.max(0,(draft?.current_overall_pick||1)-3),Math.max(8,(draft?.current_overall_pick||1)+5)).map((pick)=><div className={pick.overall_pick===draft?.current_overall_pick?styles.activePick:styles.pickRow} key={pick.id}><span>{pick.overall_pick}</span><b>{teams.find((team)=>team.id===pick.team_id)?.name}</b><small>{rankedPlayers.find((player)=>player.id===pick.player_id)?.name || '—'}</small></div>)}{!picks.length&&<p className={styles.emptyRoom}>Draft order not prepared.</p>}</section>
            {draft && <section><div className={styles.boardHead}><div><p className={styles.panelLabel}>MY QUEUE</p><h2>Auto-pick priority</h2></div><span>{queue.length}</span></div>{queue.map((item)=><div className={styles.queueRow} key={item.id}><span>{item.rank}</span><div><b>{item.player.name}</b><small>{item.player.position} · {item.player.team}</small></div><form action={removeFromQueue}><input type="hidden" name="leagueId" value={leagueId}/><input type="hidden" name="playerId" value={item.player_id}/><SubmitButton aria-label={`Remove ${item.player.name} from queue`} pendingLabel="…">×</SubmitButton></form></div>)}{!queue.length&&<p className={styles.emptyRoom}>Queue players from the board. If your timer expires, the first available name is selected.</p>}</section>}
