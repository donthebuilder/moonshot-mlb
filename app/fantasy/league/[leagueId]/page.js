    ...
    <main className={styles.roomApp}>
      ...
      <div className={styles.roomBody}>
        ...
        <section className={styles.draftHero}>
          {/* ── THE ROOM STILL SAID "BUILD THE DRAFT BOARD" IN WEEK 1 ──────────
              A completed draft fell into the same else-branch as a league that
              has not drafted at all: 135 of 135 picks in, rosters set, and the
              page still opened with the instruction to go build it. Complete is
              its own state now, and it points at the two pages that matter once
              the board is done. */}
          <div><p className={styles.panelLabel}>{draftComplete ? 'LEAGUE ROOM' : 'LIVE LEAGUE ROOM'}</p><h1>{draft?.status === 'live' ? `Pick ${draft.current_overall_pick} is on the clock` : draftComplete ? 'The board is done.' : 'Build the draft board.'}</h1><p>{draft?.status === 'live' ? `${currentTeam?.name || 'Next team'} · ${draft.timer_seconds} second timer` : draftComplete ? <>Every pick is in. Set your week in <Link href={`/fantasy/league/${leagueId}/team`}>My Team</Link>, or work the <Link href={`/fantasy/league/${leagueId}/wire`}>Wire</Link>.</> : `${String(league.scoring||'ppr').replace('_','-').toUpperCase()} · ${league.draft_order_method} order · ${league.draft_timer_seconds}s picks`}</p></div>
        ...
        {/* Prepare snake draft was never disabled, so a finished league kept a
            live button that rebuilds the draft order -- next to a Start draft
        ...
        <div className={styles.draftLayout}>
          ...
          <aside className={styles.draftSide}>
            ...
            {draft && !draftComplete && <section><div className={styles.boardHead}><div><p className={styles.panelLabel}>MY QUEUE</p><h2>Auto-pick priority</h2></div><span>{queue.length}</span></div>{queue.map((item)=><div className={styles.queueRow} key={item.id}><span>{item.rank}</span><div><b>{item.player.name}</b><small>{item.player.position} · {item.player.team}</small></div><form action={removeFromQueue}><input type="hidden" name="leagueId" value={leagueId}/><input type="hidden" name="playerId" value={item.player_id}/><input type="hidden" name="viewPosition" value={selectedPosition}/><input type="hidden" name="viewQuery" value={String(query?.q||'')}/><SubmitButton aria-label={`Remove ${item.player.name} from queue`} pendingLabel="…">×</SubmitButton></form></div>)}{!queue.length&&<p className={styles.emptyRoom}>Queue players from the board. If your timer expires, the first available name is selected.</p>}</section>}
